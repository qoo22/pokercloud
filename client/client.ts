...
import type { ActionType, LegalAction } from '../src/table.js';
import {
  cardFace,
  cardBack as cardBackSvg,
  cardSlot,
  VISUAL_CSS,
  showHandBanner,
  flashAllIn,
...
const each = <T extends Element>(sel: string, fn: (el: T) => void) =>
  Array.from(document.querySelectorAll<T>(sel)).forEach(fn);
...
let balance = 0;
let tables: LobbyTableInfo[] = [];
let state: TableStateView | null = null;
let currentTableId: string | null = null;
let lastSummary: HandSummary | null = null;
/** 配牌前に受け取ったコミットメント。開示値と照合するため、こちらを正とする */
let committedHash: string | null = null;
let committedFor: string | null = null;
let mySeedForHand: string | null = null;
 ...
 */
let lastVerdict: { handNumber: number; passed: boolean; detail: string; query: string } | null = null;
/** 直前に描いたボードの枚数。増えた分だけ配布演出を出すために持つ */
let lastBoardLen = 0;
let lastPotShown = 0;
let theme: CardTheme = 'classic';
...
function handle(msg: ServerMessage): void {
  switch (msg.t) {
    ...
    case 'balance':
      balance = msg.balance;
      if (msg.gold !== undefined) goldBalance = msg.gold;
      renderBalance();
      renderLobby();
      renderTournaments();
      break;
    ...
    case 'shop.state':
      ...
      if (pendingBuySku) { const s = pendingBuySku; pendingBuySku = null; showPurchaseDialog(s); }
      break;
    ...
    case 'reward':
      ...
      if (activeTab === 'shop') send({ t: 'shop.list' });
      break;
    ...
    case 'table.state': {
      ...
      if (state.pot > lastPotShown) {
        const potEl = document.querySelector<HTMLElement>('.pot');
...
function renderBalance(): void {
  ...
  if (v && profile) v.textContent = `👑 ${profile.vip.tierName}`;
}
...
function renderLobby(): void {
  ...
  each<HTMLButtonElement>('[data-join]', (b) => {
...
function showBuyInDialog(info: LobbyTableInfo): void {
  ...
  modal(
    ...
    (close) => {
      const range = $<HTMLInputElement>('bi-range');
...
function modal(html: string, wire: (close: () => void) => void): void {
 ...
 */
const cardHtml = (code: string, size: 'sm' | 'md' | 'lg' = 'md', highlight = false, extra = '') =>
  cardFace(code, { size, highlight, extra });
const backHtml = (size: 'sm' | 'md' | 'lg' = 'sm', extra = '') => cardBackSvg({ size, extra });
...
function renderTable(): void {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const badges =
      (i === st.buttonIndex ? '<span class="badge">D</span>' : '') +
  ...
  $('felt').innerHTML = html;
  each<HTMLButtonElement>('[data-sit]', (b) => {
    b.onclick = () => {
 ...
 */
function renderBoard(st: TableStateView, winningCards: Set<string>): string {
 ...
 */
function celebrate(summary: HandSummary): void {
  ...
  for (const seatIdx of winners) {
    const el = felt.querySelector<HTMLElement>(`.seat[data-seat="${seatIdx}"]`);
    if (el) flyChips(felt, el, 6);
  }
...
function renderActions(): void {
  ...
  if (check) html += `<button data-act="check">チェック</button>`;
  if (call) html += `<button data-act="call">コール ${fmt(call.amount ?? 0)}</button>`;
  if (raise) html += `<button class="primary" data-act="raise">${raise.type === 'bet' ? 'ベット' : 'レイズ'} <span id="rv">${fmt(raise.min!)}</span></button>`;
  html += '</div>';
  ...
  const slider = document.getElementById('rs') as HTMLInputElement | null;
  if (slider) {
    slider.oninput = () => {
      const v = document.getElementById('rv');
      if (v) v.textContent = fmt(+slider.value);
    };
  }
  each<HTMLButtonElement>('[data-act]', (b) => {
    b.onclick = () => {
      const a = b.dataset.act as ActionType;
      const to = a === 'raise' && slider ? +slider.value : undefined;
      const realType = a === 'raise' ? (raise!.type as ActionType) : a;
  ...
  each<HTMLButtonElement>('[data-set]', (b) => {
    b.onclick = () => {
      if (!slider) return;
...
function buildPresets(st: TableStateView, raise: LegalAction): Array<[string, number]> {
  const clamp = (v: number) => Math.max(raise.min!, Math.min(raise.max!, Math.round(v)));
  return [
  ...
  let html = `<dl class="kv">
    ...
    <dt>サーバーシード</dt><dd>${
      revealed
        ...
        : '<span class="sealed">ハンド終了後に開示</span>'
    }</dd>
  </dl>`;
...
const shorten = (s: string) => (s.length > 20 ? `${s.slice(0, 10)}…${s.slice(-6)}` : s);
...
function logEvent(e: { type: string; [k: string]: unknown }): void {
  switch (e.type) {
    case 'blind':
      log(`${nameOfSeat(e.seat as number)} ${String(e.blind).toUpperCase()} ${fmt(e.amount as number)}`);
      break;
    case 'straddle':
      log(`${nameOfSeat(e.seat as number)} ストラドル ${fmt(e.amount as number)}`, 'hl');
      break;
    case 'street':
      if ((e.board as string[]).length) log(`— ${STREET_LABEL[e.street as string]} —`, 'hl');
      break;
    case 'action': {
      log(
        `${nameOfSeat(e.seat as number)} ${ACTION_JA[e.action as string]}${e.amount ? ` ${fmt(e.amount as number)}` : ''}${e.allIn ? ' (ALL IN)' : ''}`,
      );
      if (e.allIn) {
        const felt = document.getElementById('felt');
        if (felt) flashAllIn(felt);
      }
      break;
    }
    case 'uncalled_return':
      log(`${nameOfSeat(e.seat as number)} へ ${fmt(e.amount as number)} 返却`);
      break;
    case 'award':
      log(`${nameOfSeat(e.seat as number)} +${fmt(e.amount as number)}`, 'win');
      break;
  }
}

function logResult(s: HandSummary): void {
  if (s.showdown) {
...
$('btn-rebuy').onclick = () => {
  ...
  modal(
    ...
    (close) => {
      const range = $<HTMLInputElement>('rb-range');
...
function beep(freq: number, ms: number, gain = 0.14): void {
  if (!soundOn) return;
  initAudio();
  const ctx = audioCtx;
  if (!ctx) return;
...
/** 手番が回ってきた合図（上がる2音） */
function soundYourTurn(): void { beep(660, 110); setTimeout(() => beep(880, 130), 120); }
/** 残り30秒（落ち着いた単音） */
function soundWarn30(): void { beep(560, 160); }
/** 残り15秒（強めの2連打） */
function soundWarn15(): void { beep(760, 130); setTimeout(() => beep(760, 160), 190); }
/** 残り5秒からの毎秒カウント（短く鋭い） */
function soundTick(): void { beep(980, 70, 0.1); }

export function setSoundEnabled(on: boolean): void {
  soundOn = on;
  store.set('poker.sound', on ? 'on' : 'off');
  if (on) { initAudio(); beep(880, 120); } // オンにした手応えを返す
}
...
let turnSoundFor: string | null = null;

setInterval(() => {
  if (!state || state.actingSeat === null) { lastWarnAt = -1; turnSoundFor = null; return; }
  const seat = state.seats[state.actingSeat];
  if (seat?.timeLeftMs === null || seat?.timeLeftMs === undefined) return;
  ...
  const bar = document.querySelector<HTMLElement>('.seat.acting .timer');
  if (bar) {
    bar.style.width = `${pct}%`;
    // 残り 1/3 で黄、1/6 で赤。色でも残量が分かるようにする
    bar.dataset.level = pct <= 16.7 ? 'crit' : pct <= 33.3 ? 'warn' : 'ok';
  }
  const label = document.querySelector<HTMLElement>('.seat.acting .timer-sec');
  if (label) label.textContent = `${Math.ceil(seat.timeLeftMs / 1000)}`;

  // 音は自分の手番のときだけ。他人の手番で鳴ると煩わしい
  const myTurn = state.yourSeat !== null && state.yourSeat === state.actingSeat;
  if (!myTurn) { lastWarnAt = -1; turnSoundFor = null; return; }

  const key = `${state.handNumber ?? 0}:${state.actingSeat}:${Math.round(total)}`;
  if (turnSoundFor !== key) { turnSoundFor = key; lastWarnAt = -1; soundYourTurn(); }

  const sec = Math.ceil(seat.timeLeftMs / 1000);
  if (sec === lastWarnAt) return;
  if (sec === 30) { lastWarnAt = sec; soundWarn30(); }
  else if (sec === 15) { lastWarnAt = sec; soundWarn15(); }
  else if (sec <= 5 && sec >= 1) { lastWarnAt = sec; soundTick(); }
}, 200);
...
function applyTheme(next: CardTheme): void {
  theme = next;
  document.documentElement.setAttribute('data-theme', next);
  store.set('poker.theme', next);
  const btn = document.getElementById('btn-theme');
  if (btn) btn.textContent = next === 'classic' ? '🎨 クラシック' : '🎨 ネオン';
}
applyTheme((store.get('poker.theme') as CardTheme) ?? 'classic');
...
function applySuits(mode: 'four' | 'two'): void {
...
applySuits((store.get('poker.suits') as 'four' | 'two') ?? 'four');
const suitsBtn = document.getElementById('btn-suits');
if (suitsBtn)
  suitsBtn.onclick = () =>
    applySuits(document.documentElement.getAttribute('data-suits') === 'four' ? 'two' : 'four');

// 効果音のオン/オフ。設定は localStorage に残す
function applySoundBtn(): void {
...
if (soundBtn) soundBtn.onclick = () => { setSoundEnabled(!isSoundEnabled()); applySoundBtn(); };
...
// トーナメント・ショップ・マイページ
//
// ここから下は P2 の追加分。設計方針は上と同じで、判断はすべてサーバー側にあり、
// クライアントは受け取ったビューを描くだけ。金額の計算も進捗の判定もここではしない。
// ---------------------------------------------------------------------------

import type { TournamentSummaryView, TournamentView, ShopView, ProfileView, SlotView, SlotResultView }
  from '../src/server/protocol.js';

let tournaments: TournamentSummaryView[] = [];
let tourView: TournamentView | null = null;
let shop: ShopView | null = null;
...
function switchTab(tab: string): void {
  activeTab = tab;
  each<HTMLElement>('.tab', (b) => b.classList.toggle('active', (b as HTMLElement).dataset.tab === tab));
  each<HTMLElement>('.tabpage', (p) => p.classList.add('hidden'));
...
/** 直前の結果（表示用に保持する） */
let slotLast: SlotResultView | null = null;

function renderSlot(): void {
  const el = document.getElementById('slot');
  if (!el) return;
  if (!slot) { el.innerHTML = '<p class="note">読み込み中…</p>'; return; }
  const s = slot;
  // client/ は旧世代の簡易版。実運用は手編集の poker-client.html なので、
  // ここでは新エンジンの結果から最初の盤面だけ拾って型を通す
  const oc = slotLast?.outcome as { base?: Array<{ grid: string[][] }> } | undefined;
  const reels: string[] = oc?.base?.[0]?.grid?.map((col) => col[0]) ?? ['chip', 'club', 'diamond'];
  const canSpin = !slotSpinning && s.gold >= slotBet && s.spinsLeft > 0;

  // 当たりの大きさで見せ方を変える（ジャックポットは別格に）
  let winCls = '', winText = '';
  if (slotLast && !slotSpinning) {
    if (slotLast.kind === 'max') { winCls = 'jackpot'; winText = `🎉 MAX WIN！ +${fmt(slotLast.won)}`; }
    else if (slotLast.kind === 'mega') { winCls = 'jackpot'; winText = `🎉 メガウィン！ +${fmt(slotLast.won)}`; }
    else if (slotLast.kind === 'big') { winCls = 'big'; winText = `ビッグウィン！ +${fmt(slotLast.won)}`; }
    else if (slotLast.kind === 'small') { winText = `+${fmt(slotLast.won)}`; }
    else { winText = 'ハズレ'; }
  }
  ...
  el.innerHTML = `<div class="slot-wrap">
    <div class="slot-machine">
      ...
      <div class="reels">
        ${reels.map((k: string) => `<div class="reel ${slotSpinning ? 'spinning' : ''}">${SLOT_ICON[k] ?? '❓'}</div>`).join('')}
      </div>
  ...
  if (spinBtn) spinBtn.onclick = () => {
    if (slotSpinning || !slot || slot.gold < slotBet || slot.spinsLeft <= 0) return;
    slotSpinning = true;
    slotLast = null;
    renderSlot();
...
function onSlotResult(r: SlotResultView): void {
  slotLast = r;
  if (slot) { slot.gold = r.goldLeft; slot.spinsLeft = r.spinsLeft; }
  // 即座に確定させると回転が一瞬で終わって手応えが無いので、少しだけ見せる
  setTimeout(() => {
    slotSpinning = false;
    renderSlot();
    if (r.kind === 'max' || r.kind === 'mega') { beep(880, 140); setTimeout(() => beep(1174, 160), 150); setTimeout(() => beep(1568, 260), 320); }
    else if (r.kind === 'big') { beep(784, 130); setTimeout(() => beep(1046, 190), 140); }
    else if (r.kind === 'small') beep(660, 110);
  }, 700);
}
...
function renderTournaments(): void {
  ...
  const BOUNTY_JA: Record<string, string> = {
    none: '',
    classic: 'ノックアウト',
    progressive: 'PKO',
    mystery: 'ミステリーバウンティ',
  };
  const SPEED_JA: Record<string, string> = { normal: '', turbo: 'ターボ', hyper: 'ハイパー' };
  ...
  each<HTMLButtonElement>('[data-tour]', (b) => {
    b.onclick = () => {
          ...
          ${
            b.recent.length
              ? `<div class="lb">${b.recent
                  ...
                  .map(
                    (r) =>
                      `<div><span>${escapeHtml(r.winner)} ➜ ${escapeHtml(r.victim)}</span><span>${fmt(r.cash)}${r.label ? `（${escapeHtml(r.label)}）` : ''}</span></div>`,
                  )
                  .join('')}</div>`
              : ''
          }
...
function renderShop(): void {
  ...
  html += `<div class="sec"><h3>チャレンジパス</h3><div class="packs">${
    shop.passPremium.owned
  ...
  each<HTMLButtonElement>('[data-buy]', (b) => {
 ...
 */
function showPurchaseDialog(sku: string): void {
  if (!shop) return;
  // ショップ内の全商品からこの SKU を探す
  type Item = { name: string; priceJpy: number; detail: string; tag?: string };
  let item: Item | null = null;
  const offer = shop.offers.find((o) => o.sku === sku);
  if (offer) item = { name: offer.name, priceJpy: offer.priceJpy, detail: offer.description, tag: offer.reason };
  const cp = shop.chipPacks.find((p) => p.sku === sku);
  if (cp) item = { name: cp.name, priceJpy: cp.priceJpy, detail: `チップ ${fmt(cp.chips)}` };
  const gp = shop.goldPacks.find((p) => p.sku === sku);
  if (gp) item = { name: gp.name, priceJpy: gp.priceJpy, detail: `ゴールド 🪙 ${fmt(gp.gold)}` };
  ...
  modal(
    ...
    (close) => {
      ...
      $('pay-ok').onclick = () => {
        const btn = $<HTMLButtonElement>('pay-ok');
        btn.disabled = true;
        btn.textContent = '処理中…';
...
function renderProfile(): void {
  ...
  const missionList = (list: ProfileView['missions']) => list
    .map(
      (m) => `<div class="mission">
        <div class="grow">
  ...
  each<HTMLButtonElement>('[data-mission]', (b) => {
  ...
  each<HTMLButtonElement>('[data-buy]', (b) => {
    b.onclick = () => {
      ...
      if (shop) showPurchaseDialog(sku);
      else { pendingBuySku = sku; send({ t: 'shop.list' }); }
    };
  });
}

/** ショップ情報の到着を待っている購入。届いたら確認ダイアログを開く */
let pendingBuySku: string | null = null;

function showReward(title: string, chips: number, gold: number, detail?: string): void {
  modal(
