...
const SUIT_SHAPES: Record<string, string> = {
...
export const SUIT_KEY: Record<string, string> = { '♠': 's', '♥': 'h', '♦': 'd', '♣': 'c' };
 ...
 */
export const GLYPH_WIDTH = { single: 0.68, double: 1.15 } as const;

/** 角の指数が占める矩形（カード幅を 1 とした座標） */
export function indexBox(rank: string): { x0: number; y0: number; x1: number; y1: number } {
...
export function pipBox(x: number, y: number): { x0: number; y0: number; x1: number; y1: number } {
  const m = CARD_METRICS;
 ...
 */
export const PIP_LAYOUT: Record<number, Array<[number, number]>> = {
  2: [[0.5, 0.14], [0.5, 0.86]],
 ...
 */
const COURT_MOTIF: Record<string, string> = {
  // 王冠
...
const suitSvg = (suit: string, cls: string) =>
  `<svg class="pip ${cls}" viewBox="0 0 100 100" aria-hidden="true">${SUIT_SHAPES[suit] ?? ''}</svg>`;

/**
 * 表向きのカード。code は "♠A" のような「スート記号 + ランク」。
 * サーバーから届く表記をそのまま受け取れるようにしてある。
 */
export function cardFace(code: string, opts: CardOptions = {}): string {
  const suit = code[0];
  ...
  if (rank === 'A') {
    ...
    center = `<div class="center pips">${pips
      .map(
        ([x, y]) =>
          `<span class="p" style="left:${x * 100}%;top:${y * 100}%${y > 0.5 ? ';transform:translate(-50%,-50%) rotate(180deg)' : ''}">${suitSvg(suit, 'sm')}</span>`,
      )
      .join('')}</div>`;
  } else {
    // 絵札
 ...
 */
export function cardBack(opts: CardOptions = {}): string {
  const size = opts.size ?? 'md';
...
/** 空きスロット（まだ配られていないボード） */
export function cardSlot(size: 'sm' | 'md' | 'lg' = 'md'): string {
...
/** 役名バナーを出す */
export function showHandBanner(container: HTMLElement, text: string): void {
...
/** オールインの閃光 */
export function flashAllIn(container: HTMLElement): void {
  const el = document.createElement('div');
 ...
 */
export function flyChips(container: HTMLElement, from: HTMLElement, count = 5): void {
...
export function confetti(container: HTMLElement, pieces = 60): void {
...
/** ポット額の増加を強調する */
export function bumpPot(potEl: HTMLElement): void {
