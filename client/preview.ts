...
function render(): void {
  const size = (document.getElementById('size') as HTMLSelectElement).value as 'sm' | 'md' | 'lg';

  const grid = SUITS.map(
    (s) =>
      `<div class="row"><div class="rowlabel">${s}</div>${RANKS.map((r) => cardFace(s + r, { size })).join('')}</div>`,
  ).join('');
...
document.getElementById('theme')!.addEventListener('click', () => {
  ...
  (document.getElementById('theme') as HTMLElement).textContent =
...
document.getElementById('suits')!.addEventListener('click', () => {
  ...
  (document.getElementById('suits') as HTMLElement).textContent = next === 'four' ? 'スート：4色' : 'スート：赤黒2色';
});
...
document.getElementById('fx')!.addEventListener('click', () => {
  ...
  flashAllIn(felt);
  confetti(felt, 80);
  const src = felt.querySelector<HTMLElement>('.hole');
  if (src) flyChips(felt, src, 8);
});
