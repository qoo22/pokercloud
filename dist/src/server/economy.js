...
export const DAILY_MISSIONS = [
    { id: 'play_hands', name: 'ハンドを 20 回プレイする', target: 20, rewardChips: 8_000, rewardXp: 20 },
    { id: 'win_hands', name: 'ハンドに 5 回勝つ', target: 5, rewardChips: 10_000, rewardXp: 30 },
    { id: 'showdown_win', name: 'ショーダウンで 2 回勝つ', target: 2, rewardChips: 12_000, rewardXp: 40 },
    { id: 'play_tournament', name: 'トーナメントに 1 回参加する', target: 1, rewardChips: 15_000, rewardXp: 50 },
];
...
export const PASS_TIERS = Array.from({ length: 30 }, (_, i) => {
    const tier = i + 1;
    return {
        tier,
        xpRequired: tier * 100,
        free: tier % 3 === 0 ? { chips: 20_000 * tier } : { chips: 5_000 * tier },
        premium: tier % 5 === 0 ? { chips: 60_000 * tier, gold: 20 } : { chips: 25_000 * tier },
    };
});
...
export class Economy {
    ...
    purchase(userId, sku, receipt) {
        ...
        if (sku === PASS_PREMIUM_SKU.sku)
            this.store.setProgress(userId, `pass:${PASS_SEASON_ID}:premium`, 1);
    ...
    claimDailyBonus(userId) {
        ...
        const base = Math.max(5_000, Math.round(u.chips * 0.03));
        const streakBonus = 1 + Math.min(streak, 7) * 0.15;
        const amount = Math.round(base * streakBonus * tier.dailyMultiplier);
    ...
    advanceMission(userId, missionId, by = 1) {
        ...
        const value = cur && cur.day === day ? cur.value : 0;
    ...
    missionStatus(userId) {
        ...
        return DAILY_MISSIONS.map((m) => {
            ...
            return {
                ...
                name: m.name,
                target: m.target,
                progress: p && p.day === day ? p.value : 0,
                rewardChips: m.rewardChips,
                rewardXp: m.rewardXp,
                claimed: !!claimed && claimed.day === day,
            };
        });
    }
    claimMission(userId, missionId) {
        ...
        if (!p || p.day !== day || p.value < def.target)
            return { ok: false, error: 'まだ達成していません' };
        const claimed = this.store.getProgress(userId, `mission_claimed:${missionId}`);
        if (claimed && claimed.day === day)
            return { ok: false, error: '受け取り済みです' };
        this.store.setProgress(userId, `mission_claimed:${missionId}`, 1, day);
        this.store.post(userId, 'chips', def.rewardChips, 'mission_reward', missionId);
        this.addPassXp(userId, def.rewardXp);
        return { ok: true, chips: def.rewardChips, xp: def.rewardXp };
    }
    // --- チャレンジパス ---------------------------------------------------------
    addPassXp(userId, xp) {
        const key = `pass:${PASS_SEASON_ID}:xp`;
        const cur = this.store.getProgress(userId, key)?.value ?? 0;
        this.store.setProgress(userId, key, cur + xp);
    }
    passStatus(userId) {
        const xp = this.store.getProgress(userId, `pass:${PASS_SEASON_ID}:xp`)?.value ?? 0;
        const premium = (this.store.getProgress(userId, `pass:${PASS_SEASON_ID}:premium`)?.value ?? 0) === 1;
        let tier = 0;
        ...
        return {
            ...
            tiers: PASS_TIERS.map((t) => ({
                ...
                claimedFree: (this.store.getProgress(userId, `pass:${PASS_SEASON_ID}:claim_free:${t.tier}`)?.value ?? 0) === 1,
                claimedPremium: (this.store.getProgress(userId, `pass:${PASS_SEASON_ID}:claim_prem:${t.tier}`)?.value ?? 0) === 1,
            })),
        };
    }
    /**
    ...
    claimPassRewards(userId) {
        ...
        for (const t of st.tiers) {
            ...
            if (!t.claimedFree) {
                ...
                gold += t.free.gold ?? 0;
                this.store.setProgress(userId, `pass:${PASS_SEASON_ID}:claim_free:${t.tier}`, 1);
                tiers.push(t.tier);
            }
            if (st.premium && !t.claimedPremium) {
                chips += t.premium.chips ?? 0;
                gold += t.premium.gold ?? 0;
                this.store.setProgress(userId, `pass:${PASS_SEASON_ID}:claim_prem:${t.tier}`, 1);
                if (!tiers.includes(t.tier))
                    tiers.push(t.tier);
            }
        }
        if (chips)
            this.store.post(userId, 'chips', chips, 'pass_reward', PASS_SEASON_ID);
        if (gold)
            this.store.post(userId, 'gold', gold, 'pass_reward', PASS_SEASON_ID);
    ...
    onHandPlayed(userId, opts) {
        ...
        this.addPassXp(userId, opts.won ? 6 : 3);
