...
/** 永続ディスクを付けたあとの状態 */
const SAFE = { ...VOLATILE, persisted: true, configured: true, summary: 'データは残っています(起動 2 回目)' };
async function withGateway(st, fn) {
...
describe('復元の安全装置', () => {
    ...
    test('健康診断は残っていれば ok:true', async () => {
        await withGateway(SAFE, async (base) => {
            const j = await (await fetch(`${base}/admin/health?key=${encodeURIComponent(SECRET)}`)).json();
...
//# sourceMappingURL=restore-guard.test.js.map