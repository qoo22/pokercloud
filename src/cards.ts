...
export function parseCard(s: string): Card {
  ...
  if (r < 0 || u < 0) throw new Error(`不正なカード表記: ${s}`);
 ...
 */
export function createSecureRng(poolBytes = 4096): Rng {
  if (typeof globalThis.crypto?.getRandomValues !== 'function') {
    throw new Error(
      'CSPRNG が利用できません。Node.js 19 以上、または Web Crypto 対応ブラウザで実行してください。',
    );
  }
  ...
  const next32 = (): number => {
    ...
    const v =
      ((pool[offset] << 24) >>> 0) + (pool[offset + 1] << 16) + (pool[offset + 2] << 8) + pool[offset + 3];
    offset += 4;
    return v;
  };
 ...
 */
export function createSeededRng(seed: number | number[]): Rng {
...
export class Deck {
  ...
  draw(): Card {
    if (this.index >= this.cards.length) throw new Error('デッキが尽きました');
