...
export type BacCard = { r: number; s: number };
...
export type BacBets = { p: number; b: number; tie: number };

/** バカラの数え方: 10・絵札は0、Aは1 */
export const bacValue = (r: number): number => (r >= 10 ? 0 : r);
const tot = (cs: BacCard[]) => cs.reduce((a, c) => a + bacValue(c.r), 0) % 10;

/**
 * バンカーが三枚目を引くか(標準規則)。
 * p3 はプレイヤー三枚目の**値**(引いていなければ null → バンカーは 0-5 で引く)
 */
export const bankerDraws = (bt: number, p3: number | null): boolean => {
 ...
 */
export function dealFromCards(next: () => BacCard): BacHand {
  const p = [next()];
  const b = [next()];
  p.push(next());
  b.push(next());
  const order: BacHand['order'] = [
    { s: 'p', i: 0 },
    { s: 'b', i: 0 },
    { s: 'p', i: 1 },
    { s: 'b', i: 1 },
...
export function dealBaccarat(rnd: () => number = Math.random): BacHand {
  const shoe: BacCard[] = [];
  for (let d = 0; d < 8; d++)
    for (let s = 0; s < 4; s++)
 ...
 */
export function baccaratReturn(bets: BacBets, res: BacHand['res']): number {
  if (res === 'P') return bets.p * 2;
  if (res === 'B') return Math.floor(bets.b * 1.95);
 ...
 */
export const BAC_DECLARE_RATE: Record<'H' | 'L', number> = { H: 0.25, L: 0.15 };

/** 宣言が的中したか。card は配り順で最後の1枚(=絞る札) */
export function declareHit(declare: 'H' | 'L' | null, card: BacCard): boolean {
