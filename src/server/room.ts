...
import { Hand, type SeatConfig, type ActionType, type HandEvent } from '../table.js';
import { FairnessSession } from '../fair.js';
import { cardToString, cardToDisplay, type Card } from '../cards.js';
...
import { showdownEquity, outsFor, currentLeaders, type ShowdownPlayer } from '../../solo/showdown.js';
...
export interface TableConfig {
  ...
  blindsProvider?: () => { smallBlind: number; bigBlind: number; ante: number };
  /**
...
export interface RoomHooks {
  ...
  onHandResult?(room: Room, summary: HandSummary, seats: Array<{ seat: number; userId: string }>): void;
}
...
interface Seat {
  ...
  timeBankMs: number;
  /** 次のハンドが終わったら卓を降りる */
  standPending: boolean;
  /** ストラドルを予約している（UTG に回ってきたら自動で置く） */
  straddleArmed: boolean;
  /** 装着中のブレスレット（コスメ。クライアント申告制） */
...
export class Room {
  ...
  private cosmetics = new Map<string, string | null>();
  private phase: Phase = 'waiting';
  private hand: Hand | null = null;
  private handId: string | null = null;
  private handNumber = 0;
  private buttonIndex = 0;
  private fairness: FairnessSession | null = null;
  /** 今のハンドで席 → クライアントシード提出済みか */
  private seedSubmitted = new Set<number>();
  private lastReveal: HandSummary['fairness'] | null = null;

  private actionDeadline: number | null = null;
  /** 現在の手番の基本持ち時間 */
  private actionBaseMs = ACTION_MS;
  /** 現在の手番に実際に与えられた総時間(クライアントの残り時間バーの分母) */
  private actionTotalMs = ACTION_MS;
  private actionStartedAt: number | null = null;
  private timers: unknown[] = [];
  private eventCursor = 0;

  private potHistory: number[] = [];
  private handTimestamps: number[] = [];
  /** 直近のハンドで飛んだ席と、その撃墜者 */
  private lastBusts = new Map<number, BustInfo>();
  /** 現在のブラインド。トーナメントではレベルごとに書き換わる */
  private blinds: { smallBlind: number; bigBlind: number; ante: number };
  ...
  constructor(
    ...
    this.cfg = {
      mode: 'cash',
      blindsProvider: undefined as unknown as () => { smallBlind: number; bigBlind: number; ante: number },
      minBuyInBB: 20,
      maxBuyInBB: 100,
      secretUnlockAt: 0,
      rakePercent: 0.04,
    ...
    if (this.cfg.maxSeats < 2 || this.cfg.maxSeats > 9) throw new Error('maxSeats は 2〜9 です');
  ...
  get currentBlinds(): { smallBlind: number; bigBlind: number; ante: number } {
    return { ...this.blinds };
  }
  ...
  setStyle(userId: string, name: string | undefined, bracelet: string | null | undefined): void {
    if (bracelet !== undefined) this.cosmetics.set(userId, bracelet);
    if (name) for (const m of this.members.values()) if (m.userId === userId) m.name = name;
    const seat = this.seatOfUser(userId);
    if (!seat) return;
    let changed = false;
    if (name && seat.name !== name) { seat.name = name; changed = true; }
    if (bracelet !== undefined && seat.bracelet !== bracelet) { seat.bracelet = bracelet; changed = true; }
  ...
  leave(sessionId: string): void {
    ...
    if (!this.seatOfUser(m.userId)) this.cosmetics.delete(m.userId);
  ...
  stand(sessionId: string): ErrorCode | null {
    ...
    if (this.isTournament) return 'ILLEGAL_ACTION';
    const m = this.members.get(sessionId);
    if (!m) return 'NOT_AUTHENTICATED';
    const seat = this.seatOfUser(m.userId);
  ...
  private cashOut(seat: Seat): void {
    if (seat.stack > 0) {
   ...
   */
  private noteSeats(): void {
    if (!this.bank.noteSeat || this.isTournament) return;
    for (const s of this.seats) if (s) this.bank.noteSeat(s.userId, this.cfg.tableId, s.stack);
  }

  /** 精算(bank.deposit)と着席記録の抹消をまとめて行う */
  private payOutSeat(s: Seat): void {
    if (s.stack > 0) this.bank.deposit(s.userId, s.stack, `${this.cfg.tableId}:cashout`);
  ...
  cashOutAll(): void {
    ...
    for (const s of this.seats) if (s) this.payOutSeat(s);
  }
  ...
  sweepExpiredSeats(): void {
    ...
    for (const s of this.seats) {
      if (!s || this.isInCurrentHand(s.seat)) continue;
      if (!this.isDisconnectExpired(s) && !s.standPending) continue;
      this.payOutSeat(s);
      swept = true;
    }
    if (swept) this.broadcastState();
  }
  ...
  sit(sessionId: string, seatIndex: number | undefined, buyIn: number): ErrorCode | null {
    ...
    const m = this.members.get(sessionId);
    if (!m) return 'NOT_AUTHENTICATED';
    if (this.seatOfUser(m.userId)) return 'ALREADY_SEATED';

    if (buyIn < this.minBuyIn || buyIn > this.maxBuyIn) return 'INVALID_BUYIN';

    let index = seatIndex;
    if (index === undefined) {
      index = this.seats.findIndex((s) => s === null);
      if (index < 0) return 'SEAT_TAKEN';
    }
    if (index < 0 || index >= this.cfg.maxSeats) return 'SEAT_TAKEN';
    if (this.seats[index] !== null) return 'SEAT_TAKEN';

    if (!this.bank.withdraw(m.userId, buyIn, `${this.cfg.tableId}:buyin`)) return 'INSUFFICIENT_FUNDS';
  ...
  rebuy(sessionId: string, amount: number): ErrorCode | null {
    ...
    const seat = this.seatOfUser(m.userId);
    if (!seat) return 'NOT_SEATED';
    if (this.isInCurrentHand(seat.seat)) return 'ILLEGAL_ACTION'; // ハンド中の増資は不可
    if (amount <= 0 || seat.stack + amount > this.maxBuyIn) return 'INVALID_BUYIN';
    if (!this.bank.withdraw(m.userId, amount, `${this.cfg.tableId}:rebuy`)) return 'INSUFFICIENT_FUNDS';
    seat.stack += amount;
  ...
  setSitOut(sessionId: string, sitOut: boolean): ErrorCode | null {
    ...
    if (!sitOut) seat.standPending = false;
    this.broadcastState();
  ...
  seatDirect(userId: string, name: string, stack: number, seatIndex?: number): number | null {
    let index = seatIndex;
    if (index === undefined) index = this.seats.findIndex((s) => s === null);
    if (index < 0 || index >= this.cfg.maxSeats || this.seats[index] !== null) return null;
  ...
  playersInSeats(): Array<{ userId: string; name: string; seat: number; stack: number }> {
    return this.seats
  ...
  setBlinds(b: { smallBlind: number; bigBlind: number; ante: number }): void {
    this.blinds = { ...b };
    this.broadcastState();
  }
  ...
  setStraddle(sessionId: string, enabled: boolean): ErrorCode | null {
    if (!this.straddleEnabled) return 'ILLEGAL_ACTION';
    const m = this.members.get(sessionId);
    if (!m) return 'NOT_AUTHENTICATED';
    const seat = this.seatOfUser(m.userId);
    if (!seat) return 'NOT_SEATED';
  ...
  submitSeed(sessionId: string, seed: string): ErrorCode | null {
    ...
    if (this.phase !== 'seed_window' || !this.fairness) return 'SEED_WINDOW_CLOSED';
    const seat = this.seatOfUser(m.userId);
    if (!seat) return 'NOT_SEATED';
    if (!this.fairness.submitClientSeed(seat.seat, seed)) return 'SEED_WINDOW_CLOSED';
  ...
  private eligibleSeats(): Seat[] {
    ...
    if (this.isTournament) {
      return this.seats.filter((s): s is Seat => s !== null && s.stack > 0);
    }
    return this.seats.filter(
      (s): s is Seat => s !== null && !s.sittingOut && s.stack > 0 && !this.isDisconnectExpired(s),
    );
  }

  private isDisconnectExpired(s: Seat): boolean {
    return s.disconnectedAt !== null && this.clock.now() - s.disconnectedAt > this.cfg.disconnectGraceMs;
  }

  /** 条件が揃っていれば次のハンドを開始する */
  maybeStartHand(): void {
    if (this.phase !== 'waiting' || this.paused) return;
    // トーナメントではブラインドを毎ハンド問い合わせる
    if (this.cfg.blindsProvider) this.blinds = this.cfg.blindsProvider();
    // 猶予を過ぎた切断者は Sit Out にする（トーナメントでは席を維持したまま自動フォールド）
    if (!this.isTournament) {
      for (const s of this.seats) {
        if (s && this.isDisconnectExpired(s) && !s.sittingOut) s.sittingOut = true;
      }
    }
   ...
   */
  private beginSeedWindow(): void {
    this.phase = 'seed_window';
    this.handNumber++;
    if (!this.isTournament && this.handNumber % 20 === 0) {
      for (const s of this.seats) if (s) s.timeBankMs = this.cfg.timeBankMs;
    }
  ...
  private startHand(): void {
    if (this.phase !== 'seed_window' || !this.fairness) return;
    ...
    const seatConfigs: SeatConfig[] = players.map((p) => ({
      id: p.userId,
      name: p.name,
      stack: p.stack,
    }));
    ...
    const straddles: Array<{ seat: number; amount: number }> = [];
    if (this.straddleEnabled && players.length >= 3) {
      ...
      for (let i = 1; i <= this.cfg.maxStraddles; i++) {
        ...
        if (!seat || !seat.straddleArmed || e === buttonInEngine) break;
        if (seat.stack <= amount) break;
    ...
    if (this.hand.isComplete) {
      if (this.shouldStageRunout(0)) this.startRunoutReveal(0);
      else this.finishHand();
    }
  }

  private engineToTable: number[] = [];
  // オールイン後の段階公開で、いま全クライアントに見せてよい場札の枚数。null なら全枚数。
  private boardRevealLimit: number | null = null;
  ...
  private revealStats: RevealStat[] | null = null;
  // 勝率ゲージ用キャッシュ（ストリートごとに 1 回だけモンテカルロを回す）
  private equityCacheKey: string | null = null;
  private equityCacheVal = 0;

  private nextButton(seatNumbers: number[]): number {
    const sorted = [...seatNumbers].sort((a, b) => a - b);
    for (const s of sorted) if (s > this.buttonIndex) return s;
    return sorted[0];
  }

  private tableSeatOf(engineSeat: number): number {
    return this.engineToTable[engineSeat] ?? engineSeat;
  }
  private engineSeatOf(tableSeat: number): number {
    return this.engineToTable.indexOf(tableSeat);
  }

  private isInCurrentHand(tableSeat: number): boolean {
    if (this.phase !== 'hand' || !this.hand) return false;
  ...
  act(sessionId: string, handId: string, action: ActionType, toAmount?: number): ErrorCode | null {
    const m = this.members.get(sessionId);
    if (!m) return 'NOT_AUTHENTICATED';
    if (this.phase !== 'hand' || !this.hand) return 'STALE_HAND';
    // 古いハンドへの遅れたクリックを弾く。これが無いと、次のハンドで意図しない
    // アクションが実行される（通信が遅い環境で頻発する）
    if (handId !== this.handId) return 'STALE_HAND';

    const seat = this.seatOfUser(m.userId);
    if (!seat) return 'NOT_SEATED';
    const engineSeat = this.engineSeatOf(seat.seat);
    if (engineSeat < 0 || this.hand.actingSeat !== engineSeat) return 'NOT_YOUR_TURN';
    ...
    if (this.actionStartedAt !== null) {
      ...
      if (over > 0) seat.timeBankMs = Math.max(0, seat.timeBankMs - over);
    }
  ...
  private afterAction(boardBefore = this.hand!.board.length): void {
    this.pushEvents();
    if (this.hand!.isComplete) {
      // オールインで残りの場札が一度に配られた決着なら、1段ずつ開く演出に回す
      if (this.shouldStageRunout(boardBefore)) this.startRunoutReveal(boardBefore);
      else this.finishHand();
    } else {
      this.armActionTimer();
      this.broadcastState();
  ...
  private shouldStageRunout(committedLen: number): boolean {
    const h = this.hand;
    if (!h || !h.isComplete || !h.result?.showdown) return false;
    if (h.board.length - committedLen <= 0) return false; // 新たに配られた札が無い
   ...
   */
  private startRunoutReveal(committedLen: number): void {
    const h = this.hand!;
    this.clearTimers();
    ...
    const jitter = (base: number): number => Math.round(base * big) + (this.clock.now() % 350);
    const preDelay = (n: number): number => Math.round((n <= 3 ? 1400 : n === 4 ? 1900 : 2700) * big); // flop / turn / river
  ...
  private computeRevealStats(): RevealStat[] {
    const h = this.hand;
    if (!h) return [];
    const board: Card[] =
      this.boardRevealLimit !== null ? h.board.slice(0, this.boardRevealLimit) : h.board;
    const live = h.players.map((p, e) => ({ p, e })).filter((x) => !x.p.folded);
    const players: ShowdownPlayer[] = live.map((x) => ({ seat: x.e, hole: x.p.holeCards }));
    // フォールドした席の既知ホールカードはマック（デッド）。ランアウトに出得ないので山札から除く。
    // 除かないと残りの勝率・アウツがわずかに歪む（実装指示書 §15/§19 が明示的に禁止する挙動）。
    const dead: Card[] = h.players
      .filter((p) => p.folded && p.holeCards.length === 2)
      .flatMap((p) => p.holeCards);
    // 残り札が少ないとき（フロップ以降）は数え上げで厳密。多いとき（プリフロップ全公開）だけ乱数を使う。
    const rng = { randomInt: (m: number) => Math.floor(Math.random() * m) };
  ...
  private armActionTimer(): void {
    this.clearTimers();
    const h = this.hand;
    if (!h || h.isComplete || h.actingSeat === null) return;
  ...
  private onActionTimeout(): void {
    ...
    if (seat) {
      ...
      if (!this.isTournament) seat.sittingOut = true;
    }
  ...
  private finishHand(): void {
    ...
    for (let e = 0; e < h.players.length; e++) {
      ...
      if (seat) seat.stack = h.players[e].stack;
    }
    ...
    for (let e = 0; e < h.players.length; e++) netChange[this.tableSeatOf(e)] = r.netChange[e];
    ...
    const summary: HandSummary = {
      ...
      hands: r.hands
        ...
        .filter((x): x is HandSummary['hands'][number] => x !== null),
    ...
    if (this.handTimestamps.length > 50) this.handTimestamps.shift();

    this.broadcast({ t: 'hand.result', tableId: this.cfg.tableId, summary });
    this.broadcastState();

    // 統計・ミッション・永続化はフックへ。Room 自身はそれらを知らない
    this.hooks.onHandResult?.(
      this,
      summary,
      h.players.map((_, e) => ({ seat: this.tableSeatOf(e), userId: h.players[e].id })),
    );
    ...
    for (const s of this.seats) {
      ...
      if (s.stack <= 0) {
        if (this.isTournament) {
          ...
          busted.push(
            this.lastBusts.get(s.seat) ?? { userId: s.userId, seat: s.seat, eliminatedBy: [], stackAtStart: 0 },
          );
          continue;
        }
      ...
      if (this.isTournament) continue; // 以下はキャッシュゲーム専用の後片付け
      ...
      // 猶予を過ぎた切断者は席を引き払う
      if (this.isDisconnectExpired(s)) this.payOutSeat(s);
    }
  ...
  private pushEvents(): void {
    ...
    if (!h) return;
    const all = h.events;
    if (this.eventCursor >= all.length) return;
    const slice = all.slice(this.eventCursor);
    this.eventCursor = all.length;

    // ホールカードの配布イベントは全体には流さない。演出用の匿名イベントに置き換える
    const publicEvents: HandEvent[] = slice.map((e) =>
      e.type === 'deal_hole' ? { ...e, cards: [] } : e,
    );
    this.broadcast({ t: 'table.events', tableId: this.cfg.tableId, handId: this.handId!, events: publicEvents });
  }

  private broadcast(msg: ServerMessage): void {
    for (const sessionId of this.members.keys()) this.io.send(sessionId, msg);
  }

  broadcastState(): void {
    for (const sessionId of this.members.keys()) this.sendStateTo(sessionId);
  }

  private sendBalanceTo(userId: string): void {
    for (const m of this.members.values()) {
      if (m.userId === userId) this.io.send(m.sessionId, { t: 'balance', balance: this.bank.balanceOf(userId) });
    }
  }
  ...
  private buildState(userId: string): TableStateView {
    ...
    const seats: PublicSeatView[] = this.seats.map((s, i) => {
      ...
      return {
        ...
        timeLeftMs:
          h && h.actingSeat === e && this.actionDeadline !== null
    ...
    if (h && myEngineSeat >= 0) {
      ...
      if (me && !me.folded && me.holeCards.length === 2 && !h.isComplete) {
        ...
        if (cards.length >= 5) yourHand = describeHand(evaluateBest(cards));
        else yourHand = me.holeCards[0] >> 2 === me.holeCards[1] >> 2 ? 'ワンペア' : 'ハイカード';
      }
    }

    // 自分の現在の勝率（未知の相手ハンドに対するモンテカルロ推定。表示用）
    let yourEquity: number | null = null;
  ...
  lobbyInfo(): LobbyTableInfo {
    ...
    if (this.handTimestamps.length >= 2) {
      ...
      if (span > 0) handsPerHour = Math.round(((this.handTimestamps.length - 1) / span) * 3600000);
    }
    return {
      tableId: this.cfg.tableId,
      name: this.cfg.name,
  ...
  private seatOfUser(userId: string): Seat | null {
    for (const s of this.seats) if (s && s.userId === userId) return s;
    return null;
  }
  ...
  chipsOnTable(): number {
    ...
    if (this.hand && !this.hand.isComplete) {
      ...
      for (const s of this.seats) if (s && !inHand.has(s.seat)) sum += s.stack;
      return sum;
    }
    for (const s of this.seats) if (s) sum += s.stack;
    return sum;
  }
