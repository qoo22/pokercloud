...
export type SlotSymKey =
  | 'chip' | 'club' | 'diamond' | 'heart' | 'spade' | 'crown' | 'seven'
  | 'wild' | 'scatter';
 ...
 */
export const PAY_SYMBOLS: SlotPaySymbol[] = [
...
export const SLOT_CFG = {
   ...
   */
  scatterWeightAnte: 17,   // 通常(13)より高くないと意味が無いので、旧比率(約1.3倍)を維持して追随
  /** アンティベットの賭け金倍率 */
  anteCost: 1.5,
  /**
   ...
   */
  freeSpinsByScatter: { 3: 8, 4: 12, 5: 16 } as Record<number, number>,
  /**
   * フリーゲーム中の上乗せ(第89弾・オーナー指定)。突入と同じ「3個以上」で成立するが、
   * **回数は突入より小さい**(固定WILDが載った状態で回せるぶん1回の価値が高いため)。
   * 再トリガー回数に上限は設けない。
   */
  freeRetriggerByScatter: { 3: 4, 4: 8, 5: 12 } as Record<number, number>,
  /**
...
export const WILD_MULT_TABLE: { m: number; w: number }[] = [
  { m: 2, w: 50 },
  { m: 3, w: 30 },
  { m: 4, w: 14 },
  { m: 5, w: 6 },
...
export const SCATTER_PAY: Record<number, number> = { 3: 2, 4: 5, 5: 20 };
...
export type Grid = SlotSymKey[][]; // [reel][row]
...
export interface TumbleStep {
   ...
   */
  wins: { key: SlotSymKey; count: number; ways: number; pay: number; line?: number; wildMult?: number; allWild?: boolean }[];
...
export interface SlotOutcome {
   ...
   */
  respin?: { grid0: Grid; steps: TumbleStep[]; payX: number; lockedReels: number[] };
  /** フリーゲームに入ったか */
  freeEntered: boolean;
  free?: {
    mode: FreeMode['key'];
...
function reelTable(reel: number, scatterWeight: number): { key: SlotSymKey; w: number }[] {
  const t: { key: SlotSymKey; w: number }[] = PAY_SYMBOLS.map((s) => ({ key: s.key, w: s.weight }));
  if (reel >= 1 && reel <= 3) t.push({ key: 'wild', w: SLOT_CFG.wildWeight });
  t.push({ key: 'scatter', w: scatterWeight });
  return t;
}

function drawFrom(table: { key: SlotSymKey; w: number }[], rnd: Rnd): SlotSymKey {
...
function newGrid(rnd: Rnd, scatterWeight: number): Grid {
  const g: Grid = [];
 ...
 */
function overlayStackedWilds(grid: Grid, rnd: Rnd, stripScale = 1): number[] {
  ...
  for (let r = 0; r < REELS; r++) {
    ...
    let n = 0;
    for (let y = 0; y < ROWS; y++) {
      if ((pos + y) % L <= 2) { grid[r][y] = 'wild'; n++; }
    }
    if (n >= ROWS) full.push(r);
  }
  return full;
}

/** WILD絡み当選の倍率を引く(フリーゲーム用) */
function drawWildMult(rnd: Rnd): number {
  const total = WILD_MULT_TABLE.reduce((a, b) => a + b.w, 0);
  let x = rnd() * total;
  for (const e of WILD_MULT_TABLE) { x -= e.w; if (x <= 0) return e.m; }
 ...
 *   - 複数ラインが成立したらすべて加算する
 */
function evaluate(grid: Grid): { wins: TumbleStep['wins']; hits: [number, number][] } {
  const wins: TumbleStep['wins'] = [];
  const hitSet = new Set<string>();

  for (let li = 0; li < PAYLINES.length; li++) {
    const line = PAYLINES[li];
    let best: { key: SlotSymKey; count: number; pay: number } | null = null;
    ...
    for (const sym of PAY_SYMBOLS) {
      ...
      for (let r = 0; r < REELS; r++) {
        ...
        if (c === sym.key || c === 'wild') len++;
        else break;
      }
      if (len < 3) continue;
      const pay = sym.pay[len - 3];
      if (pay <= 0) continue;
      if (!best || pay > best.pay) best = { key: sym.key, count: len, pay };
    }

    if (best) {
      // 当選区間に「その絵柄そのもの」が1つでもあるか。無ければ全部WILDで成立している
      let natural = false;
      for (let r = 0; r < best.count; r++) if (grid[r][line[r]] === best.key) natural = true;
      wins.push({
        key: best.key, count: best.count, ways: 1, pay: best.pay, line: li,
        ...(natural ? {} : { allWild: true }),
      });
      for (let r = 0; r < best.count; r++) hitSet.add(`${r},${line[r]}`);
    }
  }

  const hits: [number, number][] = [...hitSet].map((k) => {
    const [a, b] = k.split(',');
...
function countScatter(grid: Grid): number {
  let n = 0;
  for (let r = 0; r < REELS; r++) for (let y = 0; y < ROWS; y++) if (grid[r][y] === 'scatter') n++;
  return n;
}
...
function evaluateOnce(
  grid: Grid,
  opts: {
    /** 全体に掛かる倍率。フリーゲームの永続マルチプライヤー */
    mult?: number;
    /**
     * フリーゲームのWILD倍率。**突入時に1回だけ抽選してフリー中ずっと同じ値**を使う。
     * WILDを含む当選ラインに1回だけ掛ける(同じラインにWILDが2個以上あっても1回)。
     */
    wildMult?: number;
  } = {},
): { steps: TumbleStep[]; payX: number } {
  const { wins, hits } = evaluate(grid);
  if (!wins.length) return { steps: [], payX: 0 };

  if (opts.wildMult) {
    for (const w of wins) {
      const line = PAYLINES[w.line ?? 0];
      for (let r = 0; r < w.count; r++) {
        if (grid[r][line[r]] === 'wild') { w.wildMult = opts.wildMult; break; }
      }
    }
  }
 ...
 */
export function spin(rnd: Rnd, opts: SpinOptions = {}): SlotOutcome {
  ...
  if (out.freeEntered) {
    ...
    for (let guard = 0; left > 0 && guard < SLOT_CFG.freeSpinsGuard; guard++) {
      ...
      spins.push({
        ...
        heldCells: [...held].map((k) => k.split(',').map(Number) as [number, number]),
        freshCells: fresh,
      });
