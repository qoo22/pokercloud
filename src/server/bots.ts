...
function fromDisplay(s: string): Card {
...
interface Personality {
  tight: number;   // 高いほど降りやすい
  aggr: number;    // 高いほどベット/レイズ好き
  bluff: number;   // ブラフ頻度
  err: number;     // 「人間的なミス」の頻度
  thinkMul: number;
}
...
function preflopStrength(a: Card, b: Card): number {
  ...
  if (r1 === r2) return 0.52 + ((hi - 2) / 12) * 0.4; // 22=0.52 .. AA=0.92
  let s = 0.1 + (hi - 2) * 0.028 + (lo - 2) * 0.012;
  if (suited) s += 0.05;
  const gap = hi - lo;
  if (gap === 1) s += 0.05; else if (gap === 2) s += 0.02;
  if (hi === 14 && lo >= 10) s += 0.1;
...
class Bot {
  ...
  private readonly lifeHands = 6 + Math.floor(rnd() * 34);
  private leaving = false;
  /** 卓のステークスに応じた強さ (0=最弱テーブル, 1=最高額テーブル) */
  private tier = 0;
  readonly name = pick(NAMES) + (rnd() < 0.18 ? String(Math.floor(rnd() * 98) + 1) : '');
  ...
  private tourMisses = 0;

  constructor(
    private url: string,
    private onDead: () => void,
    private preferTableId: string | null = null,
    private mode: 'cash' | 'tour' = 'cash',
  ) {}

  start() {
    try { this.ws = new WebSocket(this.url); } catch { this.dead(); return; }
    const ws = this.ws;
    ws.onopen = () => {
    ...
    ws.onerror = () => { /* close が続けて来る */ };
    // 保険: キャッシュは3〜6分で退店。トーナメント要員は登録先がある限り待機し続ける
    // (常設SNG/MTTで「人間が来たら即開始」の頭数を維持するため)。未登録のまま20分で退店
    if (this.mode === 'tour') {
      this.after(20 * 60000, () => { if (!this.tourId) { try { this.ws?.close(); } catch { /* noop */ } } });
    } else this.after(180000 + rnd() * 180000, () => this.leaveSoon());
  }
  ...
  private handle(m: any) {
    if (this.mode === 'tour') {
      ...
      if (m.t === 'tour.tournaments' && !this.tourId) {
        const cands = (m.tournaments as any[]).filter((t) => {
          if (t.state !== 'registering') return false;
          if (t.buyIn + (t.fee ?? 0) > BOT_MAX_TOUR_BUYIN) return false;
          // 定期大会(tt-)は9人(=1卓ぶん)まで埋める
          if (String(t.tournamentId).startsWith('tt-')) return t.entrants < 9;
          // 常設SNGは「あと人間1人で開始」の状態まで埋めて待つ
          if (t.type === 'sng') return t.entrants < t.maxPlayers - 1;
          // 常設MTTは「最低人数以上・9人まで」埋める → 即開催され、人間はレイトレジでいつでも参加できる
          return t.entrants < Math.max(t.minPlayers ?? 6, 9);
        });
        if (!cands.length) {
          // 3回続けて空振りなら退店して枠を空ける
          if (++this.tourMisses >= 3) { this.after(1000, () => { try { this.ws?.close(); } catch { /* noop */ } }); return; }
      ...
      if (m.t === 'tournament.state' && m.view && m.view.tournamentId === this.tourId) {
        ...
        if (v.state === 'finished' || v.state === 'cancelled' || v.yourFinishPosition) {
          ...
          this.after(2000 + rnd() * 4000, () => { try { this.ws?.close(); } catch { /* noop */ } });
          return;
        }
    ...
    if (m.t === 'lobby.tables' && !this.tableId && this.mode === 'cash') {
      const cands = (m.tables as any[]).filter(
        (t) => t.seatedCount < t.maxSeats && t.minBuyIn <= BOT_MAX_BUYIN,
      );
      if (!cands.length) { this.leaveSoon(); return; }
    ...
    if (m.t === 'table.state' && m.state) this.onState(m.state);
    if (m.t === 'error') {
      // 手番のままエラーになった場合の安全弁。
      ...
      this.after(500, () => {
        ...
        this.pendingSt = null;
        if (!st || st.yourSeat === null || st.actingSeat !== st.yourSeat || !st.legalActions?.length || !st.handId) return;
        const check = st.legalActions.find((a: any) => a.type === 'check');
        this.act(st, check ? 'check' : 'fold');
      });
    }
  }

  private pendingSt: any = null;
  /** 最後に受け取った卓状態。エラー時・行動不発時の安全弁が「今も自分の手番か」を確認するのに使う */
  private latestSt: any = null;
  ...
  private statSeen = new Set<string>();
  private stat = { fold: 0, passive: 0, betInit: 0, raiseFacing: 0 };
  /** 相手のプリフロップの行動から推定したレンジ(V2 §6)と、ポストフロップで攻めたストリート */
  private preRange = new Map<number, string>();
  private aggBySeat = new Map<number, Set<string>>();
  /** 自分自身のレンジ(自分のプリフロップの行動から。リバーソルバーのunsafe re-solve用) */
  private myPreSpec: string | null = null;
  private myAggSet = new Set<string>();

  private onState(st: any) {
    if (st.tableId !== this.tableId) return;
    this.latestSt = st;
    // ハンド区切りの検知
    if (st.handId && st.handId !== this.lastHandId) {
      this.lastHandId = st.handId;
      this.handsPlayed++;
      this.aggro = false; this.wasAggro = false; this.streetKey = '';
      this.preRange.clear(); this.aggBySeat.clear();
      this.myPreSpec = null; this.myAggSet.clear();
    }
    const sk = `${st.handId}:${st.street}`;
    if (sk !== this.streetKey) { this.streetKey = sk; this.wasAggro = this.aggro; this.aggro = false; }
    // 相手傾向の観測(fold / 受動 / 自発ベット / 対抗レイズ) + 相手レンジの推定(V2 §6)
    if (st.handId && st.street !== 'waiting') {
      for (const x of st.seats) {
        if (!x.userId || x.seat === st.yourSeat || !x.lastAction) continue;
        const key = `${st.handId}:${st.street}:${x.seat}:${x.lastAction}`;
        if (this.statSeen.has(key)) continue;
        this.statSeen.add(key);
        const isAggr = x.lastAction === 'bet' || x.lastAction === 'raise' || x.lastAction === 'allin';
        if (x.lastAction === 'fold') this.stat.fold++;
        else if (x.lastAction === 'check' || x.lastAction === 'call') this.stat.passive++;
        else if (x.lastAction === 'bet') this.stat.betInit++;
        else if (x.lastAction === 'raise' || x.lastAction === 'allin') this.stat.raiseFacing++;
        if (st.street === 'preflop') {
          // プリフロップの行動 → 初期レンジ信念。3ベット/4ベットポットはより強いレンジへ「昇格」させる
          if (isAggr) {
            if (x.streetBet >= st.bigBlind * 16) this.preRange.set(x.seat, VILLAIN_PRE.fourBettor);
            else if (x.streetBet >= st.bigBlind * 7) this.preRange.set(x.seat, VILLAIN_PRE.threeBettor);
            else {
              const dealt = st.seats.filter((y: any) => y.userId && !y.sittingOut).map((y: any) => y.seat);
              const p = positionLabel(x.seat, st.buttonIndex, dealt, st.maxSeats);
              this.preRange.set(x.seat,
                p === 'UTG' || p === 'HJ' ? VILLAIN_PRE.raiserEarly :
                p === 'CO' ? VILLAIN_PRE.raiserMid : VILLAIN_PRE.raiserLate);
            }
          } else if (x.lastAction === 'call') {
            // 3ベット/4ベットへのコールはレンジが大きく締まる → 常に上書きで昇格
            if (st.currentBet >= st.bigBlind * 16) this.preRange.set(x.seat, VILLAIN_PRE.fourBetCaller);
            else if (st.currentBet >= st.bigBlind * 7) this.preRange.set(x.seat, VILLAIN_PRE.threeBetCaller);
            else if (!this.preRange.has(x.seat)) this.preRange.set(x.seat, VILLAIN_PRE.caller);
          ...
          if (!set) { set = new Set(); this.aggBySeat.set(x.seat, set); }
          set.add(st.street);
        }
      }
      if (this.statSeen.size > 4000) this.statSeen.clear(); // メモリ保険
    }
    // ひとりぼっち検知：45秒誰も来なければ席を立って別の卓へ(実際は退店→補充で人気卓に入る)
    if (st.yourSeat !== null && st.street === 'waiting') {
      const occupied = st.seats.filter((x: any) => x.userId).length;
      if (occupied <= 1) {
        if (!this.aloneSince) this.aloneSince = Date.now();
        else if (Date.now() - this.aloneSince > 45000) this.leaving = true;
      } else this.aloneSince = 0;
    } else this.aloneSince = 0;
    ...
    if (bigDecision) think += 1200 + rnd() * 2300;
    else if (st.street !== 'preflop' && rnd() < 0.05) think += 1200 + rnd() * 1800;
    // プリフロップの通常判断は上限2.2秒(思考倍率が高い性格でもテンポを守る)
    if (st.street === 'preflop' && !bigDecision) think = Math.min(think, 2200);
    // 全体上限6秒(以前は11秒。「botが長考しすぎる」報告を受けテンポ優先に)
    this.after(Math.min(think, 6000), () => {
      if (this.pendingSt !== st) return;
      this.pendingSt = null;
      this.decide(st);
      ...
      this.after(4000, () => {
        ...
        if (!cur || cur.yourSeat === null || cur.actingSeat !== cur.yourSeat || !cur.legalActions?.length || !cur.handId) return;
        if (`${cur.handId}:${cur.street}:${cur.currentBet}:${cur.pot}` !== key) return;
        const check = cur.legalActions.find((a: any) => a.type === 'check');
        this.act(cur, check ? 'check' : 'fold');
      });
    });
  }

  private decide(st: any) {
    const legal: any[] = st.legalActions;
    ...
    if (rnd() < this.p.err * 0.3 * (1 - 0.9 * this.tier)) {
      ...
      const opts: Array<[string, number | undefined]> = [];
      if (check) { opts.push(['check', undefined], ['check', undefined]); }
      if (cheapCall) opts.push(['call', undefined]);
      if (!check) opts.push(['fold', undefined]);
      if (raise && rnd() < 0.35) opts.push([raise.type, raise.min]);
      const [ma, mto] = opts.length ? pick(opts) : (['fold', undefined] as [string, number | undefined]);
      this.act(st, ma, mto);
      return;
    }
    ...
    if (bluffing) s = Math.max(s, 0.68 + rnd() * 0.2);
    ...
    if (s < threshold) { this.act(st, 'fold'); return; }
  ...
  private gtoDecide(st: any, hole: Card[], board: Card[]): { action: string; to?: number } | null {
    const call = st.legalActions.find((a: any) => a.type === 'call');
    const toCall = call?.amount ?? 0;
    const me = st.seats[st.yourSeat];
    const bb = st.bigBlind;
    const dealt = st.seats.filter((x: any) => x.userId && !x.sittingOut).map((x: any) => x.seat);
    const active = st.seats.filter((x: any) => x.userId && !x.sittingOut && !x.folded &&
    ...
    if (st.street === 'preflop') {
      ...
      for (const x of st.seats) {
        if (x.seat === st.yourSeat || !x.userId) continue;
        if (x.streetBet > openerBet) { openerBet = x.streetBet; openerSeat = x.seat; openerAllIn = !!x.allIn; }
      }
      const openerPos = openerSeat >= 0 && openerBet > bb
        ? positionLabel(openerSeat, st.buttonIndex, dealt, st.maxSeats) : null;
      const limpers = st.seats.filter((x: any) =>
        x.seat !== st.yourSeat && x.userId && !x.folded && x.streetBet === bb && x.lastAction === 'call').length;
      // ビッグスタックプレッシャー(§57): トナメで自分が突出したスタックならレンジを広げ圧をかける
      const maxOpp = Math.max(1, ...st.seats.filter((x: any) => x.userId && x.seat !== st.yourSeat && !x.sittingOut)
        .map((x: any) => x.stack + x.streetBet));
    ...
    const order = (s: number) => (s - (st.buttonIndex + 1) + st.maxSeats * 2) % st.maxSeats;
    const inPosition = active.every((x: any) => x.seat === st.yourSeat || order(x.seat) <= order(st.yourSeat));
    ...
    for (const x of active) {
      if (x.seat === st.yourSeat) continue;
      if (!villain || x.streetBet > villain.streetBet ||
        (x.streetBet === villain.streetBet &&
          (this.aggBySeat.get(x.seat)?.size ?? 0) > (this.aggBySeat.get(villain.seat)?.size ?? 0))) villain = x;
    }

    // 3人残りリバー用: アクション順(SB始まり)に並べた全員のレンジ/自分のindex/ベット者を作る
    let multiwaySpecs: string[] | undefined;
    let heroOrder: number | undefined;
    let bettorOrder: number | null | undefined;
    let multiwayTightens: number[] | undefined;
    if (st.street === 'river' && active.length === 3) {
      const ordered = [...active].sort((a: any, b: any) => order(a.seat) - order(b.seat));
      multiwaySpecs = ordered.map((x: any) =>
        x.seat === st.yourSeat ? (this.myPreSpec ?? VILLAIN_PRE.bbDefend)
          : (this.preRange.get(x.seat) ?? VILLAIN_PRE.bbDefend));
      multiwayTightens = ordered.map((x: any) =>
        x.seat === st.yourSeat ? this.myAggSet.size : (this.aggBySeat.get(x.seat)?.size ?? 0));
      heroOrder = ordered.findIndex((x: any) => x.seat === st.yourSeat);
      const bi = ordered.findIndex((x: any) => x.seat !== st.yourSeat && x.streetBet > 0);
  ...
  private applyGto(st: any, g: { action: string; to?: number }) {
    const legal: any[] = st.legalActions;
    const check = legal.find((a) => a.type === 'check');
    const call = legal.find((a) => a.type === 'call');
    const raise = legal.find((a) => a.type === 'raise' || a.type === 'bet');
    if (g.action === 'fold') { this.act(st, check ? 'check' : 'fold'); return; }
    if (g.action === 'check') { this.act(st, check ? 'check' : call ? 'call' : 'fold'); return; }
    if (g.action === 'call') { this.act(st, call ? 'call' : check ? 'check' : 'fold'); return; }
    if (!raise) { this.act(st, call ? 'call' : check ? 'check' : 'fold'); return; }
    if (g.action === 'allin') { this.act(st, raise.type, raise.max); return; }
    const to = Math.round(Math.min(raise.max, Math.max(raise.min, g.to ?? raise.min)));
    this.act(st, raise.type, to);
  }

  private act(st: any, action: string, toAmount?: number) {
    if (action === 'raise' || action === 'bet') {
      this.aggro = true;
      if (st.street !== 'preflop') this.myAggSet.add(st.street);
    }
...
export function startBots(url: string): void {
  ...
  const alive = new Set<Bot>();
  let target = Math.round((26 + Math.floor(rnd() * 7)) * SCALE);

  const spawn = (preferTableId: string | null = null) => {
    const bot = new Bot(url, () => {
      alive.delete(bot);
      // 退店したら数秒で別人格を補充(人数が足りない場合)
      if (alive.size < target) setTimeout(() => spawn(), 2500 + rnd() * 6000);
    }, preferTableId);
    alive.add(bot);
    bot.start();
  };
  ...
  const scoutCooldown = new Map<string, number>();
  const scout = () => {
    try {
      const ws = new WebSocket(url);
      ws.onopen = () => {
        ws.send(JSON.stringify({ t: 'hello', v: 1, name: '\u898B\u5B66\u4E2D', userId: 'bot_scout' + Math.random().toString(36).slice(2, 8) }));
        setInterval(() => { try { ws.send(JSON.stringify({ t: 'lobby.list' })); } catch { /* noop */ } }, 8000);
      };
      ws.onmessage = (ev) => {
        let m: any; try { m = JSON.parse(String(ev.data)); } catch { return; }
        if (m.t !== 'lobby.tables') return;
        const now = Date.now();
        for (const t of m.tables as any[]) {
          if (t.seatedCount < 1 || t.seatedCount >= t.maxSeats) continue;
          if (t.minBuyIn > BOT_MAX_BUYIN) continue;
          const hasBot = [...alive].some((b) => b.tableId2 === t.tableId);
          if (hasBot) continue;
          if ((scoutCooldown.get(t.tableId) ?? 0) > now) continue;
          if (alive.size >= Math.round(42 * SCALE)) break;
      ...
      ws.onerror = () => { /* close が来る */ };
    } catch { setTimeout(scout, 10000); }
  };
  setTimeout(scout, 3000);

  // ---- トーナメント要員: 定期大会(tt-)の最低人数をbotで担保する ----
  const tourAlive = new Set<Bot>();
  const spawnTour = () => {
  ...
  setTimeout(() => { for (let i = 0; i < Math.round(60 * SCALE); i++) setTimeout(spawnTour, i * 700 + rnd() * 600); }, 4000);
  setInterval(() => {
    const deficit = TOUR_POOL - tourAlive.size;
    for (let i = 0; i < Math.min(5, deficit); i++) setTimeout(spawnTour, i * 1200 + rnd() * 800);
  }, 8000);

  // 起動直後に時間差で入店
  for (let i = 0; i < target; i++) setTimeout(spawn, 800 + i * (900 + rnd() * 1500));

  // 人口をゆらす：ときどき目標人数を変え、不足はすばやく補充
  setInterval(() => {
    if (rnd() < 0.4) target = Math.max(Math.round(20 * SCALE), Math.min(Math.round(38 * SCALE), target + (rnd() < 0.5 ? -1 : 1)));
    const deficit = target - alive.size;
    for (let i = 0; i < Math.min(deficit, 2); i++) setTimeout(spawn, i * 3000 + rnd() * 4000);
    if (deficit < 0 && alive.size > 0) pick([...alive]).leaveSoon();
  }, 12000);
}
