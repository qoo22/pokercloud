...
import { bszDouble, bszDealDouble, bszResolveDouble, BSZ_DOUBLE_CAP_X, type BszDoubleDeal } from './bsz.js';
import { Room, type TableConfig, type RoomIO, type RoomBank, type Scheduler, realScheduler } from './room.js';
...
/**
 * 1回の入出金で扱える上限(第150弾)。台帳1行は 2^53≈9007兆までなので
 * SAFE_POST で分割して記帳する。回数が現実的な範囲に収まるよう 1000京 を上限にし、
 * 桁違いの値で回り続けないようループにもガードを置く
 */
 ...
 */
const isBot = (userId: string): boolean => userId.startsWith('bot_');
...
function randomTableCode(): string {
  let s = '';
...
export class Lobby {
  ...
  /** プライベート卓（ロビー一覧に出さない） */
  private privateIds = new Set<string>();
  /** 卓コード → tableId */
  private tableCodes = new Map<string, string>();
  /** 招待コードの使用履歴（メモリ上。再起動でリセットされる点は HANDOFF 参照） */
  private redeemedCodes = new Map<string, Set<string>>();
  private tournaments = new Map<string, Tournament>();
  /** 定期開催トーナメントの連番と、終了後の掃除予定時刻 */
  private tourSeq = 0;
  private tourPruneAt = new Map<string, number>();
   ...
   */
  private tunnelHold = new Map<string, { bet: number; payX: number }>();
  /**
   ...
   */
  private tunnelDeal = new Map<string, { deal: BszDoubleDeal; stakeX: number; keepX: number }>();
  private resumeTokens = new Map<string, { userId: string; name: string }>();
  private static readonly RESUME_CACHE_MAX = 3000;
  private cfg: Required<Omit<LobbyConfig, 'store' | 'tournaments'>> & {
    tournaments: TournamentConfig[];
  };
  ...
  constructor(
    ...
    this.cfg = { ...DEFAULTS, ...cfg, tournaments: cfg.tournaments ?? [] } as typeof this.cfg;
    this.store = cfg.store ?? new MemoryStore();
    this.economy = new Economy(this.store, () => this.clock.now());

    const io: RoomIO = { send: (sid, msg) => this.transport.send(sid, msg) };
    const bank: RoomBank = {
      // 秘密卓のバイインは 50京まであり、台帳1回の記帳(2^53≈9007兆)を超える。
      // バカラの大口ベットやGMコードと同じく SAFE_POST で分割記帳する(第150弾)
      withdraw: (userId, amount, ref) => {
        if (isBot(userId)) return true;   // ハウスのNPC。台帳は通さない
        if (!Number.isFinite(amount) || amount < 0 || amount > MONEY_MAX_POST) return false;
        if (this.store.balance(userId, 'chips') < amount) return false;
        for (let left = amount, guard = 0; left > 0; guard++) {
          if (guard > MAX_POST_CHUNKS) return false;   // 桁違いの値で回り続けない
          const c = Math.min(left, SAFE_POST);
          if (this.store.post(userId, 'chips', -c, 'table_buyin', ref) === null) return false;
          left -= c;
        }
        return true;
      },
      deposit: (userId, amount, ref) => {
        if (isBot(userId)) return;        // ハウスのNPC。台帳は通さない
        if (!Number.isFinite(amount) || amount <= 0) return;
        for (let left = Math.min(amount, MONEY_MAX_POST), guard = 0; left > 0; guard++) {
          if (guard > MAX_POST_CHUNKS) break;
          const c = Math.min(left, SAFE_POST);
    ...
    for (const t of this.cfg.tournaments) this.createTournament(t);
   ...
   */
  private recoverOpenSeats(): void {
    let rows: ReturnType<typeof this.store.listOpenSeats>;
    try {
   ...
   */
  private startSeatSweeper(): void {
    const tick = () => {
      for (const room of this.rooms.values()) {
        try { room.sweepExpiredSeats(); } catch { /* 1卓の失敗で全体を止めない */ }
      }
  ...
  cashOutAllTables(): void {
    for (const room of this.rooms.values()) {
      try { room.cashOutAll(); } catch { /* noop */ }
    }
  }
  ...
  private authKeyCache: string | null = null;
  ...
  private hashPin(code: string, pin: string): string {
   ...
   */
  issueTransferCode(userId: string): { code: string; pin: string } {
   ...
   */
  redeemTransferCode(codeRaw: string, pin: string): { ok: boolean; userId?: string; error?: string } {
    // 入力ゆれ(小文字・スペース・ハイフン無し)を吸収する
    const norm = codeRaw.toUpperCase().replace(/[^A-Z0-9]/g, '');
    if (norm.length !== 12) return { ok: false, error: 'コードの形式が違います' };
    const code = `${norm.slice(0, 4)}-${norm.slice(4, 8)}-${norm.slice(8, 12)}`;

    const row = this.store.getTransferCode(code);
    if (!row) return { ok: false, error: 'コードが見つかりません' };
    ...
    if (!this.store.getUser(row.userId)) return { ok: false, error: 'アカウントが見つかりません' };
  ...
  private makeResumeToken(userId: string): string {
    return `${userId}.${this.signUserId(userId)}`;
  }
  /** 署名付きトークンを検証して userId を返す。改ざん・形式不正なら null */
  private verifyResumeToken(token: string): string | null {
    const i = token.lastIndexOf('.');
    if (i <= 0) return null;
    const userId = token.slice(0, i);
    const sig = token.slice(i + 1);
    if (!/^[\w.-]{1,64}$/.test(userId) || sig.length !== 32) return null;
    return sig === this.signUserId(userId) ? userId : null;
  }

  private createTournament(cfg: TournamentConfig): Tournament {
    const t = new Tournament(
      cfg,
      this.io,
      this.bank,
      {
        collectEntry: (userId, amount, ref) =>
          this.store.post(userId, 'chips', -amount, 'tournament_buyin', ref) !== null,
  ...
  private startTourScheduler(): void {
    const NAMES = [
    ...
    const PRESETS: Array<{ label: string; reEntryMax: number; addOn: { price: number; chips: number } | null }> = [
    ...
    const tick = () => {
      const now = Date.now();
      // 1) 定刻を過ぎた大会を開始
      for (const t of this.tournaments.values()) t.tickSchedule();
      ...
      for (const [id, t] of this.tournaments) {
        ...
        if (t.state === 'finished' || t.state === 'cancelled') {
          ...
          if (at === undefined) this.tourPruneAt.set(id, now + 180_000);
          else if (now >= at) {
      ...
      for (const base of this.cfg.tournaments) {
        ...
        if (cur && (cur.state === 'finished' || cur.state === 'cancelled')) {
          ...
          if (at === undefined) this.tourPruneAt.set(base.tournamentId, now + 60_000);
          else if (now >= at) {
      ...
      const upcoming = [...this.tournaments.entries()].filter(
        ([id, t]) => id.startsWith('tt-') && t.state === 'registering',
      ).length;
  ...
  private onHandResult(
    room: Room,
    summary: Parameters<NonNullable<Room['hooks']['onHandResult']>>[1],
  ...
  private rememberResumeToken(token: string, userId: string, name: string): void {
   ...
   */
  private tunnelCashOut(userId: string): number {
  ...
  canSeeTable(userId: string, room: Room): boolean {
    ...
    if (this.store.hasReceipt(`unlock:${room.tableId}:${userId}`)) return true;
    if (this.store.balance(userId, 'chips') < need) return false;
  ...
  onDisconnect(sessionId: string): void {
    ...
    if (!s) return;
    for (const tableId of s.tables) this.getRoom(tableId)?.leave(sessionId);
  ...
  onMessage(sessionId: string, msg: ClientMessage): void {
    ...
    if (msg.t === 'hello') return this.handleHello(s, msg);
    if (msg.t === 'ping') return this.transport.send(sessionId, { t: 'pong', ts: msg.ts });
    if (!s.authenticated) return this.err(sessionId, 'NOT_AUTHENTICATED', '先に hello を送ってください');
    ...
    switch (msg.t) {
      ...
      case 'table.watch': {
        ...
        if (!room) return this.err(sessionId, 'NO_SUCH_TABLE', 'そのテーブルはありません');
        // 秘密卓は解禁前だと「そもそも無い」ことにする(存在を悟らせない)
        if (!this.canSeeTable(s.userId, room)) return this.err(sessionId, 'NO_SUCH_TABLE', 'そのテーブルはありません');
      ...
      case 'hand.act':
        return this.withRoom(sessionId, msg.tableId, (room) =>
          room.act(sessionId, msg.handId, msg.action, msg.toAmount),
        );
      case 'fair.seed':
      ...
      case 'tour.watch': {
        ...
        if (!t) return this.err(sessionId, 'NO_SUCH_TABLE', 'そのトーナメントはありません');
      ...
      case 'tour.register': {
        ...
        if (code) return this.err(sessionId, code, ERROR_MESSAGES[code]);
        this.sendBalance(s.userId);
        this.transport.send(sessionId, { t: 'tournament.state', view: t.view(s.userId) });
        return;
      }
      ...
      case 'table.create': {
        ...
        const timer: ReturnType<typeof setInterval> = setInterval(() => {
          ...
          if (!rm) { clearInterval(timer); return; }
      ...
      case 'code.redeem': {
        ...
        if (chips) {
          ...
          if (!used) { used = new Set(); this.redeemedCodes.set(s.userId, used); }
          if (used.has(code)) return this.err(sessionId, 'ILLEGAL_ACTION', 'このコードは使用済みです');
          used.add(code);
          this.store.savePurchase({
            userId: s.userId,
            sku: 'invite_code',
        ...
        if (tid) {
          ...
          if (!room) return this.err(sessionId, 'NO_SUCH_TABLE', 'その卓はすでに終了しています');
      ...
      case 'shop.purchase': {
        ...
        if (!r.ok) return this.err(sessionId, 'ILLEGAL_ACTION', r.error ?? '購入できませんでした');
        this.sendBalance(s.userId);
        this.transport.send(sessionId, {
          t: 'reward',
      ...
      case 'daily.claim': {
        ...
        if (!r.ok) return this.err(sessionId, 'ILLEGAL_ACTION', r.error ?? '受け取れませんでした');
        this.sendBalance(s.userId);
        this.transport.send(sessionId, {
          t: 'reward',
      ...
      case 'transfer.redeem': {
        ...
        if (!r.ok) return this.err(sessionId, 'ILLEGAL_ACTION', r.error ?? '引き継げませんでした');
        const userId = r.userId!;
        // 引き継ぎ先のアカウントへこのセッションを付け替える。
        // 卓に着いたままだと状態が食い違うので、先に全部降りてもらう
        for (const room of this.rooms.values()) {
          try { room.leave(sessionId); } catch { /* 座っていなければ何もしない */ }
        }
      ...
      case 'baccarat.deal': {
        ...
        if (!r.ok) return this.err(sessionId, 'ILLEGAL_ACTION', r.error ?? '配れませんでした');
        this.sendBalance(s.userId);
        this.transport.send(sessionId, {
          t: 'baccarat.result',
          result: {
      ...
      case 'tunnel.spin': {
        ...
        if (!r.ok) return this.err(sessionId, 'ILLEGAL_ACTION', r.error ?? '回せませんでした');
        const o = r.outcome!;
      ...
      case 'tunnel.double': {
        ...
        if (!hold) return this.err(sessionId, 'ILLEGAL_ACTION', 'ダブルできる獲得がありません');
        ...
        if (r.payX > BSZ_DOUBLE_CAP_X) this.tunnelCashOut(s.userId);
        return;
      }
      ...
      case 'tunnel.double.pick': {
        ...
        if (!hold || !pending) return this.err(sessionId, 'ILLEGAL_ACTION', '選べるダブルがありません');
        this.tunnelDeal.delete(s.userId);
        const r = bszResolveDouble(pending.deal, pending.stakeX, pending.keepX, msg.pick ?? 0);
        if (r.payX > 0) this.tunnelHold.set(s.userId, { bet: hold.bet, payX: r.payX });
        else this.tunnelHold.delete(s.userId);
        const canDouble = r.payX > 0 && r.payX <= BSZ_DOUBLE_CAP_X;
        this.transport.send(sessionId, {
          t: 'tunnel.double',
          result: { result: r, pending: Math.round(r.payX * hold.bet), canDouble },
      ...
      case 'user.style': {
        ...
        const name =
          typeof msg.name === 'string' && msg.name.trim().length > 0
            ...
            : undefined;
        const bracelet =
          msg.bracelet === null || (typeof msg.bracelet === 'string' && /^b[1-6]$/.test(msg.bracelet))
            ? msg.bracelet
            : undefined;
        if (name) {
          s.name = name;
          this.store.upsertUser(s.userId, name);
          if (s.resumeToken) this.rememberResumeToken(s.resumeToken, s.userId, name);
        }
        for (const room of this.rooms.values()) room.setStyle(s.userId, name, bracelet);
        for (const t of this.tournaments.values()) t.setStyle(s.userId, name, bracelet);
        this.sendProfile(sessionId, s.userId);
        return;
      }
    }
  ...
  private shopView(userId: string): ShopView {
  ...
  private profileView(userId: string): ProfileView {
    const u = this.store.getUser(userId);
    const pass = this.economy.passStatus(userId);
    const claimable = pass.tiers.some(
      (t) => t.unlocked && (!t.claimedFree || (pass.premium && !t.claimedPremium)),
    ) || pass.boxesEarned > pass.boxesClaimed;
    return {
      userId,
      name: u?.name ?? userId,
  ...
  private sendProfile(sessionId: string, userId: string): void {
    this.transport.send(sessionId, { t: 'profile', profile: this.profileView(userId) });
  }

  private sendBalance(userId: string): void {
  ...
  private sendToUser(userId: string, msg: ServerMessage): void {
    for (const s of this.sessions.values()) if (s.userId === userId) this.transport.send(s.sessionId, msg);
  }

  private withRoom(sessionId: string, tableId: string, fn: (room: Room) => ErrorCode | null): void {
    ...
    if (code) this.err(sessionId, code, ERROR_MESSAGES[code]);
  }

  private handleHello(s: Session, msg: Extract<ClientMessage, { t: 'hello' }>): void {
  ...
  private rateLimitOk(s: Session): boolean {
  ...
  private err(sessionId: string, code: ErrorCode, message: string): void {
  ...
  totalChips(): number {
    ...
    for (const r of this.rooms.values()) sum += r.chipsOnTable();
    return sum;
  }
  ...
  dispose(): void {
    ...
    for (const t of this.tournaments.values()) t.dispose();
  }
