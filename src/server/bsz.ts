...
export interface BszLineHit { line: number; key: string; x: number; cells: number[] }
export interface BszAnyHit { key: string; count: number; x: number }
...
export function bszEvaluate(g: BszGrid): { lines: BszLineHit[]; anys: BszAnyHit[]; payX: number } {
  ...
  BSZ_LINES.forEach((cells, i) => {
    ...
    if (w) lines.push({ line: i, key: w.key, x: w.x, cells });
  });
  ...
  for (const key of Object.keys(BSZ_ANY_PAY)) {
    ...
    if (n >= 3 && x) anys.push({ key, count: n, x });
  }
