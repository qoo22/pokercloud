...
export const BSZ_SYMS: BszSym[] = [
  'blank',
  'cherry', 'orange', 'plum', 'melon', 'bell', 'eight', 'bar', 'red7', 'blue7', 'joker',
];

/** ダブルダウンの強さ比べに使う序列(大きいほど強い) */
export const BSZ_RANK: Record<BszSym, number> = {
  blank: 0,
 ...
 */
export const BSZ_WEIGHTS: Record<BszSym, number> = {
  /**
...
/** トンネルの倍率と重み */
export const BSZ_TUNNEL: Array<{ x: number; w: number }> = [
...
export const BSZ_SPECIAL: Record<string, number> = {
  joker: 800, blue7: 30, red7: 20, any7: 10, bar: 5, eight: 4,
  bell: 3, melon: 2, plum: 2, orange: 1, cherry: 1,
};

export type BszGrid = BszSym[];   // 長さ9

const pickSym = (rnd: () => number): BszSym => {
...
export function bszSpinGrid(rnd: () => number, hold?: Partial<Record<number, BszSym>>): BszGrid {
  const g: BszGrid = [];
  for (let i = 0; i < 9; i++) g.push(hold?.[i] ?? pickSym(rnd));
  return g;
}

/** ライン配当の1本ぶん。ジョーカーは代用。揃わなければ null */
function lineWin(a: BszSym, b: BszSym, c: BszSym): { key: string; x: number } | null {
  const cells = [a, b, c];
  if (cells.includes('blank')) return null;   // ブランクが1つでもあればラインは切れる
  const real = cells.filter((s) => s !== 'joker');
  // 3つともジョーカー
  if (real.length === 0) return { key: 'joker', x: BSZ_LINE_PAY.joker };
  const first = real[0];
  if (real.every((s) => s === first)) return { key: first, x: BSZ_LINE_PAY[first] };
  // 赤7と青7の混在は ANY 7 として成立する(実機と同じ)
  if (real.every((s) => s === 'red7' || s === 'blue7')) return { key: 'any7', x: BSZ_LINE_PAY.any7 };
  return null;
}

export interface BszLineHit {
  line: number; key: string; x: number; cells: number[];
  /** 中央のワイルドを使って成立した役か。トンネル倍率はこれにだけ掛かる(第179弾) */
  wild?: boolean;
}
export interface BszAnyHit {
  key: string; count: number; x: number;
  /** この ANY に使ったジョーカーの枚数(第180弾)。0 なら素の個数だけで成立 */
  wilds?: number;
}

/** 盤面の判定。ライン配当と ANY 配当を別々に返す(実機と同じく両方とも足す) */
export function bszEvaluate(g: BszGrid): { lines: BszLineHit[]; anys: BszAnyHit[]; payX: number } {
  const lines: BszLineHit[] = [];
  BSZ_LINES.forEach((cells, i) => {
    ...
    if (w) lines.push({ line: i, key: w.key, x: w.x, cells, wild: g[4] === 'joker' && cells.includes(4) });
  });
  ...
  const rawCount = (key: string): number => (key === 'any7'
    ? g.filter((s) => s === 'red7' || s === 'blue7').length
    : g.filter((s) => s === key).length);
  const payOf = (key: string, n: number): number =>
    (n >= 3 ? (BSZ_ANY_PAY[key][Math.min(9, n)] ?? 0) : 0);
  ...
  const anys: BszAnyHit[] = [];
  for (const key of Object.keys(BSZ_ANY_PAY)) {
    const used = key === bestKey ? jokers : 0;
    const n = rawCount(key) + used;
    const x = payOf(key, n);
    if (x) anys.push({ key, count: n, x, wilds: used || undefined });
  }
...
export const bszPickTunnel = (rnd: () => number): number => {
  const total = BSZ_TUNNEL.reduce((a, t) => a + t.w, 0);
  let r = rnd() * total;
  for (const t of BSZ_TUNNEL) { r -= t.w; if (r < 0) return t.x; }
  return 1;
};

export interface BszStep { grid: BszGrid; lines: BszLineHit[]; anys: BszAnyHit[]; payX: number }

/** 中央リールの見せ方。null なら普通に止まるだけ */
export type BszReverse =
  | null
 ...
 */
export function bszSpin(rnd: () => number = Math.random): BszOutcome {
  const grid0 = bszSpinGrid(rnd);
  const ev = bszEvaluate(grid0);
  const base: BszStep = { grid: grid0, lines: ev.lines, anys: ev.anys, payX: ev.payX };
  ...
  if (freeEntered) {
    ...
    if (tunnel > 1) {
      const boost = (st: BszStep): number =>
        st.lines.reduce((a, l) => a + (l.wild ? l.x * (tunnel - 1) : 0), 0);
      sum += boost(base);
      for (const st of free) sum += boost(st);
    }
  }

  // 結果が決まったあとで、中央リールの見せ方だけを選ぶ
  const reverse: BszReverse = freeEntered
...
export function bszDealDouble(rnd: () => number = Math.random): BszDoubleDeal {
  const dealer = pickSym(rnd);
  const player: [BszSym, BszSym, BszSym] = [pickSym(rnd), pickSym(rnd), pickSym(rnd)];
  const tunnel = player.includes('joker') ? bszPickTunnel(rnd) : 0;
  return { dealer, player, tunnel };
}

/** 決めておいた4本と、プレイヤーが選んだ1本から勝敗を出す */
export function bszResolveDouble(
  deal: BszDoubleDeal,
  stakeX: number,
  keepX: number,
  pick = 0,
  ...
  const result: BszDoubleResult['result'] =
    BSZ_RANK[mine] > BSZ_RANK[dealer] ? 'win' : BSZ_RANK[mine] === BSZ_RANK[dealer] ? 'tie' : 'lose';

  // 3つ揃いのスペシャル(赤7と青7の混在は ANY 7 扱い)。
  // ブランク3つは配当表に無いので付かない(ここを見落とすと NaN になる)
  let specialKey: string | null = null;
  if (player[0] === player[1] && player[1] === player[2]) specialKey = player[0];
  else if (player.every((s) => s === 'red7' || s === 'blue7')) specialKey = 'any7';
