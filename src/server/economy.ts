...
import { bszSpin, type BszOutcome } from './bsz.js';
import {
  spin as spinReels, PAY_SYMBOLS, SLOT_CFG, FREE_MODES, MAX_WIN_X,
  TUMBLE_LADDER, SCATTER_PAY, REELS, ROWS, LINES, PAYLINES,
  type SlotOutcome, type FreeMode,
} from './slot.js';
import {
  dealBaccarat,
  baccaratReturn,
...
const OFFER_DEFS: Record<OfferKind, { name: string; priceJpy: number; multiplier: number; reason: string }> = {
...
function baseRateFor(priceJpy: number): number {
  let best = CHIP_PACKS[0];
  for (const p of CHIP_PACKS) if (p.priceJpy <= priceJpy) best = p;
...
export function tierOf(points: number): VipTier {
  let t = VIP_TIERS[0];
  for (const v of VIP_TIERS) if (points >= v.minPoints) t = v;
  return t;
}

export function nextTier(points: number): VipTier | null {
  for (const v of VIP_TIERS) if (points < v.minPoints) return v;
  return null;
}
 ...
 */
export function dailyBonusBase(chips: number): number {
...
/** 賭けられるゴールドの単位 */
export const SLOT_BETS = [1, 5, 10, 50] as const;
/**
 * 賭け1ゴールドあたりの基準チップ。
...
export function chipBetLadder(balance: number): number[] {
  const bal = Math.max(0, Math.floor(balance));
  if (bal < SLOT_CHIP_MIN_BET) return [];
  const out: number[] = [];
  // 1-2-5 × 10^k を小さい方から並べ、所持額を超えたら打ち切る
  for (let k = 0; k < 20 && out.length < 60; k++) {
    for (const m of [1, 2, 5]) {
      const v = m * Math.pow(10, k);
      if (v < SLOT_CHIP_MIN_BET) continue;
      // 台帳の1回上限も超えない(超える賭け金は控除の時点で例外になる)
      if (v > bal || v > SAFE_POST || v > SLOT_CHIP_MAX_BET) { k = 99; break; }
      out.push(v);
    }
  }
...
export function goldBetLadder(balance: number): number[] {
  const bal = Math.max(0, Math.floor(balance));
  const out: number[] = [];
 ...
 */
export function slotMultiplier(vipPoints: number, loginStreak: number): number {
...
export interface PassTier {
  tier: number;
  xpRequired: number;
  free: { chips?: number; gold?: number };
  premium: { chips?: number; gold?: number };
}

export const PASS_PREMIUM_SKU: Sku = {
  sku: 'pass_premium',
  name: 'プレミアムパス',
  priceJpy: 980,
  vipPoints: 980,
...
export function seasonWindowAt(now: number): {
  id: string; startsAt: number; endsAt: number; dayIndex: number; daysLeft: number; finalWeek: boolean;
} {
 ...
 */
export const PASS_TIERS: PassTier[] = Array.from({ length: PASS_TIER_COUNT }, (_, i) => {
  const tier = i + 1;
  ...
  const free: PassTier['free'] = freeHit ? { chips: 6_000 * tier } : {};
  if (tier === PASS_TIER_COUNT) free.chips = 250_000; // 無料でも完走の達成感は用意する

  // プレミアム: 毎段階チップ。5 の倍数でゴールド、節目でコスメ
  const premium: PassTier['premium'] = { chips: 20_000 * tier };
  if (tier % 5 === 0) premium.gold = tier === PASS_TIER_COUNT ? 40 : 20;
...
export class Economy {
  ...
  private buildOfferSku(sku: string): Sku | null {
    const kind = Object.keys(OFFER_DEFS).find((k) => `offer_${k}` === sku) as OfferKind | undefined;
    if (!kind) return null;
  ...
  purchase(userId: string, sku: string, receipt: string): PurchaseResult {
    ...
    if (!receipt || receipt.length < 8) return { ok: false, error: 'レシートが不正です' };
    if (this.store.hasReceipt(receipt)) return { ok: false, error: 'このレシートは既に使用されています' };

    // 貯金箱は特別扱い：溜まっている分を払い出して 0 に戻す
    if (sku === 'offer_piggy_bank') return this.claimPiggyBank(userId, receipt);
    ...
    if (chips) this.store.post(userId, 'chips', chips, 'purchase', sku);
    if (gold) this.store.post(userId, 'gold', gold, 'purchase', sku);
    if (sku === 'noads_forever') this.enableAdRemoval(userId);

    if (sku === PASS_PREMIUM_SKU.sku) this.store.setProgress(userId, `pass:${this.seasonId()}:premium`, 1);
  ...
  offersFor(userId: string): Offer[] {
    ...
    const push = (kind: OfferKind, expiresAt: number | null, overrideChips?: number) => {
      const sku = this.buildOfferSku(`offer_${kind}`)!;
      if (overrideChips !== undefined) sku.chips = overrideChips;
      out.push({
        id: kind,
    ...
    // 初回限定：一度も購入していない人だけ
    if (this.store.purchases(userId, 1).length === 0) push('first_time', null);

    // 破産レスキュー：最低卓のバイインにも届かない人へ、24 時間に 1 回
    const lastRescue = this.store.getProgress(userId, 'offer:bust_rescue');
    if (chips < 5_000 && (!lastRescue || now - lastRescue.updatedAt > 86_400_000)) push('bust_rescue', null);

    // 貯金箱：溜まっていれば常時
    if (user.piggyBank > 0) push('piggy_bank', null, user.piggyBank);
    ...
    // VIP 限定：Gold ティア以上
    if (tierOf(user.vipPoints).minPoints >= 10_000) push('vip_only', null);
  ...
  private claimPiggyBank(userId: string, receipt: string): PurchaseResult {
    const u = this.store.getUser(userId);
    if (!u || u.piggyBank <= 0) return { ok: false, error: '貯金箱が空です' };
   ...
   */
  claimDailyBonus(userId: string): { ok: boolean; amount?: number; streak?: number; error?: string } {
    const u = this.store.getUser(userId);
    if (!u) return { ok: false, error: 'ユーザーが見つかりません' };
    const day = today(this.now());
    if (u.lastDailyBonus === day) return { ok: false, error: '本日分は受け取り済みです' };
   ...
   */
  private carriedDay(cur: { day: string | null; value: number } | null): string | null {
  ...
  advanceMission(userId: string, missionId: string, by = 1): void {
  ...
  advanceWeekly(userId: string, missionId: string, by = 1): void {
  ...
  advanceSeasonal(userId: string, missionId: string, by = 1): void {
  ...
  claimMission(userId: string, missionId: string): { ok: boolean; chips?: number; xp?: number; error?: string } {
    const daily = DAILY_MISSIONS.find((m) => m.id === missionId);
    if (daily) {
      const p = this.store.getProgress(userId, `mission:${missionId}`);
      if (!p || !this.carriedDay(p) || p.value < daily.target) return { ok: false, error: 'まだ達成していません' };
      const claimed = this.store.getProgress(userId, `mission_claimed:${missionId}`);
      if (claimed && claimed.day === p.day) return { ok: false, error: '受け取り済みです' };
    ...
    if (weekly) {
      ...
      if (!p || p.day !== wk || p.value < weekly.target) return { ok: false, error: 'まだ達成していません' };
      const claimed = this.store.getProgress(userId, `weekly_claimed:${missionId}`);
      if (claimed && claimed.day === wk) return { ok: false, error: '受け取り済みです' };
    ...
    if (seasonal) {
      ...
      if (!p || p.day !== sid || p.value < seasonal.target) return { ok: false, error: 'まだ達成していません' };
      const claimed = this.store.getProgress(userId, `seasonal_claimed:${missionId}`);
      if (claimed && claimed.day === sid) return { ok: false, error: '受け取り済みです' };
  ...
  spinSlot(
    ...
    opts: { ante?: boolean; mode?: FreeMode['key'] } = {},
  ): SlotSpinResult {
    const u = this.store.getUser(userId);
    if (!u) return { ok: false, error: 'ユーザーが見つかりません' };
    const day = today(this.now());
    ...
    if (u.chips < cost) return { ok: false, error: 'チップが足りません' };
    ...
    const kind: SlotSpinResult['kind'] =
      outcome.maxWin ? 'max' : x >= 100 ? 'mega' : x >= 20 ? 'big' : x > 0 ? 'small' : 'none';
   ...
   */
  spinTunnel(userId: string, bet: number, rnd: () => number = Math.random): TunnelSpinResult {
    const u = this.store.getUser(userId);
    if (!u) return { ok: false, error: 'ユーザーが見つかりません' };
    const day = today(this.now());
    ...
    if (u.chips < bet) return { ok: false, error: 'チップが足りません' };
  ...
  dealBaccaratHand(
    ...
    if (u.chips < stake) return { ok: false, error: 'チップが足りません' };

    {
      ...
      while (rest > 0) {
        ...
        if (this.store.post(userId, 'chips', -part, 'baccarat_bet', `day:${day}`) === null) {
          ...
          if (taken > 0) this.store.post(userId, 'chips', taken, 'baccarat_bet', `refund:${day}`);
          return { ok: false, error: 'チップが足りません' };
        }
        taken += part;
        rest -= part;
    ...
    const bonus = declareHit(declare, lastCard)
      ? Math.round(stake * BAC_DECLARE_RATE[declare as 'H' | 'L'])
      : 0;
    const won = ret + bonus;
   ...
   */
  private adReward(chips: number): number {
  ...
  grantAdReward(userId: string): { ok: boolean; error?: string; reward?: number; left?: number } {
    const u = this.store.getUser(userId);
    if (!u) return { ok: false, error: 'ユーザーが見つかりません' };
    const day = today(this.now());
    const w = this.store.getProgress(userId, 'ad:watched');
    const used = w && w.day === day ? w.value : 0;
    if (used >= AD_DAILY_LIMIT) return { ok: false, error: '本日の視聴上限に達しました' };
  ...
  passStatus(userId: string) {
    ...
    let tier = 0;
    for (const t of PASS_TIERS) if (xp >= t.xpRequired) tier = t.tier;
    ...
    return {
      ...
      tiers: PASS_TIERS.map((t) => ({
        ...
        claimedPremium:
          (this.store.getProgress(userId, `pass:${this.seasonId()}:claim_prem:${t.tier}`)?.value ?? 0) === 1,
      })),
    };
  }
   ...
   */
  passPurchasePreview(userId: string): { chips: number; gold: number; tiers: number } {
    const st = this.passStatus(userId);
    if (st.premium) return { chips: 0, gold: 0, tiers: 0 };
    let chips = 0, gold = 0, tiers = 0;
    for (const t of st.tiers) {
      if (!t.unlocked || t.claimedPremium) continue;
   ...
   * （後半で買う障壁を下げるための設計）。
   */
  claimPassRewards(userId: string): { chips: number; gold: number; tiers: number[]; boxes: number } {
    const st = this.passStatus(userId);
    let chips = 0;
    let gold = 0;
    const tiers: number[] = [];
    ...
    for (const t of st.tiers) {
      ...
      if (st.premium && !t.claimedPremium) {
        ...
        if (!tiers.includes(t.tier)) tiers.push(t.tier);
      }
    }
    ...
    if (chips) this.store.post(userId, 'chips', chips, 'pass_reward', this.seasonId());
    if (gold) this.store.post(userId, 'gold', gold, 'pass_reward', this.seasonId());
   ...
   */
  onHandPlayed(userId: string, opts: { won: boolean; showdownWin: boolean; rakeContributed: number }): void {
    this.advanceMission(userId, 'play_hands');
    this.advanceWeekly(userId, 'w_hands');
    this.advanceSeasonal(userId, 's_hands');
    if (opts.won) this.advanceMission(userId, 'win_hands');
    if (opts.showdownWin) {
    ...
    if (opts.rakeContributed > 0) this.addToPiggyBank(userId, Math.round(opts.rakeContributed * 0.5));
