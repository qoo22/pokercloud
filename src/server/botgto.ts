...
export const TUNING_BOUNDS: Record<keyof Tuning, [number, number]> = {
...
export function setTuning(partial: Partial<Tuning>): void {
  for (const k of Object.keys(partial) as Array<keyof Tuning>) {
    const v = partial[k];
 ...
 * 無ければ既定値のまま(エラーにしない)。
 */
export async function loadTunedParams(): Promise<string | null> {
  try {
...
export interface BlueprintTable {
  version: number;
  sizes: { betS: number; betB: number };   // 小/大ベットのポット比(サンプル生成に使った値)
  nodes: Record<string, Record<string, BpNode>>; // key -> handBucket -> node
}
 ...
 */
export function flopPotType(heroSpec?: string | null, villSpec?: string | null): string {
 ...
 */
export function flopSprBand(spr: number): string {
  return spr <= 3 ? 'low' : spr <= 8 ? 'mid' : 'high';
}

/** 表のキーを組み立てる(オフライン生成と実行時参照で必ず同じ関数を使う) */
export function blueprintKey(flopCls: string, role: string, potType: string, sprBand: string): string {
  return `${flopCls}|${role}|${potType}|${sprBand}`;
}

let BLUEPRINT: BlueprintTable | null = null;
export function setBlueprint(bp: BlueprintTable | null): void { BLUEPRINT = bp; }
export function getBlueprint(): BlueprintTable | null { return BLUEPRINT; }

/**
 * フロップのテクスチャを少数の戦略クラスへ写す(スート同型を吸収)。
 * ペア/モノトーン/ハイカード階層 × ウェット(2トーン or コネクト)で ~11 クラス。
 */
export function flopClass(board: Card[]): string {
  if (board.length !== 3) return 'other';
  const t = classifyTexture(board);
  const ranks = board.map((c) => (c >> 2) + 2).sort((a, b) => b - a);
  const hi = ranks[0];
  if (t.paired) return hi >= 10 ? 'paired-hi' : 'paired-lo';
  if (t.monotone) return 'mono';
...
export function flopRole(inPosition: boolean, wasAggressor: boolean): string {
 ...
 */
export function flopHandBucket(hole: Card[], board: Card[]): string {
  const s = madeScore(hole, board);
  if (s >= 0.9) return 'monster';    // ストレート+/セット/フラッシュ
  if (s >= 0.78) return 'strong';    // ツーペア/弱いセット
  if (s >= 0.62) return 'toppair';   // オーバーペア/トップペア good
  if (s >= 0.5) return 'midpair';    // 弱いトップペア/上位ポケット
  const d = drawFeatures(hole, board);
  // 大きいドローはペアより先に見る(ペア+FDはブラフキャッチではなく攻めの手)
  if (d.outs >= 8) return 'bigdraw';
  // セカンドペア/ボトムペア/下位ポケット。ショーダウンバリューがありエアとは別物
  // (ブラフキャッチャーとして振る舞うので、エアに混ぜると降り過ぎ・打ち過ぎになる)
  if (s >= 0.34) return 'weakpair';
  if (d.outs >= 4) return 'weakdraw';
  return 'air';
}
 ...
 */
export function blueprintAction(c: PostCtx): GtoAction | null {
  const bp = BLUEPRINT;
  if (!bp || c.street !== 'flop' || c.board.length !== 3) return null;
  // ストリート開始時点のポット(＝相手のベットを除いたポット)でSPRとベット比を測る
  const potStart0 = Math.max(1, c.pot - c.toCall);
  const key = blueprintKey(
    flopClass(c.board),
    flopRole(c.inPosition, c.wasAggressor),
    flopPotType(c.heroSpec, c.villainSpec),
    flopSprBand(c.myStack / potStart0),
  );
  ...
  if (c.toCall <= 0) {
    ...
    if (!dist || Object.keys(dist).length === 0) dist = node.faceB ?? node.faceS;
    if (!dist || Object.keys(dist).length === 0) dist = node.faceS;
  }
  if (!dist) return null;
  const entries = Object.entries(dist).filter(([, v]) => (v ?? 0) > 0);
  let total = 0; for (const [, v] of entries) total += v ?? 0;
  if (total <= 0) return null;
  let x = c.rnd() * total;
  let label = entries[entries.length - 1][0];
  for (const [k, v] of entries) { x -= v ?? 0; if (x <= 0) { label = k; break; } }
 ...
 */
export async function loadBlueprint(): Promise<string | null> {
  try {
    const { readFileSync } = await import('node:fs');
    const url = new URL('../../../flop_blueprint.json', import.meta.url);
    const data = JSON.parse(readFileSync(url, 'utf8')) as BlueprintTable;
...
const rv = (c: string) => RANKS.indexOf(c) + 2; // '2'→2 .. 'A'→14
const rc = (v: number) => RANKS[v - 2];

/** "66+, A3s+, KTo+, A2s-A5s, JJ:0.5, AK" → Map<ハンドタイプ, 頻度0..1> */
function parseRange(spec: string): Map<string, number> {
  ...
  for (let tok of spec.split(',')) {
    ...
    if (ci >= 0) { w = Number(tok.slice(ci + 1)) || 1; tok = tok.slice(0, ci); }
    ...
    if (hi === lo) {
      ...
      for (let r = Math.min(hi, top); r <= Math.max(hi, top); r++) put(rc(r) + rc(r), w);
    } else {
      const H = Math.max(hi, lo), L = Math.min(hi, lo);
      const suits = so ? [so] : ['s', 'o'];
      let kickers: number[] = [L];
      if (plus) { kickers = []; for (let k = L; k < H; k++) kickers.push(k); }
      else if (h2 && l2) {
        const L2 = Math.min(rv(h2), rv(l2));
        kickers = []; for (let k = Math.min(L, L2); k <= Math.max(L, L2); k++) kickers.push(k);
      }
      for (const k of kickers) for (const s of suits) put(rc(H) + rc(k) + s, w);
    }
  }
  return out;
}
...
function R(spec: string): Map<string, number> {
  let r = rangeCache.get(spec);
  if (!r) { r = parseRange(spec); rangeCache.set(spec, r); }
  return r;
}
/** ハンドがレンジに入っている頻度 (0..1) */
function freq(spec: string, type: string): number { return R(spec).get(type) ?? 0; }

/** 2枚のカード → "AKs"/"AKo"/"TT" */
export function handTypeOf(a: Card, b: Card): string {
  const r1 = (a >> 2) + 2, r2 = (b >> 2) + 2;
  const hi = Math.max(r1, r2), lo = Math.min(r1, r2);
  if (r1 === r2) return rc(hi) + rc(hi);
  return rc(hi) + rc(lo) + (((a & 3) === (b & 3)) ? 's' : 'o');
}

// ------------------------------------------------ レンジ対エクイティ (V2仕様 §6/§8)
const mkCard = (rank: number, suit: number): Card => (((rank - 2) << 2) | suit) as Card;

/** レンジ記法 → 重みつき1326コンボ(累積重みつき、specごとにキャッシュ) */
interface ComboTable { combos: Array<[Card, Card, number]>; cum: number[]; total: number }
const comboCache = new Map<string, ComboTable>();
function combosOf(spec: string): ComboTable {
  let t = comboCache.get(spec);
  if (t) return t;
  const combos: Array<[Card, Card, number]> = [];
  for (const [type, w] of R(spec)) {
    ...
    if (hi === lo) {
      ...
      for (let s1 = 0; s1 < 4; s1++) combos.push([mkCard(hi, s1), mkCard(lo, s1), w]);
    } else {
      for (let s1 = 0; s1 < 4; s1++) for (let s2 = 0; s2 < 4; s2++)
        if (s1 !== s2) combos.push([mkCard(hi, s1), mkCard(lo, s2), w]);
    }
  }
...
export function equityVsRange(
  hole: Card[], board: Card[], spec: string,
  opts?: { iters?: number; tighten?: number; rnd?: () => number },
): number | null {
  const table = combosOf(spec);
  if (!table.combos.length || hole.length < 2) return null;
  ...
  const deck: Card[] = [];
  for (let rank = 2; rank <= 14; rank++) for (let su = 0; su < 4; su++) {
    const c = mkCard(rank, su);
    if (!blocked.has(c)) deck.push(c);
  }
  const need = 5 - board.length;
  const pickCombo = (): [Card, Card] | null => {
    for (let tries = 0; tries < 30; tries++) {
      const x = r() * table.total;
      // 二分探索で重みサンプリング
      let lo = 0, hi2 = table.cum.length - 1;
      while (lo < hi2) { const mid = (lo + hi2) >> 1; if (table.cum[mid] < x) lo = mid + 1; else hi2 = mid; }
      const cb = table.combos[lo];
      if (!blocked.has(cb[0]) && !blocked.has(cb[1])) return [cb[0], cb[1]];
    }
    return null;
  };
  let sum = 0, n = 0;
  for (let i = 0; i < iters; i++) {
    ...
    for (let k = 0; k < tighten; k++) {
      ...
      if (alt && board.length >= 3 &&
        scoreBest([...alt, ...board]) > scoreBest([...vill, ...board])) vill = alt;
    }
    ...
    if (need > 0) {
      ...
      for (let k = 0; k < need; k++) {
        ...
        const tmp = avail[k]; avail[k] = avail[j]; avail[j] = tmp;
        run.push(avail[k]);
      }
    }
...
export const VILLAIN_PRE: Record<string, string> = {
  raiserEarly: RFI.UTG,
  raiserMid: RFI.CO,
  raiserLate: RFI.BTN,
...
const PUSH: Record<number, Partial<Record<Pos, string>>> = {
  15: {
...
const CALL_SHOVE: Record<number, string> = {
...
const nearestBand = (bands: number[], v: number) =>
  bands.reduce((a, b) => (Math.abs(b - v) < Math.abs(a - v) ? b : a));

// ------------------------------------------------ リバー・リアルタイムソルバー
// (V2 §26-30/Phase5 の軽量版: レンジを強さバケットに落とし、小さなベットツリーを
//  CFR+で解く unsafe re-solving。リバーは将来ストリートがないため厳密に近く解ける)

interface BucketedRange {
  boundaries: number[];          // 各バケットの最大スコア(昇順)
  weights: number[];             // バケット重み(正規化済み)
  sortedScores: number[];        // 全コンボのスコア(昇順)
  sortedW: number[];             // 対応する重みの累積
  itemW: number[];               // 各コンボの重み(昇順ソート後)
  idxBounds: number[];           // 各バケットの最終インデックス(昇順)
}

/** レンジをボード上の強さでK個の等重量バケットに分割 */
function bucketize(spec: string, board: Card[], tighten: number, K: number): BucketedRange | null {
  const table = combosOf(spec);
  const items: Array<{ score: number; w: number }> = [];
  ...
  if (tighten > 0) {
    ...
    for (let i = 0; i < cut; i++) items[i].w *= 0.25;
  }
  const total = items.reduce((s, x) => s + x.w, 0);
  const boundaries: number[] = [], weights: number[] = [], idxBounds: number[] = [];
  let acc = 0, bi = 0;
  ...
  const sortedW: number[] = []; let cw = 0;
  for (const x of items) { cw += x.w; sortedW.push(cw); }
...
function winMatrix(A: BucketedRange, B: BucketedRange, K: number): number[][] {
  const W: number[][] = Array.from({ length: K }, () => new Array(K).fill(0.5));
  const cnt: number[][] = Array.from({ length: K }, () => new Array(K).fill(0));
  const cumW = (idx: number) => (idx < 0 ? 0 : B.sortedW[idx]);
  const lowerBound = (s: number, lo: number, hi: number): number => {
    let l = lo, h = hi + 1;
    while (l < h) { const m = (l + h) >> 1; if (B.sortedScores[m] < s) l = m + 1; else h = m; }
    return l; // 最初に score >= s となる位置
  };
  const upperBound = (s: number, lo: number, hi: number): number => {
    let l = lo, h = hi + 1;
    while (l < h) { const m = (l + h) >> 1; if (B.sortedScores[m] <= s) l = m + 1; else h = m; }
    return l; // 最初に score > s となる位置
  };
  ...
  for (let idx = 0; idx < A.sortedScores.length; idx++) {
    ...
    while (bi < K - 1 && idx > A.idxBounds[bi]) bi++;
    for (let j = 0; j < K; j++) {
...
interface RvNode {
  actor: 0 | 1;                  // 0=OOP, 1=IP
  kind: 'decision' | 'showdown' | 'fold';
  folder?: 0 | 1;
  c0: number; c1: number;        // このノートまでのリバー投入額(ポット比)
  labels?: string[];             // decisionの選択肢ラベル
  children?: RvNode[];
  id?: number;                   // regretテーブルのインデックス
}

export interface RiverSolveCtx {
  heroHole: Card[];
  board: Card[];                 // 5枚
  heroSpec: string; villSpec: string;
  heroTighten?: number; villTighten?: number;
  heroIP: boolean;
  pot: number;                   // ストリート開始時点のポット
  effStack: number;              // 残り有効スタック
  facingBet: number;             // 相手の未対応ベット額(0なら自分から)
  rnd?: () => number;
  iters?: number;
  /**
   ...
   */
  bpOut?: {
    firstIn: BpDist[]; faceS: BpDist[]; faceB: BpDist[]; faceJam: BpDist[];
    items: Array<{ bucket: number; hb: string; w: number }>;
  };
  /** DCFR で解く(オフライン蒸留用) */
  discount?: boolean;
}
...
/**
 * バケット勝率行列の上で小さなベットツリーをCFR+で解き、自分の実ハンドの
 * バケットの平均戦略から1アクションを引く共通コア。
 * リバー(厳密な勝率行列)とターン(サンプルリバー平均の勝率行列)の両方から使う。
 */
interface CfrParams {
  K: number;
  W01: number[][];               // P(OOPバケットi が IPバケットj に勝つ)
  w0: ArrayLike<number>;         // OOPバケット重み(正規化済み)
  w1: ArrayLike<number>;         // IPバケット重み
  heroP: 0 | 1;
  heroBucket: number;
  pot: number; effStack: number; facingBet: number;
  rnd: () => number;
  iters: number;
  /** ベットサイズメニュー(ポット比)。省略時 0.5/1.0。フロップは理論に合わせ小さめを渡す */
  sizes?: number[];
  /**
   * ブループリント蒸留用フック。反復後、先手(firstIn)と、相手のベットサイズ別の応答
   * (faceS/faceB/faceJam)について「バケットごとの平均戦略」を畳んで返す。
   * 各要素 [bucket] は正規化済み分布。通常のソルバー呼び出しでは未指定。
   */
  capture?: (firstIn: BpDist[], faceS: BpDist[], faceB: BpDist[], faceJam: BpDist[]) => void;
  /**
   * true なら CFR+ ではなく DCFR で更新する。オフラインのブループリント生成専用。
   * ライブ解法の挙動を変えないよう既定は false(CFR+)にしてある。
   */
  discount?: boolean;
}
function runBucketCfr(ctx: CfrParams): GtoAction | null {
  const K = ctx.K;
  const r = ctx.rnd;
  ...
  let nodeSeq = 0;
  const mkShow = (c0: number, c1: number): RvNode => ({ actor: 0, kind: 'showdown', c0, c1 });
  const mkFold = (folder: 0 | 1, c0: number, c1: number): RvNode => ({ actor: 0, kind: 'fold', folder, c0, c1 });
  // ベットに対する応答(raiseはジャムのみ、ジャムに対してはfold/call)
  const respond = (resp: 0 | 1, myC: number, oppC: number, oppIsJam: boolean): RvNode => {
    const labels = ['fold', 'call'];
    const children: RvNode[] = [
    ...
    if (!oppIsJam && spr > oppC * 2.2) {
      ...
      const jamTo = spr;
      const after: RvNode = {
        actor: (1 - resp) as 0 | 1, kind: 'decision', id: nodeSeq++,
  ...
  const ipAfterCheck: RvNode = { actor: 1, kind: 'decision', id: nodeSeq++, c0: 0, c1: 0,
    labels: ['check', ...SIZES.map((s) => `b${s}`), 'jam'],
    children: [mkShow(0, 0), ...SIZES.map((s) => respond(0, 0, s, false)), respond(0, 0, spr, true)] };
  const root: RvNode = { actor: 0, kind: 'decision', id: nodeSeq++, c0: 0, c1: 0,
    labels: ['check', ...SIZES.map((s) => `b${s}`), 'jam'],
    children: [ipAfterCheck, ...SIZES.map((s) => respond(1, 0, s, false)), respond(1, 0, spr, true)] };

  // ---- CFR+ (ベクトル形式、K=10バケット) ----
  const regret = new Map<number, Float64Array>();
  const ssum = new Map<number, Float64Array>();
  const tbl = (m: Map<number, Float64Array>, id: number, a: number) => {
    let t = m.get(id); if (!t) { t = new Float64Array(K * a); m.set(id, t); } return t;
  };
  const showPay = (i: number, j: number, c0: number, c1: number): [number, number] => {
    const P = 1 + c0 + c1;
    const w = W01[i][j];
    return [w * P - c0, (1 - w) * P - c1];
  };
  const walk = (node: RvNode, r0: Float64Array, r1: Float64Array, t: number): [Float64Array, Float64Array] => {
    ...
    if (node.kind === 'fold') {
      ...
      for (let i = 0; i < K; i++) u0[i] = s1 * (node.folder === 0 ? -node.c0 : win - node.c0);
      for (let j = 0; j < K; j++) u1[j] = s0 * (node.folder === 1 ? -node.c1 : win - node.c1);
      return [u0, u1];
    }
    ...
    for (let i = 0; i < K; i++) {
      let sum = 0;
      for (let a = 0; a < A; a++) sum += Math.max(0, reg[i * A + a]);
      for (let a = 0; a < A; a++) sigma[i * A + a] = sum > 0 ? Math.max(0, reg[i * A + a]) / sum : 1 / A;
    }
    const childU: Array<[Float64Array, Float64Array]> = [];
    for (let a = 0; a < A; a++) {
      const scaled = new Float64Array(K);
      for (let i = 0; i < K; i++) scaled[i] = mine[i] * sigma[i * A + a];
    ...
    for (let a = 0; a < A; a++) {
      ...
      for (let i = 0; i < K; i++) uMine[i] += sigma[i * A + a] * cu[i];
      for (let j = 0; j < K; j++) uOpp[j] += co[j];
    }
    if (ctx.discount) {
      ...
      for (let i = 0; i < K; i++) for (let a = 0; a < A; a++) {
        const idx = i * A + a;
  ...
  const w0 = Float64Array.from(ctx.w0 as number[]), w1 = Float64Array.from(ctx.w1 as number[]);
  for (let t = 1; t <= iters; t++) walk(root, w0, w1, t);

  // ---- ブループリント蒸留: 全バケットの平均戦略を畳んで返す ----
  if (ctx.capture) {
    const small = SIZES.length ? SIZES[0] : 0.33;
    // 指定ノードのバケット別分布を、ブループリント表記へ写して返す
    const readNode = (nd: RvNode | null, face: boolean): BpDist[] => {
      const out: BpDist[] = Array.from({ length: K }, () => ({}));
      if (!nd || nd.kind !== 'decision') return out;
      const A2 = nd.children!.length;
      const ss2 = ssum.get(nd.id!);
      if (!ss2) return out;
      for (let i = 0; i < K; i++) {
        let tot = 0; for (let a = 0; a < A2; a++) tot += ss2[i * A2 + a];
        if (tot <= 0) continue;
        for (let a = 0; a < A2; a++) {
          ...
          let key: string;
          if (face) key = lb === 'fold' ? 'fold' : lb === 'call' ? 'call' : 'raise';
          else if (lb === 'check') key = 'check';
          else if (lb === 'jam') key = 'betB';
          else key = Number(lb.slice(1)) <= small + 1e-9 ? 'betS' : 'betB';
    ...
    let smallNode: RvNode | null = null, bigNode: RvNode | null = null, jamNode: RvNode | null = null;
    src.labels!.forEach((lb, idx) => {
      if (lb === 'check') return;
      const child = src.children![idx];
      if (lb === 'jam') jamNode = child;
      else if (Number(lb.slice(1)) <= small + 1e-9) smallNode = child;
      else bigNode = child;
    });
    // そのサイズが木に無い(SPRが浅くて刈られた)ときは、隣のサイズで代替する
    ctx.capture(
      readNode(firstNode, false),
      readNode(smallNode ?? bigNode, true),
      readNode(bigNode ?? smallNode, true),
      readNode(jamNode ?? bigNode ?? smallNode, true),
    );
  }
  ...
  if (ctx.facingBet <= 0) node = heroP === 0 ? root : ipAfterCheck;
  else {
    ...
    const src = heroP === 0 ? ipAfterCheck : root;
    const cands: Array<{ node: RvNode; size: number }> = [];
    src.labels!.forEach((lb, idx) => {
      if (lb === 'check') return;
  ...
  if (!node || node.kind !== 'decision' || node.actor !== heroP) return null;
  const A = node.children!.length;
  const ss = ssum.get(node.id!);
  if (!ss) return null;
  let freqs: number[] = [];
  let total = 0;
  for (let a = 0; a < A; a++) { freqs.push(ss[myB * A + a]); total += ss[myB * A + a]; }
  if (total <= 0) return null;
  ...
  for (let a = 0; a < A; a++) { x -= freqs[a]; if (x <= 0) { pick = a; break; } }
  const lb = node.labels![pick];
  if (lb === 'check') return { action: 'check' };
  if (lb === 'fold') return { action: 'fold' };
  if (lb === 'call') return { action: 'call' };
  if (lb === 'jam') return { action: 'allin' };
  const size = Number(lb.slice(1));
  return { action: 'raise', to: Math.round(ctx.pot * size) };
}

/**
 * ターンの2ストリート解(V2 Phase6完全版)。
 * ターンのベットツリーの葉(ベット-コール/チェック-チェック)をチェックダウンにせず、
 * サンプルしたリバーごとのチャンスノード → リバーベット層(check/75%/ジャム) → リバー別勝率行列
 * で評価する。これにより「ターンのベットはリバーのベットの予告」(レバレッジ)が均衡に織り込まれ、
 * マルチストリートブラフ(ターンで撃ち、リバーで撃つ/諦める)が自然に出る。
 * バケットはターン時点の強さで固定(不完全記憶)。リバーの強さ変化は Wr(リバー別行列)が担う。
 */
interface TwoStreetParams {
  K: number;
  Wavg: number[][];              // 全リバー平均(ターンのオールイン用)
  Wr: number[][][];              // リバーサンプル別
  w0: ArrayLike<number>; w1: ArrayLike<number>;
  heroP: 0 | 1; heroBucket: number;
  pot: number; effStack: number; facingBet: number;
  rnd: () => number; iters: number;
  sizes: number[];               // ターンのベットサイズ
}
interface TsNode {
  kind: 'decision' | 'chance' | 'showdown' | 'fold';
  actor?: 0 | 1;
  id?: number;
  labels?: string[];
  children?: TsNode[];
  folder?: 0 | 1;
  c0: number; c1: number;        // 投入額(ストリート開始ポット=1単位)
  wIdx?: number;                 // showdown: リバー番号。-1=平均(ターンオールイン)
}
function runTwoStreetCfr(ctx: TwoStreetParams): GtoAction | null {
  const K = ctx.K;
  const r = ctx.rnd;
  const heroP = ctx.heroP;
  ...
  const mkShow = (c0: number, c1: number, wIdx: number): TsNode => ({ kind: 'showdown', c0, c1, wIdx });
  const mkFold = (folder: 0 | 1, c0: number, c1: number): TsNode => ({ kind: 'fold', folder, c0, c1 });

  // ---- リバー層(両者の投入 c で揃った状態から)。サイズ: 75% + ジャム(近ければ) ----
  const riverLayer = (c: number, wIdx: number): TsNode => {
    const potU = 1 + 2 * c;
    const rem = spr - c;
    if (rem <= potU * 0.05) return mkShow(c, c, wIdx); // 実質オールイン済み
    const menu: Array<{ lb: string; amt: number }> = [];
    const b = 0.75 * potU;
    if (b < rem * 0.9) menu.push({ lb: 'rb', amt: b });
    if (rem <= potU * 3 || menu.length === 0) menu.push({ lb: 'rjam', amt: rem });
    const respond = (resp: 0 | 1, oppAmt: number): TsNode => ({
  ...
  const respondTurn = (resp: 0 | 1, oppC: number, oppIsJam: boolean): TsNode => {
    ...
    if (!oppIsJam && spr > oppC * 2.2) {
      labels.push('jam');
      const after: TsNode = {
        kind: 'decision', actor: (1 - resp) as 0 | 1, id: nodeSeq++,
  ...
  const walk = (node: TsNode, r0: Float64Array, r1: Float64Array, t: number): [Float64Array, Float64Array] => {
    const u0 = new Float64Array(K), u1 = new Float64Array(K);
    if (node.kind === 'fold') {
      const win = 1 + node.c0 + node.c1;
      const s1 = r1.reduce((a, b) => a + b, 0), s0 = r0.reduce((a, b) => a + b, 0);
    ...
    if (node.kind === 'chance') {
      ...
      for (const ch of node.children!) {
        ...
        for (let i = 0; i < K; i++) { u0[i] += p * a[i]; u1[i] += p * b[i]; }
      }
      return [u0, u1];
    }
    const A = node.children!.length;
  ...
  const w0 = Float64Array.from(ctx.w0 as number[]), w1 = Float64Array.from(ctx.w1 as number[]);
  for (let t = 1; t <= ctx.iters; t++) walk(root, w0, w1, t);
  ...
  if (ctx.facingBet <= 0) node = heroP === 0 ? root : ipAfterCheck;
    ...
    const frac = ctx.facingBet / Math.max(1, ctx.pot);
    const src = heroP === 0 ? ipAfterCheck : root;
    const cands: Array<{ node: TsNode; size: number }> = [];
    src.labels!.forEach((lb, idx) => {
      if (lb === 'check') return;
      const size = lb === 'jam' ? spr : Number(lb.slice(1));
      cands.push({ node: src.children![idx], size });
  ...
  let freqs: number[] = [];
  let total = 0;
  for (let a = 0; a < A2; a++) { freqs.push(ss[myB * A2 + a]); total += ss[myB * A2 + a]; }
  if (total <= 0) return null;
  freqs = freqs.map((f) => f / total).map((f) => (f < 0.035 ? 0 : f));
  const total2 = freqs.reduce((a, b) => a + b, 0);
  if (total2 <= 0) return null;
  let x = r() * total2, pick = 0;
  for (let a = 0; a < A2; a++) { x -= freqs[a]; if (x <= 0) { pick = a; break; } }
  const lb = node.labels![pick];
  if (lb === 'check') return { action: 'check' };
  if (lb === 'fold') return { action: 'fold' };
  if (lb === 'call') return { action: 'call' };
...
export function solveRiver(ctx: RiverSolveCtx): GtoAction | null {
  const K = 10;
  const heroP: 0 | 1 = ctx.heroIP ? 1 : 0;
  ...
  let myB = 0; while (myB < K - 1 && myScore > heroR.boundaries[myB]) myB++;
  return runBucketCfr({
 ...
 */
export function solveTurn(ctx: RiverSolveCtx): GtoAction | null {
  if (ctx.board.length !== 4) return null;
 ...
 */
export function solveFlop(ctx: RiverSolveCtx): GtoAction | null {
 ...
 */
export function solveFlopBlueprint(ctx: RiverSolveCtx): Record<string, BpNode> | null {
  if (ctx.board.length !== 3) return null;
  const bpOut: NonNullable<RiverSolveCtx['bpOut']> =
    { firstIn: [], faceS: [], faceB: [], faceJam: [], items: [] };
  solveSampledStreet({ ...ctx, bpOut }, 12, [0.33, 0.75]);
  if (!bpOut.items.length || !bpOut.firstIn.length) return null;
  // 決定的handBucketごとに、コンボ重みで各分布を集計
  const acc: Record<string, { first: BpDist; fS: BpDist; fB: BpDist; fJ: BpDist; w: number }> = {};
  const add = (dst: BpDist, src: BpDist, w: number) => {
    for (const k of Object.keys(src)) dst[k] = (dst[k] ?? 0) + (src[k] ?? 0) * w;
  };
  ...
  const norm = (d: BpDist): BpDist => {
    let t = 0; for (const k of Object.keys(d)) t += d[k] ?? 0;
    if (t <= 0) return {};
    const o: BpDist = {}; for (const k of Object.keys(d)) o[k] = Math.round(((d[k] ?? 0) / t) * 1000) / 1000;
    return o;
  };
...
function solveSampledStreet(ctx: RiverSolveCtx, R_SAMPLES: number, sizes: number[]): GtoAction | null {
  const K = 8;
  ...
  const blocked = new Set<Card>([...ctx.board, ...ctx.heroHole]);
  const deck: Card[] = [];
  for (let c = 0 as Card; c < 52; c++) if (!blocked.has(c)) deck.push(c);
  if (deck.length < need) return null;
  const rivers: Card[][] = []; // 各要素が1つのランアウト(ターン=1枚 / フロップ=2枚)
  ...
  // orig = distOf 時点の配列インデックス。bucketizeByStrength が in-place ソートするので、
  // ソート後に byItem(リバー別スコア表)を引くときは必ず orig を使うこと
  interface Item { c1: Card; c2: Card; w: number; strength: number; bucket: number; orig: number }
  const collect = (spec: string): Item[] => {
    const out: Item[] = [];
  ...
  if (A.length < K * 2 || B.length < K * 2) return null;

  // 各ランアウトごとの スコア配列(昇順)+累積重み を両レンジぶん作る
  const board5: Card[] = [...ctx.board, ...new Array<Card>(need).fill(0 as Card)];
  const fillBoard = (runout: Card[]): void => {
    for (let i = 0; i < need; i++) board5[ctx.board.length + i] = runout[i];
  };
  interface RiverDist { scores: number[]; cum: number[]; total: number; byItem: Float64Array }
  const distOf = (items: Item[], runout: Card[]): RiverDist => {
    fillBoard(runout);
    const pairs: Array<[number, number, number]> = []; // [score, w, itemIdx]
    for (let idx = 0; idx < items.length; idx++) {
      const it = items[idx];
      // コンボがランアウトの札を持っていたら、そのランアウトでは存在できない
      let dead = false;
      for (let i = 0; i < need; i++) if (it.c1 === runout[i] || it.c2 === runout[i]) { dead = true; break; }
      if (dead) continue;
      pairs.push([scoreBest([it.c1, it.c2, ...board5]), it.w, idx]);
    }
    pairs.sort((a, b) => a[0] - b[0]);
    const scores: number[] = [], cum: number[] = []; let total = 0;
    const byItem = new Float64Array(items.length).fill(-1);
    for (const [s, w, idx] of pairs) { total += w; scores.push(s); cum.push(total); byItem[idx] = s; }
  ...
  const winProb = (score: number, d: RiverDist): number => {
    if (d.total <= 0) return 0.5;
    let lo = 0, hi = d.scores.length;
    while (lo < hi) { const m = (lo + hi) >> 1; if (d.scores[m] < score) lo = m + 1; else hi = m; }
    const below = lo > 0 ? d.cum[lo - 1] : 0;
    let hi2 = d.scores.length, lo2 = lo;
    while (lo2 < hi2) { const m = (lo2 + hi2) >> 1; if (d.scores[m] <= score) lo2 = m + 1; else hi2 = m; }
    const ties = (lo2 > 0 ? d.cum[lo2 - 1] : 0) - below;
    return (below + 0.5 * ties) / d.total;
  };

  // コンボの強さ = 対レンジ勝率のリバー平均(そのコンボが河でブロックされる回は除外)
  const strengthOf = (items: Item[], myDists: RiverDist[], oppDists: RiverDist[]): void => {
    for (let idx = 0; idx < items.length; idx++) {
      let sum = 0, n = 0;
      for (let r = 0; r < rivers.length; r++) {
        const s = myDists[r].byItem[idx];
        if (s < 0) continue; // このリバーでは存在できないコンボ
        sum += winProb(s, oppDists[r]); n++;
      }
  ...
  const bucketizeByStrength = (items: Item[], tighten: number): { weights: number[]; bounds: number[] } | null => {
    ...
    const bounds = new Array<number>(K).fill(1);
    let acc = 0, bi = 0;
    for (let i = 0; i < items.length; i++) {
      acc += items[i].w;
      items[i].bucket = bi;
      weights[bi] += items[i].w;
      if (acc >= total * (bi + 1) / K && bi < K - 1) { bounds[bi] = items[i].strength; bi++; }
    }
    bounds[K - 1] = 1;
  ...
  const W01: number[][] = Array.from({ length: K }, () => new Array(K).fill(0.5));
  const cnt: number[][] = Array.from({ length: K }, () => new Array(K).fill(0));
  const Wr: number[][][] = rivers.map(() => Array.from({ length: K }, () => new Array(K).fill(0.5)));
  for (let r = 0; r < rivers.length; r++) {
    // Bのバケット別分布(byItem は orig インデックスで引く)
    const bucketPairs: Array<Array<[number, number]>> = Array.from({ length: K }, () => []);
    for (let j = 0; j < B.length; j++) {
      const s = distsB[r].byItem[B[j].orig];
      if (s < 0) continue;
      bucketPairs[B[j].bucket].push([s, B[j].w]);
    }
    const bucketDists: RiverDist[] = bucketPairs.map((pairs) => {
      pairs.sort((a, b) => a[0] - b[0]);
      const scores: number[] = [], cum: number[] = []; let total = 0;
      for (const [s, w] of pairs) { total += w; scores.push(s); cum.push(total); }
      return { scores, cum, total, byItem: new Float64Array(0) };
    });
    const cntR: number[][] = Array.from({ length: K }, () => new Array(K).fill(0));
    for (let i = 0; i < A.length; i++) {
      const s = distsA[r].byItem[A[i].orig];
      if (s < 0) continue;
      const bi = A[i].bucket, w = A[i].w;
      for (let j = 0; j < K; j++) {
        if (bucketDists[j].total <= 0) continue;
  ...
  for (let r = 0; r < rivers.length; r++) {
    fillBoard(rivers[r]);
    heroSum += winProb(scoreBest([...ctx.heroHole, ...board5]), oppDists[r]); heroN++;
  }
  const heroStrength = heroN > 0 ? heroSum / heroN : 0.5;
  const bounds = heroP === 0 ? BA.bounds : BB.bounds;
  let myB = 0; while (myB < K - 1 && heroStrength > bounds[myB]) myB++;
...
export interface River3Ctx {
  heroHole: Card[];
  board: Card[];                       // 5枚
  heroIdx: 0 | 1 | 2;                  // ポストフロップのアクション順での自分
  specs: [string, string, string];     // アクション順のレンジ
  tightens: [number, number, number];
  pot: number;                         // ストリート開始時点のポット
  bettorIdx: number | null;            // ベット者(アクション順index)。null=未ベット
  facingBet: number;                   // 自分から見た未対応額
  rnd?: () => number;
  iters?: number;
}
export function solveRiver3(ctx: River3Ctx): GtoAction | null {
  const K = 6;
  const r = ctx.rnd ?? Math.random;
  if (ctx.board.length !== 5) return null;
  // 各プレイヤーをスコアでバケット化
  ...
  const bucketRange = (b: BucketedRange, j: number): { lo: number; hi: number } => ({
    lo: j === 0 ? 0 : b.idxBounds[j - 1] + 1, hi: b.idxBounds[j],
  });
  const cumW = (b: BucketedRange, idx: number) => (idx < 0 ? 0 : b.sortedW[idx]);
  const bucketW = (b: BucketedRange, j: number) => {
    const { lo, hi } = bucketRange(b, j);
    return cumW(b, hi) - cumW(b, lo - 1);
  };
  const winIn = (s: number, b: BucketedRange, j: number): number => {
    const { lo, hi } = bucketRange(b, j);
    ...
    let l2 = l, h2 = hi + 1;
    while (l2 < h2) { const m = (l2 + h2) >> 1; if (b.sortedScores[m] <= s) l2 = m + 1; else h2 = m; }
    const ties = (cumW(b, l2 - 1) - cumW(b, lo - 1)) - below;
    return (below + 0.5 * ties) / bw;
  };
  // ペア勝率行列 W2[p][q][i][j] = P(pのバケットi が qのバケットj に勝つ)
  const pairW = (A: BucketedRange, B: BucketedRange): number[][] => {
    const W: number[][] = Array.from({ length: K }, () => new Array(K).fill(0.5));
    for (let i = 0; i < K; i++) {
  ...
  const triW = (A: BucketedRange, X: BucketedRange, Y: BucketedRange): number[][][] => {
    const W: number[][][] = Array.from({ length: K }, () =>
      Array.from({ length: K }, () => new Array(K).fill(1 / 3)));
    for (let i = 0; i < K; i++) {
      const { lo, hi } = bucketRange(A, i);
      const wTot = cumW(A, hi) - cumW(A, lo - 1);
      if (wTot <= 0) continue;
      for (let j = 0; j < K; j++) for (let k = 0; k < K; k++) {
        let acc = 0;
        for (let idx = lo; idx <= hi; idx++) {
          const w = A.sortedW[idx] - cumW(A, idx - 1);
  ...
  let nodeSeq = 0;
  type N3 =
    | { kind: 'd'; actor: 0 | 1 | 2; id: number; labels: string[]; children: N3[] }
    | { kind: 'sd'; alive: [boolean, boolean, boolean]; c: [number, number, number] }
    | { kind: 'win'; winner: 0 | 1 | 2; c: [number, number, number] };
  const sd = (alive: [boolean, boolean, boolean], c: [number, number, number]): N3 => {
    const n = alive.filter(Boolean).length;
    if (n === 1) return { kind: 'win', winner: alive.indexOf(true) as 0 | 1 | 2, c };
    return { kind: 'sd', alive, c };
  };
  // ベット後の応答列(順番に fold/call)。resp = 残り応答者列
  const respChain = (bettor: 0 | 1 | 2, resp: Array<0 | 1 | 2>, alive: [boolean, boolean, boolean],
    c: [number, number, number]): N3 => {
    if (!resp.length) return sd(alive, c);
    const [p, ...rest] = resp;
    const cFold: [number, number, number] = [...c] as [number, number, number];
    const aFold: [boolean, boolean, boolean] = [...alive] as [boolean, boolean, boolean];
    aFold[p] = false;
    const cCall: [number, number, number] = [...c] as [number, number, number];
    cCall[p] = c[bettor];
  ...
  const betNode = (bettor: 0 | 1 | 2, resp: Array<0 | 1 | 2>): N3 => {
    const c: [number, number, number] = [0, 0, 0];
    c[bettor] = BET;
    return respChain(bettor, resp, [true, true, true], c);
  };
  const n_p2_xx: N3 = { kind: 'd', actor: 2, id: nodeSeq++, labels: ['check', 'bet'],
    children: [sd([true, true, true], [0, 0, 0]), betNode(2, [0, 1])] };
  const n_p1_x: N3 = { kind: 'd', actor: 1, id: nodeSeq++, labels: ['check', 'bet'],
    children: [n_p2_xx, betNode(1, [2, 0])] };
  const root: N3 = { kind: 'd', actor: 0, id: nodeSeq++, labels: ['check', 'bet'],
  ...
  const sumOf = (v: Float64Array) => { let s = 0; for (let i = 0; i < K; i++) s += v[i]; return s; };
  const walk = (node: N3, rv: [Float64Array, Float64Array, Float64Array], t: number):
    [Float64Array, Float64Array, Float64Array] => {
    const u: [Float64Array, Float64Array, Float64Array] =
      [new Float64Array(K), new Float64Array(K), new Float64Array(K)];
    if (node.kind === 'win') {
      ...
      for (let p = 0 as 0 | 1 | 2; p <= 2; p++) {
        ...
        for (let i = 0; i < K; i++) u[p][i] = oth * gain;
      }
      return u;
    }
    if (node.kind === 'sd') {
      ...
      if (a0 && a1 && a2) {
        ...
        // 2人ショーダウン(1人フォールド済み)
        const pair: Array<[number, number, number[][]]> =
          a0 && a1 ? [[0, 1, W01m]] : a0 && a2 ? [[0, 2, W02m]] : [[1, 2, W12m]];
        ...
        for (let i = 0; i < K; i++) u[pf][i] = othFold * (-node.c[pf]);
      }
      return u;
    }
    const A = node.children.length;
    ...
    const childU: Array<[Float64Array, Float64Array, Float64Array]> = [];
    for (let a = 0; a < A; a++) {
      const scaled = new Float64Array(K);
      for (let i = 0; i < K; i++) scaled[i] = mine[i] * sigma[i * A + a];
      const next = rv.map((v, p) => (p === node.actor ? scaled : v)) as
        [Float64Array, Float64Array, Float64Array];
      childU.push(walk(node.children[a], next, t));
    }
    for (let p = 0 as 0 | 1 | 2; p <= 2; p++) {
      if (p === node.actor) {
        for (let a = 0; a < A; a++) for (let i = 0; i < K; i++) u[p][i] += sigma[i * A + a] * childU[a][p][i];
      } else {
        for (let a = 0; a < A; a++) for (let i = 0; i < K; i++) u[p][i] += childU[a][p][i];
      }
    }
    for (let i = 0; i < K; i++) for (let a = 0; a < A; a++) {
  ...
  const w: [Float64Array, Float64Array, Float64Array] = [
    Float64Array.from(B0.weights), Float64Array.from(B1.weights), Float64Array.from(B2.weights)];
  for (let t = 1; t <= iters; t++) walk(root, w, t);

  // ---- 自分のノードを特定して戦略を読む ----
  const myScore = scoreBest([...ctx.heroHole, ...ctx.board]);
  const heroB = R[ctx.heroIdx]!;
  let myB = 0; while (myB < K - 1 && myScore > heroB.boundaries[myB]) myB++;
  let node: N3 | null = null;
  if (ctx.bettorIdx === null) {
    ...
    let cur: N3 = start;
    while (cur.kind === 'd' && cur.actor !== ctx.heroIdx) cur = cur.children[1]; // call枝を辿る
    node = cur;
  }
  if (!node || node.kind !== 'd' || node.actor !== ctx.heroIdx) return null;
  const A2 = node.children.length;
  const ss = ssum.get(node.id);
  if (!ss) return null;
  let freqs: number[] = [];
...
export function positionLabel(mySeat: number, buttonIndex: number, dealtSeats: number[], maxSeats: number): Pos {
  const n = dealtSeats.length;
  if (n <= 2) return mySeat === buttonIndex ? 'SB' : 'BB';
  const order = (s: number) => (s - (buttonIndex + 1) + maxSeats * 2) % maxSeats;
  const sorted = [...dealtSeats].sort((a, b) => order(a) - order(b)); // SBから
  const idx = sorted.indexOf(mySeat);
  if (idx === 0) return 'SB';
  if (idx === 1) return 'BB';
  if (mySeat === buttonIndex) return 'BTN';
  // ブラインドとBTNを除いた並び。最後がCO、その前は UTG..HJ
  const rest = sorted.slice(2).filter((s) => s !== buttonIndex);
  const ri = rest.indexOf(mySeat);
  if (ri === rest.length - 1 && rest.length >= 2) return 'CO';
  if (ri === rest.length - 2 && rest.length >= 3) return 'HJ';
  return 'UTG';
}
...
export interface PreCtx {
  ...
  tourMode: boolean;
  rnd: () => number;
  aggr: number;          // 0..1 パーソナリティ
  loose: number;         // -0.1..+0.1 レンジ緩め補正
}
...
export function gtoPreflop(c: PreCtx): GtoAction {
  ...
  const inR = (spec: string, bonus = 0) => {
    const f0 = freq(spec, type);
    if (f0 <= 0) return r() < Math.max(0, looseAll);
    const adj = f0 < 1 ? Math.min(0, looseAll) : 0;
    return r() < Math.min(1, Math.max(0, f0 + bonus + adj));
  };
  const jam: GtoAction = { action: 'allin' };
  const openSize = (mult: number) => Math.round(c.bb * mult + c.limpers * c.bb);
  ...
  if (stackBB <= 15 && (c.tourMode || stackBB <= 9)) {
    if (facingShove) {
      ...
      if (stackBB <= 3 || inR(CALL_SHOVE[band])) return { action: 'call' };
      return { action: 'fold' };
    }
    if (c.toCall > 0) {
      // 通常レイズに直面: コール帯より一段タイトにジャムで返すか降りる
      const band = nearestBand([15, 12, 10, 8, 6], stackBB);
      if (inR(CALL_SHOVE[band]) && (type === 'AA' || type === 'KK' || type === 'QQ' || type.startsWith('AK') || r() < 0.75)) return jam;
      return { action: 'fold' };
    }
    ...
    if (!can && stackBB <= 8) can = freq(PUSH_EXTRA_8, type) > 0;
    if (!can && stackBB <= 6 && (c.pos === 'SB' || c.pos === 'BTN')) can = freq(PUSH_EXTRA_6, type) > 0;
    if (stackBB <= 2.2) can = true; // 2bb以下は実質任意の2枚
    if (c.pos === 'BB' && c.toCall === 0) return can && r() < 0.4 ? jam : { action: 'check' };
  ...
  if (c.currentBet <= c.bb && c.toCall <= c.bb) {
    if (c.pos === 'BB' && c.toCall === 0) {
      ...
      if (c.limpers > 0 && inR('99+, ATs+, AJo+, KQs')) return { action: 'raise', to: openSize(3.5) };
      return { action: 'check' };
    }
  ...
  if (facingShove) {
    // 深いスタックでのオールインには上位のみ
    if (inR('QQ+, AKs, AKo:0.6, JJ:0.3')) return { action: 'call' };
    return { action: 'fold' };
  }

  const raiseTo = (mult: number) => ({ action: 'raise' as const, to: Math.round(c.currentBet * mult) });
  const iOpened = c.myStreetBet >= c.bb * 2;

  // ---- 自分がオープン → 3ベットされた ----
  if (iOpened && c.currentBet <= c.bb * 14) {
    if (inR(FOURBET)) return c.currentBet * 2.3 >= (c.myStack + c.myStreetBet) * 0.55 ? jam : raiseTo(2.3);
    if (inR(CALL_3BET)) return { action: 'call' };
    return { action: 'fold' };
  }
  // ---- 自分が3ベット済み → 4ベットされた ----
  if (iOpened || c.currentBet > c.bb * 14) {
    if (c.myStreetBet > c.bb * 6) {
      if (inR(FIVEBET_JAM)) return jam;
      if (inR(CALL_4BET)) return { action: 'call' };
      return { action: 'fold' };
    }
    // コールドで4ベットサイズに直面
    if (inR('QQ+, AKs, AKo:0.5')) return { action: 'call' };
    return { action: 'fold' };
  }

  // ---- オープンに直面(コールド) ----
  if (c.headsUp) {
    if (inR(HU_BB_3BET)) return raiseTo(3.8);
    if (inR(HU_BB_CALL)) return { action: 'call' };
    return { action: 'fold' };
  }
  if (c.pos === 'BB') {
    if (inR(late ? BB_3BET_VS_LATE : BB_3BET_VS_EARLY, c.aggr * 0.08 * TUNE.threeBetScale)) return raiseTo(4);
    if (inR(late ? BB_CALL_VS_LATE : BB_CALL_VS_EARLY, TUNE.bbCallBonus)) return { action: 'call' };
    return { action: 'fold' };
  }
  if (c.pos === 'SB') {
    // SBはほぼ3ベットorフォールド
    if (inR(late ? THREEBET_VS_LATE : THREEBET_VS_EARLY, c.aggr * 0.1 * TUNE.threeBetScale)) return raiseTo(4);
    if (inR('22+:0.5, AQs:0.4, KQs:0.3, AQo:0.2')) return { action: 'call' };
    return { action: 'fold' };
  }
  if (inR(late ? THREEBET_VS_LATE : THREEBET_VS_EARLY, c.aggr * 0.08 * TUNE.threeBetScale)) return raiseTo(3.5);
  if (inR(c.pos === 'BTN' ? COLDCALL_BTN : COLDCALL_MID)) return { action: 'call' };
  return { action: 'fold' };
}
...
interface Texture {
  ...
  connected: boolean;
  aceHigh: boolean;
  broadway: boolean;   // K/Q/Jハイのバラバラ
  dynamicMid: boolean; // T95型: 9〜Jハイ非ペア(どのターンも状況が変わる)
  wet: boolean;
}
function classifyTexture(board: Card[]): Texture {
  const ranks = board.map((c) => (c >> 2) + 2).sort((a, b) => b - a);
  const suits = new Map<number, number>();
  for (const c of board) suits.set(c & 3, (suits.get(c & 3) ?? 0) + 1);
...
interface Draws {
  flushDraw: boolean; nutFlushDraw: boolean; oesd: boolean; gutshot: boolean;
  backdoorFlush: boolean; overcards: boolean; outs: number;
}
function drawFeatures(hole: Card[], board: Card[]): Draws {
  const all = [...hole, ...board];
  const suitCnt = new Map<number, number>();
  for (const c of all) suitCnt.set(c & 3, (suitCnt.get(c & 3) ?? 0) + 1);
  let flushDraw = false, nutFlushDraw = false, backdoorFlush = false;
  for (const [s, n] of suitCnt) {
    const holeOfSuit = hole.filter((h) => (h & 3) === s);
    if (!holeOfSuit.length) continue;
    if (n === 4) {
      flushDraw = true;
      if (holeOfSuit.some((h) => (h >> 2) + 2 === 14)) nutFlushDraw = true;
    } else if (n === 3 && board.length === 3) backdoorFlush = true;
  }
  const rankSet = new Set(all.map((c) => (c >> 2) + 2));
  if (rankSet.has(14)) rankSet.add(1); // Aはローにも
  ...
  for (let lo = 1; lo <= 10; lo++) {
    ...
    if (present.length === 4 && present.some((v) => holeRanks.has(v))) {
      ...
      if (run && present[0] > 1 && present[3] < 14) oesd = true; else gutshot = true;
    }
  }
...
function madeScore(hole: Card[], board: Card[]): number {
  ...
  switch (hv.category) {
    ...
    case HandCategory.Trips: {
      // セット(ポケットペア由来)は最強クラス
      if (pocketPair && boardRanks.includes(h1)) return 0.93;
      return 0.82;
    }
    ...
    case HandCategory.Pair: {
      if (pocketPair && h1 > top) return 0.72;                    // オーバーペア
      ...
      if (pocketPair) return h1 >= boardRanks.sort((a, b) => b - a)[1] ? 0.5 : 0.4;
      const pr = boardRanks.includes(h1) ? h1 : h2;
      const sorted = [...new Set(boardRanks)].sort((a, b) => b - a);
      if (pr === sorted[1]) return 0.46;                          // セカンドペア
      return 0.36;                                                // 下のペア
    }
    default: {
 ...
 */
export function turnCardClass(board: Card[]): 'overcard' | 'board-pair' | 'draw-complete' | 'blank' | null {
  if (board.length !== 4) return null;
  const t = board[3];
  const tr = (t >> 2) + 2;
  const franks = board.slice(0, 3).map((c) => (c >> 2) + 2);
  if (franks.includes(tr)) return 'board-pair';
  // フラッシュ完成: ターンの札を含めて同スートが3枚以上
  let suitN = 0;
  for (const c of board) if ((c & 3) === (t & 3)) suitN++;
  if (suitN >= 3) return 'draw-complete';
  ...
  for (let lo = Math.max(1, tr - 4); lo <= Math.min(10, tr); lo++) {
    let n = 0;
    for (let v = lo; v < lo + 5; v++) if (rset.has(v)) n++;
    if (n >= 3) return 'draw-complete';
  }
  if (tr > Math.max(...franks)) return 'overcard';
  return 'blank';
}

/** リバーのブロッカー情報(§23-24): ナッツスートのAブロッカー / ミスしたFD */
function riverBlockerInfo(hole: Card[], board: Card[]): { nutBlocker: boolean; missedFD: boolean } {
  const cnt = new Map<number, number>();
  for (const c of board) cnt.set(c & 3, (cnt.get(c & 3) ?? 0) + 1);
  ...
  for (const [su, n] of cnt) {
    ...
    if (n >= 3 && mine.length === 1) {
      ...
      if (rk === 14 || rk === 13) nutBlocker = true;
    }
    // ボードに2枚だけ + 手札2枚同スート = FDが完成しなかった(悪いブラフ候補)
    if (n === 2 && mine.length === 2) missedFD = true;
  }
...
export interface PostCtx {
  ...
  myStack: number;
  inPosition: boolean;
  nActive: number;        // 自分含む未フォールド人数
  wasAggressor: boolean;  // 前のアクションで最後に攻めたのが自分か
  tourMode: boolean;
  rnd: () => number;
  aggr: number;
  bluff: number;
  tight: number;
  tier: number;           // 0..1 卓レート(高いほど精度高)
  /** 相手傾向(観測統計)。0.5=平均。confidence=0なら無視される(§39-42) */
  oppFold?: number;       // 高いほど降りやすいプール
  oppAggr?: number;       // 高いほどベット/レイズが多いプール
  oppRaisey?: number;     // 高いほどベットを受けてレイズしてくるプール(V2 §31: 攻めの2変数化)
  conf?: number;          // 0..1 サンプル量に応じた確信度
...
export function gtoPostflop(c: PostCtx): GtoAction {
  const r = c.rnd;
  const conf = c.conf ?? 0;
  ...
  if (c.street === 'river' && c.nActive === 3 && c.board.length === 5 &&
    ...
    try {
      const g = solveRiver3({
        ...
        heroIdx: c.heroOrder as 0 | 1 | 2,
        specs: c.multiwaySpecs as [string, string, string],
        tightens: (c.multiwayTightens ?? [0, 0, 0]) as [number, number, number],
        pot: Math.max(1, c.pot - c.toCall),
        bettorIdx: c.bettorOrder ?? null,
        facingBet: c.toCall, rnd: r,
      });
      if (g) {
        if (g.action === 'fold' && c.toCall <= 0) return { action: 'check' };
        return g;
      }
    } catch { /* ヒューリスティックへ */ }
  }
  ...
  if (c.toCall > 0) {
    ...
    let threshold = required
      ...
      + (bigPressure ? 0.05 : 0);
    if (betFrac <= 0.4) threshold -= TUNE.smallBetDef;       // 小さいベットには広く防御
    // SPR(§19): 低SPRではワンペアの価値UP(コミット)、高SPRではワンペアでスタックオフしない(RIO)
    if (spr < 3 && s >= 0.62) threshold -= 0.05;
    if (spr > 10 && s >= 0.5 && s < 0.72) threshold += 0.05;
    // 相手傾向Exploit(§41-42): アグレッシブなプールにはブラフキャッチを広げ、受動的なプールには降りる
    if (c.street === 'river') threshold += (0.5 - oppAggr) * 0.12 * conf;
    ...
    if (c.villainSpec) {
      const mc = equityVsRange(c.hole, c.board, c.villainSpec,
        { iters: 130, tighten: c.villainAggStreets ?? 0, rnd: r });
      if (mc !== null) equity = Math.min(0.98, mc);
    }
    ...
    if (equity >= threshold || (s >= 0.55 && betFrac <= 0.4)) return { action: 'call' };
    // ブラフキャッチ: リバーでたまにMDF分だけ広げる(高ティアほど正しく)
    if (c.street === 'river' && s >= 0.5 && betFrac <= 1.0 && r() < 0.18 * c.tier) return { action: 'call' };
    return { action: 'fold' };
  }
  ...
  if (t.paired) f += 0.15;
  if (t.aceHigh) f -= 0.08;             // エースハイはKハイより低頻度(実測)
  if (t.broadway && !t.wet) f += 0.1;
  if (t.monotone) f *= 0.45;
  if (t.connected) f -= 0.12;
  ...
  if (c.wasAggressor) {
    if (tc === 'overcard') f += 0.15;
    else if (tc === 'board-pair') f -= 0.18;
    else if (tc === 'draw-complete') f -= 0.15;
  ...
  if (!c.wasAggressor && tc === 'draw-complete') sizeFrac = Math.min(sizeFrac, 0.5);
  if (multiPenalty > 0) sizeFrac = Math.min(sizeFrac, 0.5);

  const betTo = (frac: number) => {
    const amt = Math.max(c.bb, Math.round(c.pot * frac));
    return amt >= c.myStack * 0.9 ? { action: 'allin' as const } : { action: 'raise' as const, to: amt };
  };
  ...
  if (s >= valueThr) {
    ...
    if (t.aceHigh && !t.wet && s >= 0.9 && r() < 0.3) return { action: 'check' };
    // 強い手が脆弱(ウェット/動的) → バリュー前倒しで大きく
    if (s >= 0.75 && (t.dynamicMid || t.wet) && c.street !== 'flop' && multiPenalty === 0 && c.tier > 0.3 && r() < 0.4) {
      return betTo(1.1);
    }
    if (c.street === 'river' && s >= 0.85 && multiPenalty === 0) return betTo(0.75 + r() * 0.5);
    if (r() < Math.min(0.95, f + (s >= 0.72 ? 0.45 : 0.3))) return betTo(s >= 0.72 ? Math.max(sizeFrac, 0.5) : sizeFrac);
    return { action: 'check' };
  }

  // セミブラフ階層: FD/OESD > ガット > バックドア(フロップのみ)
  if (c.street !== 'river') {
    if ((d.flushDraw || d.oesd) && r() < (TUNE.semibluffF + c.aggr * 0.25) * Math.pow(0.6, multiPenalty)) return betTo(t.wet ? 0.66 : 0.5);
    if (d.gutshot && r() < 0.32 * (0.5 + c.aggr) * Math.pow(0.5, multiPenalty)) return betTo(sizeFrac);
    if (c.street === 'flop' && d.backdoorFlush && d.overcards && r() < 0.3 * c.bluff * 2 && multiPenalty === 0) return betTo(sizeFrac);
  }
  ...
  if (s <= 0.4) {
    ...
    if (tc === 'draw-complete' || tc === 'board-pair') p *= 0.5;
    let bluffSize = c.street === 'river' ? 0.66 : sizeFrac;
    if (bi.nutBlocker) { p *= 1.8; bluffSize = 0.75 + r() * 0.35; }
    else if (bi.missedFD) p *= 0.45;
    if (r() < p) return betTo(bluffSize);
  }
  return { action: 'check' };
}
