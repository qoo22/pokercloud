...
export interface HandOptions {
   ...
   */
  straddles?: Array<{ seat: number; amount: number }>;
...
export type HandEvent =
  | { type: 'hand_start'; button: number; sbSeat: number; bbSeat: number }
  | { type: 'ante'; seat: number; amount: number }
  | { type: 'blind'; seat: number; amount: number; blind: 'sb' | 'bb' }
  | { type: 'straddle'; seat: number; amount: number; order: number }
  | { type: 'deal_hole'; seat: number; cards: Card[] }
  | { type: 'street'; street: Street; board: Card[] }
  | { type: 'action'; seat: number; action: ActionType; amount: number; toAmount: number; stack: number; allIn: boolean }
  | { type: 'uncalled_return'; seat: number; amount: number }
  | { type: 'pots'; pots: Pot[] }
  | { type: 'showdown'; seat: number; cards: Card[]; hand: string }
  | { type: 'award'; seat: number; amount: number; potLevel: number }
...
export class Hand {
  ...
  readonly straddles: Array<{ seat: number; amount: number }>;
  ...
  private deck: Deck;
  private deckOrder: Card[];
  private sbSeat: number;
  private bbSeat: number;

  constructor(opts: HandOptions) {
    if (opts.seats.length < 2) throw new Error('プレイヤーは 2 人以上必要です');
    if (opts.seats.length > 9) throw new Error('席は最大 9 です');
    if (opts.seats.some((s) => s.stack <= 0)) throw new Error('スタックが 0 以下の席は着席できません');
  ...
  private begin(): void {
    ...
    let lastStraddleSeat: number | null = null;
  ...
  private nextOccupied(from: number): number {
    ...
    for (let i = 1; i <= n; i++) {
      const seat = (from + i) % n;
      if (!this.players[seat].folded) return seat;
    }
    return from;
  }

  private beginBettingRound(startSeat: number): void {
    for (const p of this.players) {
  ...
  getLegalActions(seat?: number): LegalAction[] {
    ...
    if (s === null || s !== this.actingSeat) return [];
  ...
  act(seat: number, action: ActionType, toAmount?: number): void {
    if (this.street === 'complete') throw new Error('ハンドは既に終了しています');
    ...
    if (!entry) throw new Error(`不正なアクション: ${action}（合法手: ${legal.map((l) => l.type).join(', ')}）`);
    ...
    switch (action) {
      ...
      case 'raise': {
        const to = toAmount ?? 0;
        const maxTo = p.streetBet + p.stack;
        if (to > maxTo) throw new Error(`スタックを超えています（最大 ${maxTo}）`);
        ...
        if (raiseSize >= this.lastRaiseSize) {
          ...
          for (const q of this.players) {
            if (q.seat === p.seat || q.folded || q.allIn) continue;
            q.mustAct = true;
            q.canRaise = true;
          }
        } else {
  ...
  private endStreet(): void {
    ...
    if (this.countCanAct() <= 1) {
      while (this.board.length < 5) this.dealNextStreet();
      this.finish(true);
      return;
    }
  ...
  private finish(showdown: boolean): void {
    ...
    // 3) 役を判定（ショーダウンに来た人のみ）
    const hands: Array<HandValue | null> = this.players.map(() => null);
    ...
    for (const pot of pots) {
      ...
      if (!reachedShowdown) {
        ...
        const candidates: Array<HandValue | null> = this.players.map((p) =>
          pot.eligible.includes(p.seat) ? hands[p.seat] : null,
        );
      ...
      totalRake += rake;
      const distributable: Pot = { ...pot, amount: pot.amount - rake };
    ...
    if (totalRake > 0) this.events.push({ type: 'rake', amount: totalRake });
  ...
  getStateFor(seat: number | null) {
    return {
      ...
      players: this.players.map((p) => ({
        ...
        holeCards:
          p.seat === seat || (this.isComplete && this.result?.showdown && !p.folded)
            ? p.holeCards.slice()
            : null,
      })),
      result: this.result,
  ...
  revealFairness(): FairnessReveal {
    if (!this.fairness) throw new Error('このハンドは Provably Fair を有効にしていません');
    if (!this.isComplete) throw new Error('ハンドが終了する前にシードを開示することはできません');
