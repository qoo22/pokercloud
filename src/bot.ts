 ...
 */
function handStrength(hand: Hand, seat: number): number {
  ...
  if (hand.board.length === 0) {
    ...
    if (r1 === r2) score += 0.35 + (hi - 2) / 12 * 0.15;
    if (suited) score += 0.08;
    if (gap === 1 && r1 !== r2) score += 0.05;
    if (gap > 4) score -= 0.1;
    return Math.max(0, Math.min(1, score));
  }
  // 役名は不要でカテゴリだけ分かればよいので、オブジェクトを作らない scoreBest を使う
  const category = Math.floor(scoreBest([...hole, ...hand.board]) / 759375) as HandCategory;
  const table: Record<HandCategory, number> = {
...
export function decide(hand: Hand, seat: number, rng: Rng, style: BotStyle = 'tight'): BotDecision {
  const legal = hand.getLegalActions(seat);
  if (legal.length === 0) throw new Error('合法手がありません');

  const has = (t: ActionType) => legal.find((l) => l.type === t);
  ...
  if (checkOpt) return { action: 'check' };

  if (callOpt) {
    const price = callOpt.amount ?? 0;
    const potOdds = price / Math.max(1, hand.totalPot + price);
    if (strength >= foldThreshold && strength >= potOdds * 0.9) return { action: 'call' };
    return { action: 'fold' };
  }
...
export function playOut(hand: Hand, rng: Rng, style: BotStyle = 'tight', maxSteps = 500): void {
  let steps = 0;
  while (!hand.isComplete) {
    if (steps++ > maxSteps) throw new Error('ハンドが終了しません（進行ロジックのバグの可能性）');
    const seat = hand.actingSeat;
    if (seat === null) throw new Error('アクション権を持つ席がないのにハンドが完了していません');
