...
import { type Card, rankOf, suitOf, cardToDisplay } from './cards.js';
...
const CATEGORY_NAMES_JA: Record<HandCategory, string> = {
...
const label = (r: number) => RANK_LABEL[r - 2];

/** 日本語の役名（キッカーまで含む説明） */
export function describeHand(hv: HandValue): string {
...
export function handToString(hv: HandValue): string {
...
export function evaluate5(cards: Card[]): HandValue {
  if (cards.length !== 5) throw new Error(`evaluate5 には 5 枚必要です（${cards.length} 枚渡されました）`);

  // ランクごとの枚数
  const counts = new Array(15).fill(0) as number[];
  let suitMask = 0;
  ...
  const grouped: Array<{ rank: number; count: number }> = [];
  for (let r = 14; r >= 2; r--) {
    if (counts[r] > 0) grouped.push({ rank: r, count: counts[r] });
  }
...
function sortForDisplay(
  cards: Card[],
  grouped: Array<{ rank: number; count: number }>,
  straightHigh: number,
): Card[] {
  if (straightHigh === 5) {
...
export function rank5(c0: Card, c1: Card, c2: Card, c3: Card, c4: Card): number {
  _counts.fill(0);
  _counts[(c0 >> 2) + 2]++;
  _counts[(c1 >> 2) + 2]++;
  _counts[(c2 >> 2) + 2]++;
  ...
  let straightHigh = 0;
  if (distinct === 5) {
    if (_ordered[0] - _ordered[4] === 4) straightHigh = _ordered[0];
    else if (_ordered[0] === 14 && _ordered[1] === 5) straightHigh = 5; // ホイール
  }
...
export function scoreBest(cards: Card[]): number {
  ...
  if (n === 5) return rank5(cards[0], cards[1], cards[2], cards[3], cards[4]);
  if (n === 7) {
    let best = -1;
    for (let i = 0; i < 21; i++) {
  ...
  if (n === 6) {
    ...
    for (let skip = 0; skip < 6; skip++) {
      const idx: number[] = [];
      for (let i = 0; i < 6; i++) if (i !== skip) idx.push(i);
 ...
 */
export function findWinners(hands: Array<HandValue | null>): number[] {
