 ...
 */
export function computeUncalledReturn(contributions: number[]): UncalledReturn | null {
  let top = -1;
  let topSeat = -1;
  let second = -1;
  ...
  if (topSeat < 0 || second < 0) return null;
...
export function buildPots(contributions: number[], folded: boolean[]): Pot[] {
  ...
  if (folded.length !== n) throw new Error('contributions と folded の長さが一致していません');
 ...
 */
export function awardPot(pot: Pot, winners: number[], buttonIndex: number, seatCount: number): PotAward {
  const shares = new Map<number, number>();
  if (winners.length === 0) return { pot, shares, oddChipSeat: null };

  const base = Math.floor(pot.amount / winners.length);
  let remainder = pot.amount - base * winners.length;
  for (const w of winners) shares.set(w, base);

  let oddChipSeat: number | null = null;
  if (remainder > 0) {
    // ボタンの左隣から時計回りに走査
    ...
    for (const seat of ordered) {
      ...
      if (oddChipSeat === null) oddChipSeat = seat;
      remainder--;
    }
  }
 ...
 */
export function computeRake(potAmount: number, percent: number, cap: number, sawFlop: boolean): number {
  if (!sawFlop || percent <= 0) return 0;
