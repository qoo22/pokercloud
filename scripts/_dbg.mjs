...
const drive = setInterval(() => {
  ...
  const pick =
    (acted % 5 === 4 && buttons.find((b) => b.dataset.act === 'raise')) ??
...
await sleep(6000);
clearInterval(drive);
console.log('--- actions:',JSON.stringify(document.getElementById('actions').innerHTML.slice(0,400)));
console.log('--- log:',document.getElementById('log').textContent.replace(/\s+/g,' ').slice(-700));

check('自分の手番が回ってきて操作できる', () => {
  if (acted < 5) throw new Error(`押せたのは ${acted} 回だけ`);
});
...
check('共有カードにチップ額と称号が描かれている', () => {
  ...
  if (!text.includes('ルーキー')) throw new Error(`称号が描かれていない: ${text.slice(0, 300)}`);
