...
import { Hand, type SeatConfig } from '../src/table.js';
...
import { Harness, type TestClient } from './helpers/harness.js';
import type { TournamentView } from '../src/server/protocol.js';

const seats = (stacks: number[]): SeatConfig[] =>
  stacks.map((s, i) => ({ id: `P${i}`, name: `P${i}`, stack: s }));
...
describe('ストラドル', () => {
  ...
  test('ストラドルがあってもチップの総量は保存される', () => {
    ...
    for (let i = 0; i < 300; i++) {
      ...
      const stacks: number[] = [];
      for (let k = 0; k < n; k++) stacks.push(500 + rng.randomInt(20000));
...
describe('ミステリーバウンティの封筒', () => {
  test('封筒の合計は必ずプールと一致する', () => {
    for (const [pool, count] of [
      ...
      assert.equal(
        chest.reduce((a, e) => a + e.amount, 0),
        pool,
        `プール ${pool} / ${count} 枚で合計が合わない`,
      );
    }
  });
  ...
  test('第三者が同じ手順で並びを再現できる', () => {
    ...
    const drawn = [];
    for (let i = 0; i < 12; i++) drawn.push(chest.draw()!.amount);

    // 検証側：公開情報だけから同じ並びを作る
    const rebuilt = shuffle(
      buildMysteryChest(300000, 12),
      createFairRng({ serverSeed: seed, clientSeed: 'tour-9', nonce: 0 }),
    ).map((e) => e.amount);
...
describe('バウンティプール', () => {
  ...
  test('PKO：賞金首は撃墜を重ねるほど育つ', () => {
    const p = new BountyPool({ mode: 'progressive', perEntry: 1000 });
    for (const id of ['a', 'b', 'c', 'd']) p.addEntry(id);
  ...
  test('複数人でポットを分けた場合は賞金首も分割される', () => {
    ...
    for (const id of ['a', 'b', 'c']) p.addEntry(id);
    const awards = p.knockoutSplit(['a', 'b'], 'c');
    assert.equal(awards.length, 2);
    assert.equal(
      awards.reduce((x, y) => x + y.cash, 0),
      1001,
      '分割で端数が消えている',
    );
  ...
  test('会計：支払い済み + 未払い = プール（全方式）', () => {
    for (const mode of ['classic', 'progressive'] as const) {
      const p = new BountyPool({ mode, perEntry: 777 });
      const ids = ['a', 'b', 'c', 'd', 'e'];
      for (const id of ids) p.addEntry(id);
      p.knockout('a', 'b');
  ...
  test('ミステリー：有効化前は撃墜しても何も出ない', () => {
    ...
    for (const id of ['a', 'b', 'c', 'd', 'e']) p.addEntry(id);
  ...
  test('有効化の閾値が正しく効く', () => {
    ...
    for (let i = 0; i < 100; i++) p.addEntry(`u${i}`);
...
const lastView = (c: TestClient): TournamentView | null => {
  for (let i = c.received.length - 1; i >= 0; i--) {
    const m = c.received[i];
    if (m.t === 'tournament.state') return m.view;
  }
  return null;
};

function bountyHarness(mode: 'classic' | 'progressive' | 'mystery', players = 3, extra = {}) {
  return new Harness({
    tables: [CASH_TABLE],
    signupBonus: 100000,
    tournaments: [
...
function registerAll(h: Harness, count: number): TestClient[] {
  const cs: TestClient[] = [];
  ...
  for (const c of cs) c.send({ t: 'tour.register', tournamentId: 'b1' });
  for (const c of cs) c.send({ t: 'tour.watch', tournamentId: 'b1' });
  return cs;
}

describe('バウンティトーナメント（統合）', () => {
  for (const mode of ['classic', 'progressive', 'mystery'] as const) {
    ...
    test(`${mode}：最後まで進み、チップ総量が保存される`, () => {
      ...
      const cs = registerAll(h, 3);
      for (let i = 0; i < 3000 && lastView(cs[0])!.state === 'running'; i++) h.pump(500);
  ...
  test('ミステリー：終了後にシードが開示され、コミットメントと一致する', () => {
    ...
    if (commitmentDuring) assert.equal(v.bounty.commitment, commitmentDuring, '途中と終了後でコミットメントが違う');
    h.dispose();
  });
  ...
  test('進行中はミステリーのシードが漏れていない', () => {
    ...
    for (const c of cs) {
      for (const m of c.received) {
        if (m.t !== 'tournament.state') continue;
...
describe('リエントリーとアドオン', () => {
  ...
  test('リエントリー可能な大会では、脱落後にもう一度参加できる', () => {
    ...
    let busted: TestClient | undefined;
    ...
    assert.equal(
      busted!.lastError(),
      null,
      `リエントリーが拒否された（状態=${t.state}, 残り=${lastView(busted!)?.reEntriesLeft}）`,
    );
...
describe('進行速度', () => {
  test('ターボ・ハイパーでレベルが短くなる', () => {
    const make = (speed: 'normal' | 'turbo' | 'hyper') =>
      new Harness({
        tables: [CASH_TABLE],
        tournaments: [
          {
