...
function stubFetch(status: number): void {
  globalThis.fetch = (async () => new Response(status === 200 ? 'db' : '', { status })) as typeof fetch;
}
...
test('ghsync は自分で終了シグナルを握らない', async () => {
  ...
  if (timer) clearInterval(timer);   // 定期タイマーが残るとテストが終われない
...
test('終了前の保存は、送信が固まっても6秒で諦める', async () => {
  ...
  globalThis.fetch = (() => new Promise(() => { /* 永遠に返らない */ })) as unknown as typeof fetch;
  const t0 = Date.now();
