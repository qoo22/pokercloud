...
import { type Card, parseCards, freshDeck, createSeededRng, cardToString } from '../src/cards.js';
...
function buildPresetDeck(opts: {
  ...
  for (const c of freshDeck()) if (!used.has(c)) deck.push(c);
...
const seats = (stacks: number[]): SeatConfig[] =>
  stacks.map((s, i) => ({ id: `P${i}`, name: `Player${i}`, stack: s }));
...
describe('レーキ', () => {
  ...
  test('フロップを見たらポットからレーキが引かれる', () => {
    ...
    assert.equal(
      r.netChange.reduce((a, b) => a + b, 0) + r.totalRake,
      0,
      '収支合計 + レーキ = 0',
    );
  });
...
describe('ランダム自動対戦による不変条件の検証', () => {
  for (const [style, hands] of [
    ...
    test(`${style} ボットで ${hands} ハンド：チップ総量が保存される`, () => {
      ...
      for (let i = 0; i < hands; i++) {
        ...
        const stacks: number[] = [];
        for (let k = 0; k < n; k++) stacks.push(100 + rng.randomInt(20000));
        ...
        // 誰のスタックもマイナスにならない
        for (const p of h.players) assert.ok(p.stack >= 0, `ハンド ${i}: 席 ${p.seat} のスタックが負`);

        // ポット合計と実際の配当が一致する
        const distributed = r.pots.reduce(
          (a, pr) => a + [...pr.shares.values()].reduce((x, y) => x + y, 0) + pr.rake,
          0,
        );
  ...
  test('全席オールインでも正しく決着する', () => {
    ...
    for (let i = 0; i < 500; i++) {
      const n = 2 + rng.randomInt(8);
      const stacks: number[] = [];
      for (let k = 0; k < n; k++) stacks.push(100 + rng.randomInt(3000));
      ...
      let guard = 0;
      while (!h.isComplete) {
        if (guard++ > 200) throw new Error('ハンドが終了しません');
        const seat = h.actingSeat!;
        const legal = h.getLegalActions(seat);
        const push = legal.find((a) => a.type === 'raise' || a.type === 'bet');
        if (push) h.act(seat, push.type, push.max);
        else if (legal.find((a) => a.type === 'call')) h.act(seat, 'call');
        else h.act(seat, 'check');
      }
