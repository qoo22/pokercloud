...
import { PROTOCOL_VERSION, type ClientMessage, type ServerMessage } from '../src/server/protocol.js';
...
class Peer {
  ...
  constructor(url: string) {
    ...
    this.ws.on('message', (d) => {
      ...
      if (msg.t === 'hello.ok') this.userId = msg.userId;
  ...
  wait<T extends ServerMessage['t']>(t: T, timeoutMs = 4000): Promise<Extract<ServerMessage, { t: T }>> {
    ...
    return new Promise((res, rej) => {
      ...
      this.waiters.push({
        ...
        resolve: (m) => {
          clearTimeout(timer);
          res(m as Extract<ServerMessage, { t: T }>);
        },
      });
    });
  }

  waitFor(pred: (m: ServerMessage) => boolean, timeoutMs = 6000): Promise<ServerMessage> {
...
describe('WebSocket 結合テスト', () => {
  ...
  test('2 人が実際に接続して 1 ハンド遊べ、公正性の検証も通る', async () => {
    ...
    const committed = (first as Extract<ServerMessage, { t: 'table.state' }>).state.fairness.commitment!;
    ...
    for (const p of [a, b]) {
      p.ws.on('message', (d) => {
        ...
        const st = msg.state;
        if (!st.legalActions.length || !st.handId) return;
    ...
    for (const m of a.received) {
      if (m.t !== 'table.state') continue;
      if (m.state.street === 'complete') continue;
  ...
  test('切断すると席が解放される', async () => {
    ...
    const seated = await p.waitFor(
      (m) => (m.t === 'table.state' && m.state.yourSeat !== null) || m.t === 'error',
    );
