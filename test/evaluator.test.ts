...
const ev7 = (s: string) => evaluateBest(parseCards(s));
...
describe('強さの比較', () => {
  ...
  test('フルハウスは 3 枚組を先に見る', () => {
    assert.ok(
      compareHands(ev('4s 4h 4d Ac Ad'), ev('5s 5h 5d 2c 2d')) < 0,
      '444AA より 55522 が強い（3枚組が優先）',
    );
  });
...
describe('総当たり検証', () => {
  ...
  test('デッキの 52 枚がすべてユニークで正しく文字列化できる', () => {
    ...
    for (const c of freshDeck()) seen.add(cardToString(c));
