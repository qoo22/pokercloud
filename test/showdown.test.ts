...
import {
  ...
  remainingDeck,
  buildRevealScript,
} from '../solo/showdown.js';

const P = (seat: number, s: string) => ({ seat, hole: parseCards(s) });
...
describe('勝率（全通りの数え上げ）', () => {
  ...
  test('AA vs KK のプリフロップは 81.3%（このスートでの実測値）', () => {
    ...
    assert.ok(
      Math.abs(pct(r.seats[0].equity) - 81.26) < 0.6,
      `AA の勝率が ${pct(r.seats[0].equity)}%（81.26% のはず）`,
    );
  });
  ...
  test('AKs vs QQ のプリフロップは 46.2%（コインフリップ）', () => {
    ...
    assert.ok(
      Math.abs(pct(r.seats[0].equity) - 46.21) < 0.6,
      `AKs の勝率が ${pct(r.seats[0].equity)}%（46.21% のはず）`,
    );
  });
  ...
  test('モンテカルロの結果が数え上げと一致する', () => {
    ...
    assert.ok(
      Math.abs(mc.seats[0].equity - exact.seats[0].equity) < 0.01,
      `推定 ${pct(mc.seats[0].equity)}% と数え上げ ${pct(exact.seats[0].equity)}% がずれている`,
    );
  });
...
describe('アウツ', () => {
  test('セットを追うポケットペアのアウツは 2 枚', () => {
    ...
    for (const c of me.cards) assert.ok(cardToString(c).startsWith('8'));
  });
  ...
  test('フラッシュドローのアウツは 9 枚', () => {
    ...
    for (const c of me.cards) assert.ok(cardToString(c).endsWith('h'), `${cardToString(c)} がハートでない`);
  });
  ...
  test('リバーまで出ていればアウツは無い', () => {
    ...
    for (const x of o) assert.equal(x.cards.length, 0);
  });
...
describe('演出の台本', () => {
  ...
  test('最後のステップの勝率は 0 か 1 に確定する', () => {
    ...
    for (const s of last.equity) assert.ok(s.equity === 0 || s.equity === 1, `${s.equity}`);
  });
