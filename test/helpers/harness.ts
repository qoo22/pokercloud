...
import { Lobby, type Transport, type LobbyConfig } from '../../src/server/lobby.js';
...
export class VirtualClock implements Scheduler {
  private t = 0;
  private seq = 0;
  private items: Array<{ id: number; at: number; fn: () => void }> = [];
  ...
  advance(ms: number): void {
    ...
    for (let guard = 0; guard < 10000; guard++) {
      let next: (typeof this.items)[number] | undefined;
      for (const i of this.items) if (i.at <= target && (!next || i.at < next.at)) next = i;
      if (!next) break;
...
export interface TestClient {
  ...
  /** 最後に受け取ったエラー */
  lastError(): Extract<ServerMessage, { t: 'error' }> | null;
  errors(): Array<Extract<ServerMessage, { t: 'error' }>>;
  results(): HandSummary[];
  clear(): void;
}
...
export class Harness {
  ...
  private autoPlay = new Set<string>();
  ...
  constructor(cfg: LobbyConfig) {
    const transport: Transport = {
      send: (sessionId, msg) => {
        ...
        if (msg.t === 'balance') c.balance = msg.balance;
        if (this.autoPlay.has(sessionId)) this.reactAsBot(c, msg);
      },
      close: () => {},
    };
    ...
    const client: TestClient = {
      ...
      state: () => {
        for (let i = client.received.length - 1; i >= 0; i--) {
          ...
          if (m.t === 'table.state') return m.state;
        }
        return null;
      },
      lastError: () => {
      ...
      errors: () => client.received.filter((m): m is Extract<ServerMessage, { t: 'error' }> => m.t === 'error'),
      results: () =>
        client.received
          .filter((m): m is Extract<ServerMessage, { t: 'hand.result' }> => m.t === 'hand.result')
          .map((m) => m.summary),
      clear: () => {
  ...
  enableBot(c: TestClient, style: BotStyle = 'passive'): void {
    this.autoPlay.add(c.sessionId);
    this.botStyle.set(c.sessionId, style);
  }

  private botStyle = new Map<string, BotStyle>();

  private reactAsBot(c: TestClient, msg: ServerMessage): void {
    if (msg.t !== 'table.state') return;
    const st = msg.state;
    if (!st.legalActions.length || st.handId === null) return;
  ...
  runHands(count: number, stepMs = 200, maxSteps = 5000): number {
   ...
   */
  private completedHandIds(): Set<string> {
    const ids = new Set<string>();
    for (const c of this.clients.values()) {
      for (const m of c.received) if (m.t === 'hand.result') ids.add(m.summary.handId);
    }
    return ids;
  }

  private newHandCount(before: Set<string>): number {
    let n = 0;
    for (const id of this.completedHandIds()) if (!before.has(id)) n++;
    return n;
  }
