...
import { bszEvaluate, bszSpin, bszDouble, bszSpinGrid, BSZ_LINES, BSZ_WEIGHTS, BSZ_SYMS, BSZ_SPECIAL, BSZ_FREE_SPINS, BSZ_MAX_WIN_X, BSZ_DOUBLE_CAP_X, type BszSym, type BszGrid, bszDealDouble, bszResolveDouble, BSZ_RANK } from '../src/server/bsz.js';
...
const G = (s: string): BszGrid => s.trim().split(/\s+/) as BszSym[];
...
describe('WINNING TUNNEL: 盤面の判定', () => {
  test('8ラインは横3・縦3・斜め2', () => {
    ...
    for (const l of BSZ_LINES) {
      ...
      for (const c of l) assert.ok(c >= 0 && c <= 8);
    }
...
describe('WINNING TUNNEL: フリーゲーム', () => {
  ...
  test('トンネル倍率はワイルドを通った役にだけ掛かる(第179弾)', () => {
    ...
    for (let i = 0; i < 120000 && checked < 20; i++) {
      const o = bszSpin(rnd);
      if (!o.freeEntered || o.tunnel <= 1) continue;
      let want = 0;
      for (const st of [o.base, ...o.free]) {
        for (const l of st.lines) want += l.wild ? l.x * o.tunnel : l.x;
        for (const a of st.anys) want += a.x;          // ANY には倍率を掛けない
      }
      assert.equal(o.totalPayX, Math.min(want, BSZ_MAX_WIN_X),
        '倍率がワイルドの役以外にも掛かっている');
      checked++;
    }
...
describe('WINNING TUNNEL: ダブルダウン', () => {
  ...
  test('ブランク3つ揃いでスペシャルは付かない(NaNにならない)', () => {
    ...
    for (let i = 0; i < 40000; i++) {
      ...
      if (r.player.every((s) => s === 'blank')) assert.equal(r.specialX, 0);
    }
  });
...
describe('WINNING TUNNEL: 抽選の健全性', () => {
  test('重みの並びが配当の強さと逆になっている', () => {
    for (const s of BSZ_SYMS) assert.ok(BSZ_WEIGHTS[s] > 0, `${s} の重みが無い`);
    ...
    const order: BszSym[] = ['cherry', 'orange', 'plum', 'melon', 'bell', 'eight', 'bar', 'red7', 'blue7', 'joker'];
    for (let i = 1; i < order.length; i++) {
      assert.ok(
        BSZ_WEIGHTS[order[i]] < BSZ_WEIGHTS[order[i - 1]],
        `${order[i]} が ${order[i - 1]} より薄くない`,
      );
    }
...
describe('WINNING TUNNEL: ジョーカー戻り(リバース)', () => {
  ...
  test('戻り演出を足してもRTPは帯のまま', () => {
    ...
    for (let s = 0; s < 3; s++) {
      ...
      for (let i = 0; i < N / 3; i++) pay += bszSpin(rnd).totalPayX;
    }
    const rtp = pay / N;
    assert.ok(rtp > RTP_LO && rtp < RTP_HI, `RTPが帯を外れた: ${(rtp * 100).toFixed(1)}%`);
  });
...
describe('2段階ダブル', () => {
  test('配った結果は、どれを選んでも変わらない', () => {
    ...
    assert.equal(a.pick, 0); assert.equal(b.pick, 1); assert.equal(c.pick, 2);
  });
  ...
  test('ジョーカーの倍率は配った時点で決まっている', () => {
    ...
    for (let seed = 0; seed < 200; seed++) {
      ...
      if (!deal.player.includes('joker')) continue;
