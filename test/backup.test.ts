...
let seq = 0;
const tmpPath = (tag: string) => join(tmpdir(), `poker-backup-test-${process.pid}-${++seq}-${tag}.db`);

function saveDummyHand(s: SqliteStore, id: string, seatUser: string): void {
  s.saveHand({
...
describe('バックアップ帯域削減', () => {
  test('人間スナップショットは bot とハンド履歴を除外する', async () => {
    ...
    for (let i = 0; i < 50; i++) saveDummyHand(s, `h${i}`, i % 2 ? 'u_alice' : 'bot_zzz');
  ...
  test('botだけが動いても人間指紋は不変(=送信されない)', async () => {
    ...
    const out = tmpPath('sz'); rmSync(out, { force: true });
  ...
  test('pruneBots は hands 表を上限まで刈る(DB肥大の停止)', async () => {
    ...
    for (let i = 0; i < 2100; i++) saveDummyHand(s, `k${i}`, 'u_alice');
  ...
  test('スナップショット生成は本番DBを破壊しない(コピー側だけ削る)', async () => {
    ...
    const out = tmpPath('safe'); rmSync(out, { force: true });
    s.snapshotHumansTo!(out);
...
describe('本番のデータ保存設定', () => {
  ...
  test('render.yaml に永続ディスクがある', () => {
    if (!yml) return;   // 配布物が手元に無い環境ではスキップ
