...
import { Room, type RoomBank, type RoomIO, type Scheduler } from '../src/server/room.js';

/** 手動で進められる時計 */
function fakeClock(): Scheduler & { tick(ms: number): void } {
  let now = 1_000_000;
  let seq = 0;
...
function bankOf(store: SqliteStore): RoomBank {
  return {
