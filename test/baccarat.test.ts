...
import {
  ...
  BAC_DECLARE_RATE,
  type BacCard,
} from '../src/server/baccarat.js';
import { Economy, BAC_MIN_BET, SLOT_CHIP_MAX_BET } from '../src/server/economy.js';
import { MemoryStore } from '../src/server/store.js';

/** 値の並びからカード供給を作る(スートは♠固定。値0は10で表す) */
const feed = (...vals: number[]) => {
  const cards: BacCard[] = vals.map((v) => ({ r: v === 0 ? 10 : v, s: 0 }));
  let i = 0;
  return () => cards[i++];
};
...
describe('バカラ: 数え方と三枚目規則', () => {
  ...
  test('バンカーの三枚目規則(標準テーブル全域)', () => {
    ...
    for (let bt = 0; bt <= 9; bt++) assert.equal(bankerDraws(bt, null), bt <= 5, `bt=${bt} p3なし`);
    // プレイヤー三枚目の値 p3 ごとの規則
  ...
  test('本番の配札: 8デックのシューとして矛盾がない', () => {
    for (let i = 0; i < 200; i++) {
      ...
      const recount = (cs: BacCard[]) => cs.reduce((a, c) => a + bacValue(c.r), 0) % 10;
...
describe('バカラ: 経済(支払い・上限・台帳)', () => {
  ...
  test('下限・残高不足・不正額を弾く(上限は残高のみ)', () => {
    ...
    assert.equal(eco.dealBaccaratHand('u1', { p: 1000.5 as number, b: 0, tie: 0 }, null).ok, false);
  ...
  test('SAFE_POST 超の賭け金も分割記帳で通る(上限なし)', () => {
    ...
    for (let i = 0; i < 3; i++) store.post('u1', 'chips', 7_000_000_000_000_000, 'signup_bonus');
    const before = store.balance('u1', 'chips');
  ...
  test('台帳の理由コードが baccarat_bet / baccarat_win で記録される', () => {
    ...
    if (r.won! > 0) assert.ok(rows.some((x) => x.reason === 'baccarat_win'));
  });
});
