...
const seats = (stacks: number[], names: string[]): SeatConfig[] =>
  stacks.map((s, i) => ({ id: `p${i}`, name: names[i], stack: s }));
...
describe('相手のモデル化', () => {
  ...
  test('レイズが多い相手はアグレッションが上がる', () => {
    const m = new OpponentModel();
    for (let i = 0; i < 60; i++) m.noteAction('raise', false);
...
function playMatch(
  ...
  for (let i = 0; i < decks.length; i++) {
    ...
    while (!h.isComplete) {
      ...
      const d =
        useAi && targetSeats.includes(seat)
    ...
    played++;
    for (const s of targetSeats) net += h.result.netChange[s];
  }
  return { net, played };
}

/** 同じ山札の列を作る */
function makeDecks(seed: number, count: number): number[][] {
  const rng = createSeededRng(seed);
  const out: number[][] = [];
  for (let i = 0; i < count; i++) out.push(shuffle(freshDeck(), rng));
  return out;
}
...
describe('CPU の強さ', () => {
  ...
  test('参加する手の質がボットより高い', () => {
    function selectionQuality(useAi: boolean) {
      ...
      for (let i = 0; i < decks.length; i++) {
        ...
        while (!h.isComplete && guard++ < 400) {
          ...
          if (isTarget && h.street === 'preflop' && (d.action === 'call' || d.action === 'raise')) voluntary = true;
          try {
            h.act(seat, d.action, d.toAmount);
          } catch {
            h.act(seat, h.getLegalActions(seat).some((l) => l.type === 'check') ? 'check' : 'fold');
    ...
    assert.ok(
      ai.avg > bot.avg,
      `参加した手の平均の強さが AI ${ai.avg.toFixed(3)} / ボット ${bot.avg.toFixed(3)} で上回れていない`,
    );
  });
  ...
  test('強い手をフォールドしない', () => {
    ...
    for (let i = 0; i < 40; i++) {
      ...
      if (h.isComplete || h.actingSeat !== 0) continue;
...
describe('チップ経済の設計', () => {
  ...
  test('上位の卓は時間回復では到達できない（勝つしかない）', () => {
    ...
    for (const rank of RANKS) {
      ...
      assert.ok(
        topOfTier.buyIn > rank.rechargeCap,
        `${rank.name} は回復上限 ${rank.rechargeCap} で最上位卓 ${topOfTier.name}（${topOfTier.buyIn}）に座れてしまう`,
      );
    }
  });
  ...
  test('ランクが上がると補充が速くなる', () => {
    withStorage(() => {
      ...
      for (const a of ACHIEVEMENTS) {
        const stat = p.data.stats as unknown as Record<string, number>;
