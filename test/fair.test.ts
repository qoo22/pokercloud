...
describe('SHA-256 の既知テストベクタ', () => {
  ...
  test('ランダムなバイト列 500 本で Node の crypto と一致する', () => {
    ...
    for (let i = 0; i < 500; i++) {
      ...
      for (let k = 0; k < len; k++) buf[k] = rng.randomInt(256);
...
describe('HMAC-SHA256 の既知テストベクタ', () => {
  ...
  test('ランダム入力 300 本で Node の crypto と一致する', () => {
    ...
    for (let i = 0; i < 300; i++) {
      ...
      for (let k = 0; k < kl; k++) key[k] = rng.randomInt(256);
      for (let k = 0; k < ml; k++) msg[k] = rng.randomInt(256);
...
describe('シードの生成と合成', () => {
  ...
  test('毎回異なる値が出る', () => {
    ...
    for (let i = 0; i < 200; i++) set.add(randomSeedHex(32));
...
describe('導出乱数の一様性', () => {
  test('剰余バイアスが無い（randomInt の分布検定）', () => {
    ...
    const counts = new Array(m).fill(0) as number[];
    for (let i = 0; i < n; i++) counts[rng.randomInt(m)]++;
    const expected = n / m;
  ...
  test('導出デッキでも特定のカードが 52 箇所に均等に散る', () => {
    ...
    const positions = new Array(52).fill(0) as number[];
    for (let i = 0; i < n; i++) {
...
describe('Hand への組み込み', () => {
  ...
  test('履歴は進行中は commitment のみ、終了後は serverSeed を含む', () => {
    ...
    assert.equal('serverSeed' in (during.fairness as object), false);

    h.act(0, 'fold');
    const after = h.getHandHistory();
    assert.equal('serverSeed' in (after.fairness as object), true);
  });
