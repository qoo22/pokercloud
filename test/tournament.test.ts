...
function sngHarness(players: number, seatsPerTable = players) {
  return new Harness({
    tables: [CASH_TABLE],
    signupBonus: 100000,
    tournaments: [
...
function registerAll(h: Harness, count: number, style: 'allin' | 'aggressive' = 'allin'): TestClient[] {
  const cs: TestClient[] = [];
  ...
  for (const c of cs) c.send({ t: 'tour.register', tournamentId: 'sng-1' });
  for (const c of cs) c.send({ t: 'tour.watch', tournamentId: 'sng-1' });
  return cs;
}
...
describe('ブラインド構造とペイテーブル', () => {
  ...
  test('配分は上位ほど大きい', () => {
    ...
    for (let i = 1; i < p.length; i++) assert.ok(p[i] <= p[i - 1], `${i} 位が ${i - 1} 位より多い`);
  });
...
describe('Sit & Go', () => {
  ...
  test('最後まで進行し、順位と賞金が確定する', () => {
    ...
    assert.equal(
      ...
      3000,
      `賞金合計 ${prizes.join('+')} がプール 3000 と一致しない`,
    );
...
describe('マルチテーブル（MTT）', () => {
  function mttHarness(players: number, seatsPerTable: number) {
    return new Harness({
      tables: [CASH_TABLE],
      signupBonus: 100000,
      tournaments: [
  ...
  test('全員に順位がつき、賞金合計がプールと一致する', () => {
    ...
    const cs = registerAll(h, 12);
    const t = h.lobby.getTournament('sng-1')!;
    for (let i = 0; i < 4000 && t.state === 'running'; i++) h.pump(500);
    ...
    for (const v of views) {
      if ((v.yourFinishPosition ?? 99) > paid) assert.equal(v.yourPrize, 0, '入賞圏外なのに賞金が出ている');
    }
    h.dispose();
  });
