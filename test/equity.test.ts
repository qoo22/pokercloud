...
import { parseCard, parseCards } from '../src/cards.js';
import {
  calculateEquity,
  heroEquityVsUnknown,
  resolveVisibility,
  ...
  EquityError,
  type EquityRequest,
  type EquityPlayerInput,
} from '../solo/equity.js';

const hole = (s: string): [number, number] => {
  const c = parseCards(s);
  return [c[0], c[1]];
};
const player = (id: string, cards: string, status: EquityPlayerInput['status'] = 'ALL_IN'): EquityPlayerInput => ({
  id,
...
describe('Equity エンジン: モンテカルロ（MONTE_CARLO）', () => {
  ...
  test('モンテカルロは数え上げへ収束する（フロップで照合）', () => {
    ...
    assert.ok(
      Math.abs(mc.players[0].equity - exact.players[0].equity) < 0.01,
      `MC ${pct(mc.players[0].equity)}% vs EXACT ${pct(exact.players[0].equity)}%`,
    );
  });
});
...
describe('Equity エンジン: デッドカード / フォールド', () => {
  test('フォールドした既知札はデッキから除かれ、勝率に影響する', () => {
    ...
    const players: EquityPlayerInput[] = [
      player('a', 'As Ah'),
      player('b', 'Td Tc'),
...
describe('Equity エンジン: バリデーション', () => {
  const good = (): EquityPlayerInput[] => [player('a', 'As Ah'), player('b', 'Kd Kc')];

  test('重複カードを拒否', () => {
    assert.throws(
      () => calculateEquity({ players: [player('a', 'As Ah'), player('b', 'As Kc')], board: [] }),
      (e) => e instanceof EquityError && e.code === 'DUPLICATE_CARD',
    );
  });

  test('ボードとホールの重複も拒否', () => {
    assert.throws(
      () => calculateEquity({ players: good(), board: parseCards('As 2d 3c') }),
      (e) => e instanceof EquityError && e.code === 'DUPLICATE_CARD',
    );
  });
  ...
  test('不正なボード枚数を拒否', () => {
    assert.throws(
      () => calculateEquity({ players: good(), board: parseCards('2d 3c') }),
      (e) => e instanceof EquityError && e.code === 'INVALID_BOARD',
    );
  });

  test('人数不足を拒否', () => {
    assert.throws(
      () => calculateEquity({ players: [player('a', 'As Ah')], board: [] }),
      (e) => e instanceof EquityError && e.code === 'INVALID_PLAYER_COUNT',
    );
  });
  ...
  test('不正なカード整数を拒否', () => {
    assert.throws(
      () => calculateEquity({ players: [{ id: 'a', holeCards: [99, 0], status: 'ALL_IN' }, player('b', 'Kd Kc')], board: [] }),
      (e) => e instanceof EquityError && e.code === 'INVALID_CARD',
    );
  });
});
...
describe('可視性ポリシー（resolveVisibility, §34-36）', () => {
  test('対戦中（手札非公開）のプレイヤーは HERO_VS_UNKNOWN', () => {
    assert.equal(
      resolveVisibility({ isSpectator: false, handComplete: false, cardsPublic: false }),
      EquityVisibilityMode.HERO_VS_UNKNOWN,
    );
  });
  test('ショーダウンで手札公開後のみ SHOWDOWN_ONLY', () => {
    assert.equal(
      resolveVisibility({ isSpectator: false, handComplete: true, cardsPublic: true }),
      EquityVisibilityMode.SHOWDOWN_ONLY,
    );
    // 手札がまだ公開されていなければ実カード勝率は出さない
    assert.equal(
      resolveVisibility({ isSpectator: false, handComplete: true, cardsPublic: false }),
      EquityVisibilityMode.HERO_VS_UNKNOWN,
    );
  });
  test('観戦者は権限がある時だけ SPECTATOR_ALL_KNOWN、無ければ NONE', () => {
    assert.equal(
      resolveVisibility({ isSpectator: true, handComplete: false, cardsPublic: false, spectatorAuthorized: true }),
      EquityVisibilityMode.SPECTATOR_ALL_KNOWN,
    );
    assert.equal(
      resolveVisibility({ isSpectator: true, handComplete: false, cardsPublic: false }),
      EquityVisibilityMode.NONE,
    );
  });
  test('リプレイは REPLAY', () => {
    assert.equal(
      resolveVisibility({ isSpectator: false, handComplete: true, cardsPublic: true, isReplay: true }),
      EquityVisibilityMode.REPLAY,
    );
  });
});
