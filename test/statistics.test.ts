...
describe('7 枚役の出現頻度が理論値と一致する', () => {
  test('モンテカルロ（150,000 サンプル）', () => {
    ...
    const THEORY: Record<HandCategory, number> = {
    ...
    const seven: number[] = new Array(7);

    for (let i = 0; i < n; i++) {
      const deck = shuffle(freshDeck(), rng);
      for (let k = 0; k < 7; k++) seven[k] = deck[k];
    ...
    for (let cat = 0; cat <= 8; cat++) {
      const p = THEORY[cat as HandCategory] / TOTAL;
      const expected = p * n;
      const sd = Math.sqrt(n * p * (1 - p));
      const z = Math.abs(counts[cat] - expected) / sd;
      assert.ok(
        z < 4.5,
        `${names[cat as HandCategory]}: 実測 ${counts[cat]} / 理論 ${expected.toFixed(1)}（${z.toFixed(2)}σ）`,
      );
    }
