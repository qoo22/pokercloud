...
import {
  cardFace, cardBack, cardSlot, VISUAL_CSS, SUIT_KEY,
  CARD_METRICS, PIP_LAYOUT, indexBox, pipBox,
...
const countCenterPips = (html: string): number => (html.match(/class="p"/g) ?? []).length;
...
describe('テーマとアクセシビリティ', () => {
  ...
  test('動きを減らす設定を尊重している', () => {
    assert.ok(
      VISUAL_CSS.includes('prefers-reduced-motion'),
      'アニメーションを止める指定が無い。これが無いと酔う人が出る',
    );
  });
...
describe('画像アセット', () => {
  ...
  test('合計サイズが 120KB を超えない', () => {
    ...
    const bytes = [CARD_BACK_CLASSIC, CARD_BACK_NEON, FELT_TEXTURE].reduce(
      (a, u) => a + Math.floor((u.length - 23) * 0.75),
      0,
    );
...
describe('カードの寸法', () => {
  ...
  function overlap(a: ReturnType<typeof indexBox>, b: ReturnType<typeof pipBox>): number {
  ...
  function bottomIndexBox(rank: string) {
  ...
  test('角の数字とピップが重ならない', () => {
    for (const rank of RANKS) {
      ...
      for (const [x, y] of PIP_LAYOUT[RANK_NUM[rank]]) {
        ...
        assert.equal(
          overlap(bottomIndexBox(rank), pip),
          0,
          `${rank} のピップ (${x}, ${y}) が右下の数字と重なっている`,
        );
      }
    }
  });
  ...
  test('数字とピップの間に読みやすい隙間がある', () => {
    ...
    for (const rank of RANKS) {
      ...
      for (const [x, y] of PIP_LAYOUT[RANK_NUM[rank]]) {
        ...
        assert.ok(
          Math.max(gapX, gapY) >= m.minGap,
          `${rank} のピップ (${x}, ${y}) と数字の隙間が ${Math.max(gapX, gapY).toFixed(3)} しかない`,
        );
      }
    }
  });
  ...
  test('カードの縦横比が定義どおり', () => {
    ...
    for (const [, name, w, h] of found) {
      ...
      assert.ok(
        Math.abs(ratio - CARD_METRICS.aspect) < 0.02,
        `${name} の縦横比が ${ratio.toFixed(3)}（${CARD_METRICS.aspect} のはず）`,
      );
    }
  });
