...
import { MemoryStore, SqliteStore, type Store } from '../src/server/store.js';
import {
  Economy,
  CHIP_PACKS,
  GOLD_PACKS,
  ...
  SLOT_CHIPS_PER_GOLD,
  slotMultiplier,
} from '../src/server/economy.js';
import { SLOT_CHIP_MIN_BET, SLOT_CHIP_MAX_BET, chipBetLadder, SAFE_POST } from '../src/server/economy.js';
import { PAY_SYMBOLS, spin as spinReels, MAX_WIN_X, FREE_MODES, PAYLINES, LINES, SLOT_CFG as ENGINE_CFG } from '../src/server/slot.js';
...
const setup = (store?: Store, now = () => Date.UTC(2026, 7, 5, 12)) => {
...
describe('オファー', () => {
  ...
  test('初回限定は恒常パックより明確にお得', () => {
    ...
    assert.ok(
      offer.sku.chips! / offer.sku.priceJpy > (base.chips! / base.priceJpy) * 4,
      '初回オファーの割安感が足りない',
    );
  });
...
describe('デイリーボーナス', () => {
  ...
  test('連続ログインで増える', () => {
    ...
    for (day = 0; day < 5; day++) amounts.push(e.claimDailyBonus('u1').amount!);
...
describe('ミッション', () => {
  test('進捗が進み、達成すると受け取れる', () => {
    ...
    for (let i = 0; i < m.target; i++) e.advanceMission('u1', m.id);
  ...
  test('二重受け取りはできない', () => {
    const { e } = setup();
    for (let i = 0; i < 20; i++) e.advanceMission('u1', 'play_hands');
  ...
  test('進捗は目標値で頭打ちになる', () => {
    const { e } = setup();
    for (let i = 0; i < 100; i++) e.advanceMission('u1', 'win_hands');
  ...
  test('未消化のデイリーは 7 日以内なら引き継がれる', () => {
    ...
    for (let i = 0; i < 3; i++) e.advanceMission('u1', 'win_hands');
    assert.equal(e.missionStatus('u1').find((m) => m.id === 'win_hands')!.progress, 3);

    day = 1;
    assert.equal(e.missionStatus('u1').find((m) => m.id === 'win_hands')!.progress, 3,
      '翌日に進捗が消えている（持ち越しが効いていない）');
    // 翌日に続きから進めて達成できる
    for (let i = 0; i < 2; i++) e.advanceMission('u1', 'win_hands');
    assert.equal(e.missionStatus('u1').find((m) => m.id === 'win_hands')!.progress, 5);
  ...
  test('保持期間を過ぎたデイリーはリセットされる', () => {
    ...
    let day = 0;
    const e = new Economy(s, () => Date.UTC(2026, 7, 5 + day, 12));
    for (let i = 0; i < 5; i++) e.advanceMission('u1', 'win_hands');
    day = 7; // MISSION_CARRY_DAYS を過ぎる
    assert.equal(e.missionStatus('u1').find((m) => m.id === 'win_hands')!.progress, 0,
      '保持期間を過ぎても残っている（無期限に貯められてしまう）');
    assert.equal(e.claimMission('u1', 'win_hands').ok, false);
  });
  ...
  test('ウィークリー・シーズンのミッションも進んで受け取れる', () => {
    const { s, e } = setup();
    for (let i = 0; i < 3; i++) e.onTournamentEntered('u1');
...
describe('ハンド終了時のフック', () => {
  ...
  test('非課金でもプレイだけで VIP ポイントが貯まる', () => {
    const { e } = setup();
    for (let i = 0; i < 1000; i++) e.onHandPlayed('u1', { won: false, showdownWin: false, rakeContributed: 2000 });
...
describe('永続化（SQLite）', () => {
  test('残高が2^53を超えても読める（本番で hello が沈黙した事故の再発防止）', async () => {
    ...
    s.createUser('u1', 'クジラ');
    for (let i = 0; i < 3; i++) s.post('u1', 'chips', 9_000_000_000_000_000, 'adjustment');
...
describe('デイリーボーナスの逓減', () => {
  ...
  test('KNEE までは従来どおりの率で、体感が落ちない', () => {
    for (const chips of [200_000, 1_000_000, DAILY_KNEE]) {
      assert.equal(dailyBonusBase(chips), Math.round(chips * DAILY_RATE),
        `${chips} チップの支給額が従来と変わっている`);
    }
  });
  ...
  test('ハイローラーの実支給が旧実装より桁違いに小さい', () => {
    ...
    assert.ok(dailyBonusBase(chips) * 25 < old,
      'ハイローラーの支給が十分に抑えられていない');
  });
...
describe('スロット', () => {
  ...
  test('絵柄の重みは正で、配当は揃いにくいものほど大きい', () => {
    for (const sym of PAY_SYMBOLS) assert.ok(sym.weight > 0, `${sym.key} の重みが 0 以下`);
  ...
  const seeded = (seed: number) => { let x = seed >>> 0; return () => { x = (x * 1664525 + 1013904223) >>> 0; return x / 4294967296; }; };
  ...
  test('盤面は5リール×3段で、当たりは左から連続したときだけ', () => {
    ...
    for (const col of r.grid0) assert.equal(col.length, 3, '1リールが3段でない');
  ...
  test('ペイラインは25本で、すべて5リールぶんの段を持つ', () => {
    ...
    for (const line of PAYLINES) {
      ...
      for (const row of line) assert.ok(row >= 0 && row <= 2, `段が範囲外: ${row}`);
    }
    // 同じ形のラインが重複していないこと
      ...
      for (const sp of r.free.spins) {
        ...
        for (const st of sp.steps) {
          ...
          for (const w of st.wins) {
            ...
            sum += w.pay * m;
            if (m === wm) checkedWild++; else checkedPlain++;
          }
          assert.ok(Math.abs(st.payX - sum) < 1e-9,
            `スピンの支払いがライン合計と合わない(seed ${seed}): ${st.payX} ≠ ${sum}`);
        }
      }
    }
  ...
  test('当たりが無いスピンでも盤面が返る（ハズレでリールが止まらない事故の防止）', () => {
    ...
    for (let seed = 1; seed < 200 && checked < 5; seed++) {
      const r = spinReels(seeded(seed), { mode: 'many' });
      if (r.base.length > 0) continue; // 当たったスピンは対象外
  ...
  test('アンティベットはフリーゲームに入りやすくなる', () => {
    const count = (ante: boolean) => {
      let n = 0;
      const rnd = seeded(4321);
      for (let i = 0; i < 4000; i++) if (spinReels(rnd, { ante, mode: 'many' }).freeEntered) n++;
      return n;
    };
  ...
  test('スタックドWILDは全リールで起こりうる(理論上オールワイルドが成立する)', () => {
    ...
    for (let seed = 1; seed < 60000 && seen.size < 5; seed++) {
      for (const reel of spinReels(seeded(seed), { mode: 'few' }).stackedReels) seen.add(reel);
    }
  ...
  test('フリーゲーム中はWILDがコマ単位でホールドされ、増えることはあっても減らない', () => {
    ...
    for (let seed = 1; seed < 20000 && found < 5; seed++) {
      const r = spinReels(seeded(seed), { mode: 'many' });
      if (!r.free || r.free.spins.length < 2) continue;
      found++;
      let prev = new Set<string>();
      for (const sp of r.free.spins) {
        const now = new Set(sp.heldCells.map(([x, y]) => `${x},${y}`));
        // ホールドは解除されない(フリーゲームが終わるまで残る)
        for (const k of prev) assert.ok(now.has(k), `ホールドが外れた(seed ${seed}, ${k})`);
  ...
  test('上乗せは総回数に正しく積まれ、残り回数と食い違わない', () => {
    const startBy: Record<number, number> = ENGINE_CFG.freeSpinsByScatter;
    ...
    } finally { ENGINE_CFG.scatterWeight = orig; }
  });
  ...
  test('連鎖は発生しない(1スピンにつき判定は1回だけ)', () => {
    for (let seed = 1; seed < 3000; seed++) {
      ...
      if (r.respin) assert.ok(r.respin.steps.length <= 1, `リスピンで連鎖している(seed ${seed})`);
      for (const sp of r.free?.spins ?? []) {
  ...
  test('WILD倍率はWILDを含む当選ラインにだけ・1ラインにつき1回だけ付く', () => {
    ...
    for (let seed = 1; seed < 8000; seed++) {
      ...
      for (const st of [...r.base, ...(r.respin?.steps ?? [])]) {
        for (const w of st.wins) assert.equal(w.wildMult, undefined, `通常時にWILD倍率が付いている(seed ${seed})`);
      }
      for (const sp of r.free?.spins ?? []) {
        for (const st of sp.steps) {
          for (const w of st.wins) {
            const line = PAYLINES[w.line!];
            let hasWild = false;
            for (let i = 0; i < w.count; i++) if (st.grid[i][line[i]] === 'wild') hasWild = true;
            if (hasWild) {
              withWild++;
    ...
    for (let seed = 1; seed < 4000; seed++) {
      ...
      for (const st of [...r.base, ...(r.respin?.steps ?? [])]) {
        for (const w of st.wins) {
          ...
          // 当選区間に「その絵柄そのもの」があるか
          let has = false;
          for (let i = 0; i < w.count; i++) if (st.grid[i][line[i]] === w.key) has = true;
          if (w.allWild) {
            all++;
  ...
  test('チップ建てでは払い出し倍率が掛からない（無限増殖の防止）', () => {
    const a = setup(); a.s.post('u1', 'chips', 10_000_000, 'adjustment');
    const b = setup(); b.s.post('u1', 'chips', 10_000_000, 'adjustment');
  ...
  test('フリーゲームは必ず終わる（再トリガーに上限は無いが暴走止めに触れない）', () => {
    ...
    for (let seed = 1; seed < 3000; seed++) {
      ...
      assert.ok(r.free.spins.length < ENGINE_CFG.freeSpinsGuard,
        `フリーゲームが暴走止めまで回った(seed ${seed}, ${r.free.spins.length}回)`);
      assert.equal(r.free.spins.length, Math.min(r.free.spinsTotal, ENGINE_CFG.freeSpinsGuard),
        `消化した回数と総回数が食い違う(seed ${seed})`);
    }
    // 期待上乗せが小さいので、実際には暴走止めのはるか手前で終わる
    assert.ok(worst < ENGINE_CFG.freeSpinsGuard / 2,
      `再トリガーが収束していない(最長 ${worst} 回)`);
  });
  ...
  test('突入回数と上乗せ回数がスキャッターの個数どおり', () => {
    ...
    const startBy: Record<number, number> = ENGINE_CFG.freeSpinsByScatter;
    const addBy: Record<number, number> = ENGINE_CFG.freeRetriggerByScatter;
    ...
    for (let seed = 1; seed < 6000 && (checkedStart < 20 || checkedAdd < 5); seed++) {
      ...
      assert.equal(startSpins, startBy[Math.min(r.scatters, 5)],
        `突入回数が違う(seed ${seed}, スキャッター ${r.scatters}個 → ${startSpins}回)`);
      checkedStart++;
      for (const sp of r.free.spins) {
        if (!sp.addedSpins) { assert.ok(sp.scatters < 3, '3個以上なのに上乗せが無い'); continue; }
        assert.equal(sp.addedSpins, addBy[Math.min(sp.scatters, 5)],
          `上乗せ回数が違う(スキャッター ${sp.scatters}個 → +${sp.addedSpins})`);
        checkedAdd++;
      }
    }
  ...
  test('賭け金の選択肢は所持チップに合わせて上限が開き、下限は常に残る', () => {
    ...
    assert.deepEqual(chipBetLadder(50_000), [1000, 2000, 5000, 10000, 20000, 50000],
      '5万所持のはしごが想定と違う');
    for (const list of [small, big]) {
      assert.ok(list.length > 0, '選択肢が空');
      assert.ok(list.length <= 12, `選択肢が多すぎる: ${list.length}`);
      for (const v of list) assert.ok(v >= SLOT_CHIP_MIN_BET, '下限を割る選択肢が出ている');
      for (let i = 1; i < list.length; i++) assert.ok(list[i] > list[i - 1], '昇順になっていない');
    }
    // 所持を超える選択肢は出さない
  ...
  test('賭け金は自由額（不正な値だけ拒否される）', () => {
    ...
    for (const bad of [0, -5, 1.5, NaN, Infinity, 999]) {   // 999 は下限(1,000)未満
      assert.equal(e.spinSlot('u1', bad as number).ok, false, `${bad} が通ってしまった`);
    }
  ...
  test('賭け金は50兆で頭打ちになる（例外でスピンが壊れない）', () => {
    ...
    for (const v of ladder) assert.ok(v <= SLOT_CHIP_MAX_BET, `はしごに上限超えの段がある: ${v}`);
